local a={cache={}::any}do do local function b()













local c=game:GetService('UserInputService');
local d=game:GetService('TextService');
local e=game:GetService('CoreGui');
local f=game:GetService('Teams');
local g=game:GetService('Players');
local h=game:GetService('RunService')
local i=game:GetService('TweenService');
local j=game:GetService('Lighting');
local k=h.RenderStepped;
local l=g.LocalPlayer;
local m=l:GetMouse();

local n=protectgui or(syn and syn.protect_gui)or(function()end);

writefile(
"LoRes12OT-Regular.ttf",
game:HttpGet("https://github.com/sudoaptinstallopsec123/new/raw/refs/heads/main/LoRes12OT-Regular.ttf")
)

writefile("LoRes12OT-Regular.font",game:GetService("HttpService"):JSONEncode({
name="LoRes12OT-Regular",
faces={
{
name="Regular",
weight=400,
style="normal",
assetId=getcustomasset("LoRes12OT-Regular.ttf"),
},
{
name="Bold",
weight=700,
style="normal",
assetId=getcustomasset("LoRes12OT-Regular.ttf"),
},
},
}))


local o=Font.new(getcustomasset("LoRes12OT-Regular.font"),Enum.FontWeight.Regular)

local p=Instance.new('ScreenGui');
n(p);
p.ZIndexBehavior=Enum.ZIndexBehavior.Global;
p.Parent=e;

local q={};
local r={};

getgenv().Toggles=q;
getgenv().Options=r;

local s={
Registry={};
RegistryMap={};

HudRegistry={};

FontColor=Color3.fromRGB(255,255,255);
MainColor=Color3.fromRGB(28,28,28);
BackgroundColor=Color3.fromRGB(20,20,20);
AccentColor=Color3.fromRGB(0,85,255);
OutlineColor=Color3.fromRGB(50,50,50);
RiskColor=Color3.fromRGB(255,50,50),

Black=Color3.new(0,0,0);

Font=o,
FontFace=o,
FontSize=16,

OpenedFrames={};
DependencyBoxes={};

Signals={};
ScreenGui=p;

Toggled=false;
WireframeDrag=false;
UseBlur=false;
BlurSize=15;

KeybindMode='All';

NotifyConfig={
Alignment='Left';
BarSide='Left';
PositionX=0;
PositionY=40;
};
};

s.KeyPickerList={};

s.BlurEffect=Instance.new("BlurEffect")
s.BlurEffect.Name="LinoriaBlur"
s.BlurEffect.Size=0
s.BlurEffect.Enabled=false
pcall(function()s.BlurEffect.Parent=j end)

function s:UpdateBlur()
if s.UseBlur then
if s.Toggled then
s.BlurEffect.Enabled=true
i:Create(s.BlurEffect,TweenInfo.new(0.2,Enum.EasingStyle.Linear),{Size=s.BlurSize}):Play()
end
else
local t=i:Create(s.BlurEffect,TweenInfo.new(0.2,Enum.EasingStyle.Linear),{Size=0})
t:Play()

task.delay(0.2,function()
if not s.UseBlur then
s.BlurEffect.Enabled=false
end
end)
end
end

function s:SetFontSize(t)
s.FontSize=t
for u,v in pairs(p:GetDescendants())do
if v:IsA("TextLabel")or v:IsA("TextBox")or v:IsA("TextButton")then
local w=v:GetAttribute("FontSizeOffset")
if w then
v.TextSize=t+w
end
end
end
local u=e:FindFirstChild("LinoriaMobileUI")
if u then
for v,w in pairs(u:GetDescendants())do
if w:IsA("TextLabel")or w:IsA("TextBox")or w:IsA("TextButton")then
local x=w:GetAttribute("FontSizeOffset")
if x then
w.TextSize=t+x
end
end
end
end
end

local t=0
local u=0

table.insert(s.Signals,k:Connect(function(v)
t=t+v

if t>=(1/60)then
t=0

u=u+(1/400);
if u>1 then
u=0;
end;

s.CurrentRainbowHue=u;
s.CurrentRainbowColor=Color3.fromHSV(u,0.8,1);
end
end))

local function v()
local w=g:GetPlayers();
for x=1,#w do
w[x]=w[x].Name;
end;
table.sort(w,function(x,y)return x<y end);

return w
end;

local function w()
local x=f:GetTeams();
for y=1,#x do
x[y]=x[y].Name;
end;
table.sort(x,function(y,z)return y<z end);

return x
end;

function s:SafeCallback(x,...)
if(not x)then
return
end;
if not s.NotifyOnError then
return x(...)
end;

local y,z=pcall(x,...);
if not y then
local A,B=z:find(":%d+: ");
if not B then
return s:Notify(z)
end;
return s:Notify(z:sub(B+1),3)
end;
end;

function s:AttemptSave()
if s.SaveManager then
s.SaveManager:Save();
end;
end;

function s:Create(x,y)
local z=x;
if type(x)=='string'then
z=Instance.new(x);
end;
for A,B in next,y do
if A=='Font'and typeof(B)=='Font'then
z.FontFace=B;
else
z[A]=B;
end;
end;

if z:IsA("TextLabel")or z:IsA("TextBox")or z:IsA("TextButton")then
if y.TextSize then
z:SetAttribute("FontSizeOffset",y.TextSize-s.FontSize)
else
z:SetAttribute("FontSizeOffset",0)
end
end

return z
end;
function s:ApplyTextStroke(x)
x.TextStrokeTransparency=1;

s:Create('UIStroke',{
Color=Color3.new(0,0,0);
Thickness=1;
LineJoinMode=Enum.LineJoinMode.Miter;
Parent=x;
});
end;

function s:ApplyGlow(x)

end;

function s:CreateLabel(x,y)
local z=s:Create('TextLabel',{
BackgroundTransparency=1;
FontFace=s.FontFace;
TextColor3=s.FontColor;
TextSize=s.FontSize+2;
TextStrokeTransparency=0;
});
s:ApplyTextStroke(z);

s:AddToRegistry(z,{
TextColor3='FontColor';
},y);
return s:Create(z,x)
end;

function s:MakeDraggable(x,y,z)
x.Active=true;
x.InputBegan:Connect(function(A)
if A.UserInputType==Enum.UserInputType.MouseButton1 or A.UserInputType==Enum.UserInputType.Touch then
local B=x.Position
local C=A.Position

if(C.Y-x.AbsolutePosition.Y)>(y or 40)then
return
end

local D=true
local E=false
local F=nil
local G,H

G=c.InputChanged:Connect(function(I)
if I.UserInputType==Enum.UserInputType.MouseMovement or I==A then
local J=I.Position-C

if z and s.WireframeDrag then
if not E and J.Magnitude>2 then
E=true

F=s:Create("Frame",{
Size=x.Size,
Position=x.Position,
AnchorPoint=x.AnchorPoint,
BackgroundTransparency=1,
Active=false,
ZIndex=100000,
Parent=p
})

s:Create("UIStroke",{
Color=s.AccentColor,
Thickness=1,
ApplyStrokeMode=Enum.ApplyStrokeMode.Border,
Parent=F
})
end

if E and F then
F.Position=UDim2.new(
B.X.Scale,B.X.Offset+J.X,
B.Y.Scale,B.Y.Offset+J.Y
)
end
else
x.Position=UDim2.new(
B.X.Scale,B.X.Offset+J.X,
B.Y.Scale,B.Y.Offset+J.Y
)
end
end
end)

H=c.InputEnded:Connect(function(I)
if I==A or I.UserInputType==Enum.UserInputType.Touch then
D=false
G:Disconnect()
H:Disconnect()

if z and s.WireframeDrag and E and F then
x.Position=F.Position

F:Destroy()
F=nil
end
end
end)
end
end)
end;

function s:MakeResizable(x,y,z)
y=y or Vector2.new(400,300)
z=z or Vector2.new(1400,1000)

local A=s:Create('TextButton',{
Name='ResizeGrip',
Text='',
AutoButtonColor=false,
BackgroundTransparency=1,
Size=UDim2.fromOffset(16,16),
Position=UDim2.new(1,-4,1,-4),
AnchorPoint=Vector2.new(1,1),
ZIndex=25,
Parent=x,
})

local B=s:CreateLabel({
BackgroundTransparency=1,
Size=UDim2.fromOffset(16,16),
Position=UDim2.new(1,0,1,0),
AnchorPoint=Vector2.new(1,1),
Text='◢',
TextColor3=s.OutlineColor,
TextSize=s.FontSize+2,
ZIndex=26,
Parent=A,
})
s:AddToRegistry(B,{
TextColor3='OutlineColor',
})

A.InputBegan:Connect(function(C)
if C.UserInputType~=Enum.UserInputType.MouseButton1
and C.UserInputType~=Enum.UserInputType.Touch then
return
end

local D=x.Size
local E=C.Position
local F=false
local G=nil
local H,I

H=c.InputChanged:Connect(function(J)
if J.UserInputType~=Enum.UserInputType.MouseMovement and J~=C then
return
end

local K=J.Position-E
if K.Magnitude<=2 then return end
F=true

local L=math.clamp(D.X.Offset+K.X,y.X,z.X)
local M=math.clamp(D.Y.Offset+K.Y,y.Y,z.Y)
local N=UDim2.fromOffset(L,M)

if s.WireframeDrag then
if not G then
G=s:Create('Frame',{
Size=x.Size,
Position=x.Position,
AnchorPoint=x.AnchorPoint,
BackgroundTransparency=1,
Active=false,
ZIndex=100000,
Parent=p,
})
s:Create('UIStroke',{
Color=s.AccentColor,
Thickness=1,
ApplyStrokeMode=Enum.ApplyStrokeMode.Border,
Parent=G,
})
end
G.Size=N
G.Position=x.Position
else
x.Size=N
end
end)

I=c.InputEnded:Connect(function(J)
if J~=C and J.UserInputType~=Enum.UserInputType.Touch then
return
end

H:Disconnect()
I:Disconnect()

if s.WireframeDrag and F and G then
x.Size=G.Size
G:Destroy()
end
end)
end)
end;

function s:AddToolTip(x,y)
local z,A=s:GetTextBounds(x,s.Font,s.FontSize);
local B=s:Create('Frame',{
BackgroundColor3=s.MainColor,
BorderSizePixel=1,
BorderColor3=s.AccentColor,

Size=UDim2.fromOffset(z+5,A+4),
ZIndex=100,
Parent=s.ScreenGui,

Visible=false,
})


local C=s:CreateLabel({
Position=UDim2.fromOffset(3,1),
Size=UDim2.fromOffset(z,A);
TextSize=s.FontSize;
Text=x,
TextColor3=s.FontColor,
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=B.ZIndex+1,

Parent=B;
});
s:AddToRegistry(B,{
BackgroundColor3='MainColor';
BorderColor3='AccentColor';
});
s:AddToRegistry(C,{
TextColor3='FontColor',
});
local D=false

y.MouseEnter:Connect(function()
if s:MouseIsOverOpenedFrame()then
return
end

D=true

B.Position=UDim2.fromOffset(m.X+15,m.Y+12)
B.Visible=true

while D do
h.Heartbeat:Wait()
B.Position=UDim2.fromOffset(m.X+15,m.Y+12)
end
end)

y.MouseLeave:Connect(function()
D=false
B.Visible=false
end)
end

local x=TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)

local function y(z,A)
local B=i:Create(z,x,A)
B:Play()
return B
end

function s:OnHighlight(z,A,B,C)
local D,E

z.MouseEnter:Connect(function()
if E then E:Cancel()end

local F=s.RegistryMap[A]
local G={}

for H,I in next,B do
local J=s[I]or I
G[H]=J

if F and F.Properties[H]then
F.Properties[H]=I
end
end

D=y(A,G)
end)

z.MouseLeave:Connect(function()
if D then D:Cancel()end

local F=s.RegistryMap[A]
local G={}

for H,I in next,C do
local J=s[I]or I
G[H]=J

if F and F.Properties[H]then
F.Properties[H]=I
end
end

E=y(A,G)
end)
end

function s:MouseIsOverOpenedFrame()
for z,A in next,s.OpenedFrames do
local B,C=z.AbsolutePosition,z.AbsoluteSize;
if m.X>=B.X and m.X<=B.X+C.X
and m.Y>=B.Y and m.Y<=B.Y+C.Y then

return true
end;
end;
end;

function s:IsMouseOverFrame(z)
local A,B=z.AbsolutePosition,z.AbsoluteSize;
if m.X>=A.X and m.X<=A.X+B.X
and m.Y>=A.Y and m.Y<=A.Y+B.Y then

return true
end;
end;

function s:UpdateDependencyBoxes()
for z,A in next,s.DependencyBoxes do
A:Update();
end;
end;

function s:MapValue(z,A,B,C,D)
return(1-((z-A)/(B-A)))*C+((z-A)/(B-A))*D
end;

function s:GetTextBounds(z,A,B,C)
local D=Instance.new("GetTextBoundsParams")
D.Text=z
D.Font=typeof(A)=='Font'and A or Font.new("rbxasset://fonts/families/LegacyArial.json")
D.Size=B
D.Width=math.huge

local E,F=pcall(function()
return d:GetTextBoundsAsync(D)
end)

if E and F then
return F.X,F.Y
end


local G=d:GetTextSize(z,B,Enum.Font.Code,Vector2.new(math.huge,math.huge))
return G.X,G.Y
end

function s:GetDarkerColor(z)
local A,B,C=Color3.toHSV(z);
return Color3.fromHSV(A,B,C/1.5)
end;

s.AccentColorDark=s:GetDarkerColor(s.AccentColor);

function s:AddToRegistry(z,A,B)
local C=#s.Registry+1;
local D={
Instance=z;
Properties=A;
Idx=C;
};

table.insert(s.Registry,D);
s.RegistryMap[z]=D;

if B then
table.insert(s.HudRegistry,D);
end;
end;

function s:RemoveFromRegistry(z)
local A=s.RegistryMap[z];

if A then
for B=#s.Registry,1,-1 do
if s.Registry[B]==A then
table.remove(s.Registry,B);
end;
end;

for B=#s.HudRegistry,1,-1 do
if s.HudRegistry[B]==A then
table.remove(s.HudRegistry,B);
end;
end;

s.RegistryMap[z]=nil;
end;
end;

function s:UpdateColorsUsingRegistry()
for z,A in next,s.Registry do
for B,C in next,A.Properties do
if type(C)=='string'then
A.Instance[B]=s[C];
elseif type(C)=='function'then
A.Instance[B]=C()
end
end;
end;
end;

function s:GiveSignal(z)
table.insert(s.Signals,z)
end

function s:Unload()
for z=#s.Signals,1,-1 do
local A=table.remove(s.Signals,z)
A:Disconnect()
end

if s.OnUnload then
s.OnUnload()
end

if s.BlurEffect then
s.BlurEffect:Destroy()
end

p:Destroy()
end

function s:OnUnload(z)
s.OnUnload=z
end

s:GiveSignal(p.DescendantRemoving:Connect(function(z)
if s.RegistryMap[z]then
s:RemoveFromRegistry(z);
end;
end))

local z={};
do
local A={};

function A:AddColorPicker(B,C)
local D=self.TextLabel;
assert(C.Default,'AddColorPicker: Missing default value.');

local E={
Value=C.Default;
Transparency=C.Transparency or 0;
Type='ColorPicker';
Title=type(C.Title)=='string'and C.Title or'Color picker',
Callback=C.Callback or function(E)end;
};

function E:SetHSVFromRGB(F)
local G,H,I=Color3.toHSV(F);
E.Hue=G;
E.Sat=H;
E.Vib=I;
end;

E:SetHSVFromRGB(E.Value);
local F=s:Create('Frame',{
BackgroundColor3=E.Value;
BorderColor3=s:GetDarkerColor(E.Value);
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(0,28,0,14);
ZIndex=6;
Parent=D;
});






local G=s:Create('ImageLabel',{
BorderSizePixel=0;
Size=UDim2.new(0,27,0,13);
ZIndex=5;
Image='http://www.roblox.com/asset/?id=12977615774';
Visible=not not C.Transparency;
Parent=F;
});

local H=s:Create('Frame',{
Name='Color';
BackgroundColor3=Color3.new(1,1,1);
BorderSizePixel=1;
BorderColor3=Color3.new(0,0,0);
Position=UDim2.fromOffset(F.AbsolutePosition.X,F.AbsolutePosition.Y+18),
Size=UDim2.fromOffset(230,C.Transparency and 271 or 253);
Visible=false;
ZIndex=15;
Parent=p,
});
F:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
H.Position=UDim2.fromOffset(F.AbsolutePosition.X,F.AbsolutePosition.Y+18);
end)

local I=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=16;
Parent=H;
});
local J=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
Size=UDim2.new(1,0,0,2);
ZIndex=17;
Parent=I;
});
local K=s:Create('Frame',{
BorderColor3=Color3.new(0,0,0);
Position=UDim2.new(0,4,0,25);
Size=UDim2.new(0,200,0,200);
ZIndex=17;
Parent=I;
});
local L=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=18;
Parent=K;
});
local M=s:Create('ImageLabel',{
BorderSizePixel=0;
Size=UDim2.new(1,0,1,0);
ZIndex=18;
Image='rbxassetid://4155801252';
Parent=L;
});
local N=s:Create('ImageLabel',{
AnchorPoint=Vector2.new(0.5,0.5);
Size=UDim2.new(0,6,0,6);
BackgroundTransparency=1;
Image='http://www.roblox.com/asset/?id=9619665977';
ImageColor3=Color3.new(0,0,0);
ZIndex=19;
Parent=M;
});
local O=s:Create('ImageLabel',{
Size=UDim2.new(0,N.Size.X.Offset-2,0,N.Size.Y.Offset-2);
Position=UDim2.new(0,1,0,1);
BackgroundTransparency=1;
Image='http://www.roblox.com/asset/?id=9619665977';
ZIndex=20;
Parent=N;
})

local P=s:Create('Frame',{
BorderColor3=Color3.new(0,0,0);
Position=UDim2.new(0,208,0,25);
Size=UDim2.new(0,15,0,200);
ZIndex=17;
Parent=I;
});

local Q=s:Create('Frame',{
BackgroundColor3=Color3.new(1,1,1);
BorderSizePixel=0;
Size=UDim2.new(1,0,1,0);
ZIndex=18;
Parent=P;
});
local R=s:Create('Frame',{
BackgroundColor3=Color3.new(1,1,1);
AnchorPoint=Vector2.new(0,0.5);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,0,0,1);
ZIndex=18;
Parent=Q;
});

local S=s:Create('Frame',{
BorderColor3=Color3.new(0,0,0);
Position=UDim2.fromOffset(4,228),
Size=UDim2.new(0.5,-6,0,20),
ZIndex=18,
Parent=I;
});
local T=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=18,
Parent=S;
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=T;
});

local U=s:Create('TextBox',{
BackgroundTransparency=1;
Position=UDim2.new(0,5,0,0);
Size=UDim2.new(1,-5,1,0);
Font=s.Font;
PlaceholderColor3=Color3.fromRGB(190,190,190);
PlaceholderText='Hex color',
Text='#FFFFFF',
TextColor3=s.FontColor;
TextSize=s.FontSize;
TextStrokeTransparency=0;
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=20,
Parent=T;
});

s:ApplyTextStroke(U);

local V=s:Create(S:Clone(),{
Position=UDim2.new(0.5,2,0,228),
Size=UDim2.new(0.5,-6,0,20),
Parent=I
});
local W=s:Create(V.Frame:FindFirstChild('TextBox'),{
Text='255, 255, 255',
PlaceholderText='RGB color',
TextColor3=s.FontColor
});
local X,Y,Z;

if C.Transparency then
X=s:Create('Frame',{
BorderColor3=Color3.new(0,0,0);
Position=UDim2.fromOffset(4,251);
Size=UDim2.new(1,-8,0,15);
ZIndex=19;
Parent=I;
});
Y=s:Create('Frame',{
BackgroundColor3=E.Value;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=19;
Parent=X;
});
s:AddToRegistry(Y,{BorderColor3='OutlineColor'});

s:Create('ImageLabel',{
BackgroundTransparency=1;
Size=UDim2.new(1,0,1,0);
Image='http://www.roblox.com/asset/?id=12978095818';
ZIndex=20;
Parent=Y;
});
Z=s:Create('Frame',{
BackgroundColor3=Color3.new(1,1,1);
AnchorPoint=Vector2.new(0.5,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(0,1,1,0);
ZIndex=21;
Parent=Y;
});
end;

local _=s:CreateLabel({
Size=UDim2.new(1,0,0,14);
Position=UDim2.fromOffset(5,5);
TextXAlignment=Enum.TextXAlignment.Left;
TextSize=s.FontSize;
Text=E.Title,
TextWrapped=false;
ZIndex=16;
Parent=I;
});
local aa={}
do
aa.Options={}
aa.Container=s:Create('Frame',{
BorderColor3=Color3.new(),
ZIndex=14,
Visible=false,
Parent=p
})

aa.Inner=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.fromScale(1,1);
ZIndex=15;
Parent=aa.Container;
});
s:Create('UIListLayout',{
Name='Layout',
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=aa.Inner;
});
s:Create('UIPadding',{
Name='Padding',
PaddingLeft=UDim.new(0,4),
Parent=aa.Inner,
});
local function ab()
aa.Container.Position=UDim2.fromOffset(
(F.AbsolutePosition.X+F.AbsoluteSize.X)+4,
F.AbsolutePosition.Y+1
)
end

local function ac()
local ad=60
for ae,af in next,aa.Inner:GetChildren()do
if af:IsA('TextLabel')then
ad=math.max(ad,af.TextBounds.X)
end
end

aa.Container.Size=UDim2.fromOffset(
ad+8,
aa.Inner.Layout.AbsoluteContentSize.Y+4
)
end

F:GetPropertyChangedSignal('AbsolutePosition'):Connect(ab)
aa.Inner.Layout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(ac)

task.spawn(ab)
task.spawn(ac)

s:AddToRegistry(aa.Inner,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});

function aa:Show()
self.Container.Visible=true
end

function aa:Hide()
self.Container.Visible=false
end

function aa:AddOption(ad,ae)
if type(ae)~='function'then
ae=function()end
end

local af=s:CreateLabel({
Active=false;
Size=UDim2.new(1,0,0,15);
TextSize=s.FontSize-1;
Text=ad;
ZIndex=16;
Parent=self.Inner;
TextXAlignment=Enum.TextXAlignment.Left,
});
s:OnHighlight(af,af,
{TextColor3='AccentColor'},
{TextColor3='FontColor'}
);
af.InputBegan:Connect(function(ag)
if ag.UserInputType~=Enum.UserInputType.MouseButton1 and ag.UserInputType~=Enum.UserInputType.Touch then
return
end

ae()
end)
end

aa:AddOption('Copy color',function()
s.ColorClipboard=E.Value
s:Notify('Copied color!',2)
end)

aa:AddOption('Paste color',function()
if not s.ColorClipboard then
return s:Notify('You have not copied a color!',2)
end
E:SetValueRGB(s.ColorClipboard)
end)


aa:AddOption('Copy HEX',function()
pcall(setclipboard,E.Value:ToHex())
s:Notify('Copied hex code to clipboard!',2)
end)

aa:AddOption('Copy RGB',function()
pcall(setclipboard,table.concat({math.floor(E.Value.R*255),math.floor(E.Value.G*255),math.floor(E.Value.B*255)},', '))
s:Notify('Copied RGB values to clipboard!',2)
end)

end

s:AddToRegistry(I,{BackgroundColor3='BackgroundColor';BorderColor3='OutlineColor';});
s:AddToRegistry(J,{BackgroundColor3='AccentColor';});
s:AddToRegistry(L,{BackgroundColor3='BackgroundColor';BorderColor3='OutlineColor';});
s:AddToRegistry(T,{BackgroundColor3='MainColor';BorderColor3='OutlineColor';});
s:AddToRegistry(V.Frame,{BackgroundColor3='MainColor';BorderColor3='OutlineColor';});
s:AddToRegistry(W,{TextColor3='FontColor',});
s:AddToRegistry(U,{TextColor3='FontColor',});

local ab={};
for ac=0,1,0.1 do
table.insert(ab,ColorSequenceKeypoint.new(ac,Color3.fromHSV(ac,1,1)));
end;

local ac=s:Create('UIGradient',{
Color=ColorSequence.new(ab);
Rotation=90;
Parent=Q;
});
U.FocusLost:Connect(function(ad)
if ad then
local ae,af=pcall(Color3.fromHex,U.Text)
if ae and typeof(af)=='Color3'then
E.Hue,E.Sat,E.Vib=Color3.toHSV(af)
end
end

E:Display()
end)

W.FocusLost:Connect(function(ad)
if ad then
local ae,af,ag=W.Text:match('(%d+),%s*(%d+),%s*(%d+)')
if ae and af and ag then
E.Hue,E.Sat,E.Vib=Color3.toHSV(Color3.fromRGB(ae,af,ag))
end
end

E:Display()
end)

function E:Display()
E.Value=Color3.fromHSV(E.Hue,E.Sat,E.Vib);
M.BackgroundColor3=Color3.fromHSV(E.Hue,1,1);

s:Create(F,{
BackgroundColor3=E.Value;
BackgroundTransparency=E.Transparency;
BorderColor3=s:GetDarkerColor(E.Value);
});
if Y then
Y.BackgroundColor3=E.Value;
Z.Position=UDim2.new(1-E.Transparency,0,0,0);
end;

N.Position=UDim2.new(E.Sat,0,1-E.Vib,0);
R.Position=UDim2.new(0,0,E.Hue,0);

U.Text='#'..E.Value:ToHex()
W.Text=table.concat({math.floor(E.Value.R*255),math.floor(E.Value.G*255),math.floor(E.Value.B*255)},', ')

s:SafeCallback(E.Callback,E.Value);
s:SafeCallback(E.Changed,E.Value);
end;

function E:OnChanged(ad)
E.Changed=ad;
ad(E.Value)
end;

function E:Show()
for ad,ae in next,s.OpenedFrames do
if ad.Name=='Color'then
ad.Visible=false;
s.OpenedFrames[ad]=nil;
end;
end;

H.Visible=true;
s.OpenedFrames[H]=true;
end;
function E:Hide()
H.Visible=false;
s.OpenedFrames[H]=nil;
end;
function E:SetValue(ad,ae)
local af=Color3.fromHSV(ad[1],ad[2],ad[3]);
E.Transparency=ae or 0;
E:SetHSVFromRGB(af);
E:Display();
end;

function E:SetValueRGB(ad,ae)
E.Transparency=ae or 0;
E:SetHSVFromRGB(ad);
E:Display();
end;

M.InputBegan:Connect(function(ad)
if ad.UserInputType==Enum.UserInputType.MouseButton1 or ad.UserInputType==Enum.UserInputType.Touch then
local function ae(af,ag)
local ah=M.AbsolutePosition.X;
local ai=ah+M.AbsoluteSize.X;
local aj=math.clamp(af,ah,ai);

local ak=M.AbsolutePosition.Y;
local al=ak+M.AbsoluteSize.Y;
local am=math.clamp(ag,ak,al);

E.Sat=(aj-ah)/(ai-ah);
E.Vib=1-((am-ak)/(al-ak));
E:Display();
end

ae(ad.Position.X,ad.Position.Y)

local af=c.InputChanged:Connect(function(af)
if af.UserInputType==Enum.UserInputType.MouseMovement or af==ad then
ae(af.Position.X,af.Position.Y)
end
end)

local ag
ag=c.InputEnded:Connect(function(ah)
if ah==ad or ah.UserInputType==Enum.UserInputType.Touch then
af:Disconnect()
ag:Disconnect()
s:AttemptSave()
end
end)
end
end);
Q.InputBegan:Connect(function(ad)
if ad.UserInputType==Enum.UserInputType.MouseButton1 or ad.UserInputType==Enum.UserInputType.Touch then
local function ae(af)
local ag=Q.AbsolutePosition.Y;
local ah=ag+Q.AbsoluteSize.Y;
local ai=math.clamp(af,ag,ah);

E.Hue=((ai-ag)/(ah-ag));
E:Display();
end

ae(ad.Position.Y)

local af=c.InputChanged:Connect(function(af)
if af.UserInputType==Enum.UserInputType.MouseMovement or af==ad then
ae(af.Position.Y)
end
end)

local ag
ag=c.InputEnded:Connect(function(ah)
if ah==ad or ah.UserInputType==Enum.UserInputType.Touch then
af:Disconnect()
ag:Disconnect()
s:AttemptSave()
end
end)
end
end);
F.InputBegan:Connect(function(ad)
if(ad.UserInputType==Enum.UserInputType.MouseButton1 or ad.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then
if H.Visible then
E:Hide()
else
aa:Hide()
E:Show()
end;
elseif ad.UserInputType==Enum.UserInputType.MouseButton2 and not s:MouseIsOverOpenedFrame()then
aa:Show()
E:Hide()
end
end);

if Y then
Y.InputBegan:Connect(function(ad)
if ad.UserInputType==Enum.UserInputType.MouseButton1 or ad.UserInputType==Enum.UserInputType.Touch then
local function ae(af)
local ag=Y.AbsolutePosition.X;
local ah=ag+Y.AbsoluteSize.X;
local ai=math.clamp(af,ag,ah);

E.Transparency=1-((ai-ag)/(ah-ag));
E:Display();
end

ae(ad.Position.X)

local af=c.InputChanged:Connect(function(af)
if af.UserInputType==Enum.UserInputType.MouseMovement or af==ad then
ae(af.Position.X)
end
end)

local ag
ag=c.InputEnded:Connect(function(ah)
if ah==ad or ah.UserInputType==Enum.UserInputType.Touch then
af:Disconnect()
ag:Disconnect()
s:AttemptSave()
end
end)
end
end);
end;

s:GiveSignal(c.InputBegan:Connect(function(ad)
if(ad.UserInputType==Enum.UserInputType.MouseButton1 or ad.UserInputType==Enum.UserInputType.Touch)then
local ae,af=H.AbsolutePosition,H.AbsoluteSize;
local ag=F.AbsolutePosition;
local ah=F.AbsoluteSize;

if m.X<ae.X or m.X>ae.X+af.X
or m.Y<ag.Y or m.Y>ae.Y+af.Y then

if not(m.X>=ag.X and m.X<=ag.X+ah.X
and m.Y>=ag.Y and m.Y<=ag.Y+ah.Y)then
E:Hide();
end
end;

if not s:IsMouseOverFrame(aa.Container)then
aa:Hide()
end
end;

if ad.UserInputType==Enum.UserInputType.MouseButton2 and aa.Container.Visible then
if not s:IsMouseOverFrame(aa.Container)and not s:IsMouseOverFrame(F)then
aa:Hide()
end
end
end))

function E:GetTransparency()
return E.Transparency
end;

function E:OnTransparencyChanged(ad)
E.TransparencyChanged=ad;
ad(E.Transparency);
end;

local ad=E.Display;
E.Display=function(ae)
ad(ae);
s:SafeCallback(E.TransparencyChanged,E.Transparency);
end;

E:Display();
E.DisplayFrame=F

r[B]=E;

return self
end;

function A:AddColorPickerAlpha(aa,ab)
ab=ab or{};
if ab.Transparency==nil then
ab.Transparency=0;
end;
return A.AddColorPicker(self,aa,ab)
end;

function A:AddKeyPicker(aa,ab)
local ac=self;
local ad=self.TextLabel;
local ae=self.Container;

assert(ab.Default,'AddKeyPicker: Missing default value.');

local af={
Value=ab.Default;
Toggled=false;
Mode=ab.Mode or'Toggle';
Type='KeyPicker';
Callback=ab.Callback or function(af)end;
ChangedCallback=ab.ChangedCallback or function(af)end;

SyncToggleState=ab.SyncToggleState or false;
};
if af.SyncToggleState then
ab.Modes={'Toggle'}
ab.Mode='Toggle'
end

local ag=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(0,28,0,15);
ZIndex=6;
Parent=ad;
});
local ah=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=7;
Parent=ag;
});
s:AddToRegistry(ah,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});
local ai=s:CreateLabel({
Size=UDim2.new(1,0,1,0);
TextSize=s.FontSize-1;
Text=ab.Default;
TextWrapped=true;
ZIndex=8;
Parent=ah;
});
local aj=s:Create('Frame',{
BorderColor3=Color3.new(0,0,0);
Position=UDim2.fromOffset(ad.AbsolutePosition.X+ad.AbsoluteSize.X+4,ad.AbsolutePosition.Y+1);
Size=UDim2.new(0,60,0,45+2);
Visible=false;
ZIndex=14;
Parent=p;
});
ad:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
aj.Position=UDim2.fromOffset(ad.AbsolutePosition.X+ad.AbsoluteSize.X+4,ad.AbsolutePosition.Y+1);
end);
local ak=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=15;
Parent=aj;
});
s:AddToRegistry(ak,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});
s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=ak;
});
local al=s:Create('Frame',{
BackgroundTransparency=1,
Size=UDim2.new(1,0,0,18),
Visible=false,
ZIndex=110,
Parent=s.KeybindContainer,
})

local am=s:CreateLabel({
Position=UDim2.new(0,2,0,0),
Size=UDim2.new(1,-4,1,0),
TextSize=s.FontSize-1,
TextXAlignment=Enum.TextXAlignment.Left,
ZIndex=111,
Parent=al,
},true)

local B=ab.Modes or{'Always','Toggle','Hold'};
local C={};

for D,E in next,B do
local F={};
local G=s:CreateLabel({
Active=false;
Size=UDim2.new(1,0,0,15);
TextSize=s.FontSize-1;
Text=E;
ZIndex=16;
Parent=ak;
});
function F:Select()
for H,I in next,C do
I:Deselect();
end;

af.Mode=E;

G.TextColor3=s.AccentColor;
s.RegistryMap[G].Properties.TextColor3='AccentColor';

aj.Visible=false;
end;
function F:Deselect()
af.Mode=nil;
G.TextColor3=s.FontColor;
s.RegistryMap[G].Properties.TextColor3='FontColor';
end;

G.InputBegan:Connect(function(H)
if(H.UserInputType==Enum.UserInputType.MouseButton1 or H.UserInputType==Enum.UserInputType.Touch)then
F:Select();
s:AttemptSave();
end;
end);
if E==af.Mode then
F:Select();
end;

C[E]=F;
end;

function af:Update()
if ab.NoUI then
return
end;

local D=af:GetState();

local E=(af.Value=='None')and'...'or af.Value
am.Text=string.format('[%s] %s (%s)',E,ab.Text,af.Mode);
local F=s.KeybindMode or'All'
if F=='Active'then
al.Visible=D==true
elseif F=='Toggled'then
local G=false
if ac and ac.Type=='Toggle'then
G=ac.Value==true
elseif af.SyncToggleState and ac then
G=ac.Value==true
else
G=true
end
al.Visible=G
else
al.Visible=true
end

am.TextColor3=D and s.AccentColor or s.FontColor;
s.RegistryMap[am].Properties.TextColor3=D and'AccentColor'or'FontColor';

local G=0
local H=0

for I,J in next,s.KeybindContainer:GetChildren()do
if J:IsA('Frame')and J.Visible then
G=G+18;
local K=J:FindFirstChildOfClass('TextLabel')
if K and(K.TextBounds.X+20>H)then
H=K.TextBounds.X+20
end
end;
end;

s.KeybindFrame.Size=UDim2.new(0,math.max(H+10+15,210),0,G+23)
end;
function af:GetState()
if af.Mode=='Always'then
return true
elseif af.Mode=='Hold'then
if af.Value=='None'then
return false
end

local D=af.Value;
if D=='MB1'or D=='MB2'or D=='Touch'then
return D=='MB1'and c:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
or D=='MB2'and c:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)
or D=='Touch'and true
else
return c:IsKeyDown(Enum.KeyCode[af.Value])
end;
else
return af.Toggled
end;
end;

function af:SetValue(D)
local E,F=D[1],D[2];
ai.Text=E;
af.Value=E;
C[F]:Select();
af:Update();
end;

function af:OnClick(D)
af.Clicked=D
end

function af:OnChanged(D)
af.Changed=D
D(af.Value)
end

if ac.Addons then
table.insert(ac.Addons,af)
table.insert(s.KeyPickerList,af)
end

function af:DoClick()
if ac.Type=='Toggle'and af.SyncToggleState then
ac:SetValue(not ac.Value)
end

s:SafeCallback(af.Callback,af.Toggled)
s:SafeCallback(af.Clicked,af.Toggled)
end

local D=false;
ag.InputBegan:Connect(function(E)
if(E.UserInputType==Enum.UserInputType.MouseButton1 or E.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then
D=true;

ai.Text='';

local F;
local G='';

task.spawn(function()
while(not F)do
if G=='...'then
G='';
end;

G=G..'.';
ai.Text=G;

wait(0.4);
end;
end);

wait(0.2);

local H;
H=c.InputBegan:Connect(function(I)
local J;

if I.UserInputType==Enum.UserInputType.Keyboard then
J=I.KeyCode.Name;
elseif I.UserInputType==Enum.UserInputType.MouseButton1 then
J='MB1';
elseif I.UserInputType==Enum.UserInputType.MouseButton2 then
J='MB2';
elseif I.UserInputType==Enum.UserInputType.Touch then
J='Touch';
end;

F=true;
D=false;

ai.Text=J;
af.Value=J;
s:SafeCallback(af.ChangedCallback,I.KeyCode or I.UserInputType)
s:SafeCallback(af.Changed,I.KeyCode or I.UserInputType)

s:AttemptSave();
H:Disconnect();
end);
elseif E.UserInputType==Enum.UserInputType.MouseButton2 and not s:MouseIsOverOpenedFrame()then
aj.Visible=true;
end;
end);

s:GiveSignal(c.InputBegan:Connect(function(E)
if(not D)then
if af.Mode=='Toggle'then
local F=af.Value;

if F=='MB1'or F=='MB2'or F=='Touch'then
if F=='MB1'and E.UserInputType==Enum.UserInputType.MouseButton1
or F=='MB2'and E.UserInputType==Enum.UserInputType.MouseButton2
or F=='Touch'and E.UserInputType==Enum.UserInputType.Touch then
af.Toggled=not af.Toggled
af:DoClick()
end;
elseif E.UserInputType==Enum.UserInputType.Keyboard then
if E.KeyCode.Name==F then
af.Toggled=not af.Toggled;
af:DoClick()
end;
end;
end;

af:Update();
end;
if(E.UserInputType==Enum.UserInputType.MouseButton1 or E.UserInputType==Enum.UserInputType.Touch)then
local F,G=aj.AbsolutePosition,aj.AbsoluteSize;
if m.X<F.X or m.X>F.X+G.X
or m.Y<(F.Y-20-1)or m.Y>F.Y+G.Y then

aj.Visible=false;
end;
end;
end))

s:GiveSignal(c.InputEnded:Connect(function(E)
if(not D)then
af:Update();
end;
end))

af:Update();
r[aa]=af;

return self
end;

z.__index=A;
z.__namecall=function(aa,ab,...)
return A[ab](...)
end;
end;

local aa={};

do
local ab={};
function ab:AddBlank(ac)
local ad=self;
local ae=ad.Container;
s:Create('Frame',{
BackgroundTransparency=1;
Size=UDim2.new(1,0,0,ac);
ZIndex=1;
Parent=ae;
});
end;

function ab:AddRow(ac)
local ad=self
local ae=ad.Container

local af=type(ac)=='number'and math.max(1,ac)or 2

local ag=s:Create('Frame',{
BackgroundTransparency=1,
Size=UDim2.new(1,0,0,0),
ZIndex=1,
Parent=ae
})

s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Horizontal,
SortOrder=Enum.SortOrder.LayoutOrder,
Padding=UDim.new(0,8),
Parent=ag
})

local ah={}

for ai=1,af do
local aj={Type='Groupbox'}

local ak=s:Create('Frame',{
BackgroundTransparency=1,
Size=UDim2.new(1/af,-((af-1)*8)/af,1,0),
ZIndex=1,
Parent=ag
})

local al=s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical,
SortOrder=Enum.SortOrder.LayoutOrder,
Padding=UDim.new(0,4),
Parent=ak
})

aj.Container=ak
setmetatable(aj,aa)

function aj:Resize()
local am=0
for A,B in next,ag:GetChildren()do
if B:IsA('Frame')then
local C=B:FindFirstChildOfClass('UIListLayout')
if C and C.AbsoluteContentSize.Y>am then
am=C.AbsoluteContentSize.Y
end
end
end
ag.Size=UDim2.new(1,0,0,am)
if ad.Resize then
ad:Resize()
end
end

al:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
aj:Resize()
end)

table.insert(ah,aj)
end

ad:AddBlank(1)
if ad.Resize then ad:Resize()end

return unpack(ah)
end;
function ab:AddLabel(ac,ad)
local ae={};

local af=self;
local ag=af.Container;

local ah=s:CreateLabel({
Size=UDim2.new(1,-4,0,15);
TextSize=s.FontSize;
Text=ac;
TextWrapped=ad or false,
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=5;
Parent=ag;
});
if ad then
local ai=select(2,s:GetTextBounds(ac,s.Font,s.FontSize,Vector2.new(ah.AbsoluteSize.X,math.huge)))
ah.Size=UDim2.new(1,-4,0,ai)
else
s:Create('UIListLayout',{
Padding=UDim.new(0,4);
FillDirection=Enum.FillDirection.Horizontal;
HorizontalAlignment=Enum.HorizontalAlignment.Right;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=ah;
});
end

ae.TextLabel=ah;
ae.Container=ag;
function ae:SetText(ai)
ah.Text=ai

if ad then
local aj=select(2,s:GetTextBounds(ai,s.Font,s.FontSize,Vector2.new(ah.AbsoluteSize.X,math.huge)))
ah.Size=UDim2.new(1,-4,0,aj)
end

af:Resize();
end

if(not ad)then
setmetatable(ae,z);
end

af:AddBlank(5);
af:Resize();

return ae
end;
function ab:AddButton(...)
local ac={};
local function ad(ae,af,...)
local ag=select(1,...)
if type(ag)=='table'then
af.Text=ag.Text
af.Func=ag.Func
af.DoubleClick=ag.DoubleClick
af.Tooltip=ag.Tooltip
else
af.Text=select(1,...)
af.Func=select(2,...)
end

assert(type(af.Func)=='function','AddButton: `Func` callback is missing.');
end

ad('Button',ac,...)

local ae=self;
local af=ae.Container;

local function ag(ah)
local ai=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-4,0,20);
ZIndex=5;
});
local aj=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=ai;
});
local ak=s:CreateLabel({
Size=UDim2.new(1,0,1,0);
TextSize=s.FontSize;
Text=ah.Text;
ZIndex=6;
Parent=aj;
});

s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=aj;
});
s:AddToRegistry(ai,{
BorderColor3='Black';
});
s:AddToRegistry(aj,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
local al=TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)
ai.MouseEnter:Connect(function()
i:Create(ai,al,{BorderColor3=s.AccentColor}):Play()
i:Create(aj,al,{BackgroundColor3=s.AccentColor}):Play()
end)

ai.MouseLeave:Connect(function()
i:Create(ai,al,{BorderColor3=Color3.new(0,0,0)}):Play()
i:Create(aj,al,{BackgroundColor3=s.MainColor}):Play()
end)
return ai,aj,ak
end

local function ah(ai)
local function aj(ak,al,am)
local A=Instance.new('BindableEvent')
local B=ak:Once(function(...)

if type(am)=='function'and am(...)then
A:Fire(true)
else
A:Fire(false)
end
end)
task.delay(al,function()
B:disconnect()
A:Fire(false)
end)
return A.Event:Wait()
end

local function ak(al)
if s:MouseIsOverOpenedFrame()then
return false
end

if al.UserInputType~=Enum.UserInputType.MouseButton1 and al.UserInputType~=Enum.UserInputType.Touch then
return false
end

return true
end

ai.Outer.InputBegan:Connect(function(al)
if not ak(al)then return end

if ai.Locked then return end

if ai.DoubleClick then
s:RemoveFromRegistry(ai.Label)
s:AddToRegistry(ai.Label,{TextColor3='AccentColor'})

ai.Label.TextColor3=s.AccentColor
ai.Label.Text='Are you sure?'
ai.Locked=true

local am=aj(ai.Outer.InputBegan,0.5,ak)

s:RemoveFromRegistry(ai.Label)
s:AddToRegistry(ai.Label,{TextColor3='FontColor'})

ai.Label.TextColor3=s.FontColor
ai.Label.Text=ai.Text
task.defer(rawset,ai,'Locked',false)

if am then
s:SafeCallback(ai.Func)
end

return
end

s:SafeCallback(ai.Func);
end)
end

ac.Outer,ac.Inner,ac.Label=ag(ac)
ac.Outer.Parent=af

ah(ac)

function ac:AddTooltip(ai)
if type(ai)=='string'then
s:AddToolTip(ai,self.Outer)
end
return self
end

function ac:AddButton(...)
local ai={}

ad('SubButton',ai,...)

self.Outer.Size=UDim2.new(0.5,-2,0,20)

ai.Outer,ai.Inner,ai.Label=ag(ai)

ai.Outer.Position=UDim2.new(1,3,0,0)
ai.Outer.Size=UDim2.fromOffset(self.Outer.AbsoluteSize.X-2,self.Outer.AbsoluteSize.Y)
ai.Outer.Parent=self.Outer

function ai:AddTooltip(aj)
if type(aj)=='string'then
s:AddToolTip(aj,self.Outer)
end
return ai
end

if type(ai.Tooltip)=='string'then
ai:AddTooltip(ai.Tooltip)
end

ah(ai)
return ai
end

if type(ac.Tooltip)=='string'then
ac:AddTooltip(ac.Tooltip)
end

ae:AddBlank(5);
ae:Resize();

return ac
end;

function ab:AddDivider()
local ac=self;
local ad=self.Container

local ae={
Type='Divider',
}

ac:AddBlank(2);
local af=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-4,0,5);
ZIndex=5;
Parent=ad;
});
local ag=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=af;
});
s:AddToRegistry(af,{
BorderColor3='Black';
});
s:AddToRegistry(ag,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
ac:AddBlank(9);
ac:Resize();
end

function ab:AddInput(ac,ad)
assert(ad.Text,'AddInput: Missing `Text` string.')

local ae={
Value=ad.Default or'';
Numeric=ad.Numeric or false;
Finished=ad.Finished or false;
Type='Input';
Callback=ad.Callback or function(ae)end;
};
local af=self;
local ag=af.Container;

local ah=s:CreateLabel({
Size=UDim2.new(1,0,0,15);
TextSize=s.FontSize;
Text=ad.Text;
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=5;
Parent=ag;
});

af:AddBlank(1);

local ai=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-4,0,20);
ZIndex=5;
Parent=ag;
});
local aj=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=ai;
});
s:AddToRegistry(aj,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
s:OnHighlight(ai,ai,
{BorderColor3='AccentColor'},
{BorderColor3='Black'}
);
if type(ad.Tooltip)=='string'then
s:AddToolTip(ad.Tooltip,ai)
end

s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=aj;
});
local ak=s:Create('Frame',{
BackgroundTransparency=1;
ClipsDescendants=true;

Position=UDim2.new(0,5,0,0);
Size=UDim2.new(1,-5,1,0);

ZIndex=7;
Parent=aj;
})

local al=s:Create('TextBox',{
BackgroundTransparency=1;

Position=UDim2.fromOffset(0,0),
Size=UDim2.fromScale(5,1),

Font=s.Font;
PlaceholderColor3=Color3.fromRGB(190,190,190);
PlaceholderText=ad.Placeholder or'';

Text=ad.Default or'';
TextColor3=s.FontColor;
TextSize=s.FontSize;
TextStrokeTransparency=0;
TextXAlignment=Enum.TextXAlignment.Left;

ZIndex=7;
Parent=ak;
});

s:ApplyTextStroke(al);
function ae:SetValue(am)
if ad.MaxLength and#am>ad.MaxLength then
am=am:sub(1,ad.MaxLength);
end;

if ae.Numeric then
if(not tonumber(am))and am:len()>0 then
am=ae.Value
end
end

ae.Value=am;
al.Text=am;

s:SafeCallback(ae.Callback,ae.Value);
s:SafeCallback(ae.Changed,ae.Value);
end;

if ae.Finished then
al.FocusLost:Connect(function(am)
if not am then return end

ae:SetValue(al.Text);
s:AttemptSave();
end)
else
al:GetPropertyChangedSignal('Text'):Connect(function()
ae:SetValue(al.Text);
s:AttemptSave();
end);
end

local function am()
local A=2
local B=ak.AbsoluteSize.X

if not al:IsFocused()or al.TextBounds.X<=B-2*A then
al.Position=UDim2.new(0,A,0,0)
else
local C=al.CursorPosition
if C~=-1 then
local D=string.sub(al.Text,1,C-1)
local E=d:GetTextSize(D,al.TextSize,al.Font,Vector2.new(math.huge,math.huge)).X

local F=al.Position.X.Offset+E

if F<A then
al.Position=UDim2.fromOffset(A-E,0)
elseif F>B-A-1 then
al.Position=UDim2.fromOffset(B-E-A-1,0)
end
end
end
end

task.spawn(am)

al:GetPropertyChangedSignal('Text'):Connect(am)
al:GetPropertyChangedSignal('CursorPosition'):Connect(am)
al.FocusLost:Connect(am)
al.Focused:Connect(am)

s:AddToRegistry(al,{
TextColor3='FontColor';
});

function ae:OnChanged(A)
ae.Changed=A;
A(ae.Value);
end;

af:AddBlank(5);
af:Resize();

r[ac]=ae;

return ae
end;

function ab:AddToggle(ac,ad)
assert(ad.Text,'AddInput: Missing `Text` string.')

local ae={
Value=ad.Default or false;
Type='Toggle';

Callback=ad.Callback or function(ae)end;
Addons={},
Risky=ad.Risky,
};
local af=self;
local ah=af.Container;

local ai=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(0,13,0,13);
ZIndex=5;
Parent=ah;
});
s:AddToRegistry(ai,{
BorderColor3='Black';
});
local aj=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=ai;
});
s:AddToRegistry(aj,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
local ak=s:Create("UIGradient",{
Rotation=90,
Parent=aj,
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.fromRGB(255,255,255)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100)),
}),
});
local al=s:CreateLabel({
Size=UDim2.new(0,216,1,0);
Position=UDim2.new(1,6,0,0);
TextSize=16;
Text=ad.Text;
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=6;
Parent=aj;
});
s:Create('UIListLayout',{
Padding=UDim.new(0,4);
FillDirection=Enum.FillDirection.Horizontal;
HorizontalAlignment=Enum.HorizontalAlignment.Right;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=al;
});
local am=s:Create('Frame',{
BackgroundTransparency=1;
Size=UDim2.new(0,170,1,0);
ZIndex=8;
Parent=ai;
});
s:OnHighlight(am,ai,
{BorderColor3='AccentColor'},
{BorderColor3='Black'}
);
function ae:UpdateColors()
ae:Display();
end;
if type(ad.Tooltip)=='string'then
s:AddToolTip(ad.Tooltip,am)
end

function ae:Display()
aj.BackgroundColor3=ae.Value and s.AccentColor or s.MainColor;
aj.BorderColor3=ae.Value and s.AccentColorDark or s.OutlineColor;

s.RegistryMap[aj].Properties.BackgroundColor3=ae.Value and'AccentColor'or'MainColor';
s.RegistryMap[aj].Properties.BorderColor3=ae.Value and'AccentColorDark'or'OutlineColor';
end;

function ae:OnChanged(A)
ae.Changed=A;
A(ae.Value);
end;

function ae:SetValue(A)
A=(not not A);
ae.Value=A;
ae:Display();

for B,C in next,ae.Addons do
if C.Type=='KeyPicker'and C.SyncToggleState then
C.Toggled=A
C:Update()
end
end

s:SafeCallback(ae.Callback,ae.Value);
s:SafeCallback(ae.Changed,ae.Value);
s:UpdateDependencyBoxes();
end;
am.InputBegan:Connect(function(A)
if(A.UserInputType==Enum.UserInputType.MouseButton1 or A.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then
ae:SetValue(not ae.Value)
s:AttemptSave();
end;
end);
if ae.Risky then
s:RemoveFromRegistry(al)
al.TextColor3=s.RiskColor
s:AddToRegistry(al,{TextColor3='RiskColor'})
end

ae:Display();
af:AddBlank(ad.BlankSize or 5+2);
af:Resize();

ae.TextLabel=al;
ae.Container=ah;
setmetatable(ae,z);

q[ac]=ae;

s:UpdateDependencyBoxes();

return ae
end;

function ab:AddSlider(ac,ad)
assert(ad.Default,'AddSlider: Missing default value.');
assert(ad.Text,'AddSlider: Missing slider text.');
assert(ad.Min,'AddSlider: Missing minimum value.');
assert(ad.Max,'AddSlider: Missing maximum value.');
assert(ad.Rounding,'AddSlider: Missing rounding value.');
local ae={
Value=ad.Default;
Min=ad.Min;
Max=ad.Max;
Rounding=ad.Rounding;
MaxSize=232;
Type='Slider';
Callback=ad.Callback or function(ae)end;
};

local af=self;
local ah=af.Container;
if not ad.Compact then
s:CreateLabel({
Size=UDim2.new(1,0,0,10);
TextSize=16;
Text=ad.Text;
TextXAlignment=Enum.TextXAlignment.Left;
TextYAlignment=Enum.TextYAlignment.Bottom;
ZIndex=5;
Parent=ah;
});
af:AddBlank(3);
end

local ai=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-4,0,13);
ZIndex=5;
Parent=ah;
});
s:AddToRegistry(ai,{
BorderColor3='Black';
});
local aj=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=ai;
});
s:AddToRegistry(aj,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
local ak=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderColor3=s.AccentColorDark;
Size=UDim2.new(0,0,1,0);
ZIndex=7;
Parent=aj;
});
s:AddToRegistry(ak,{
BackgroundColor3='AccentColor';
BorderColor3='AccentColorDark';
});
local al=s:Create("UIGradient",{
Rotation=90,
Parent=ak,
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.fromRGB(255,255,255)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100)),
}),
});








local am=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
BackgroundTransparency=1;
Position=UDim2.new(1,0,0,0);
Size=UDim2.new(0,1,1,0);
ZIndex=8;
Parent=ak;
});

s:AddToRegistry(am,{
BackgroundColor3='AccentColor';
});
local A=s:CreateLabel({
Size=UDim2.new(1,0,1,0);
TextSize=s.FontSize;
Text='Infinite';
ZIndex=9;
Parent=aj;
});
s:OnHighlight(ai,ai,
{BorderColor3='AccentColor'},
{BorderColor3='Black'}
);
if type(ad.Tooltip)=='string'then
s:AddToolTip(ad.Tooltip,ai)
end

function ae:UpdateColors()
ak.BackgroundColor3=s.AccentColor;
ak.BorderColor3=s.AccentColorDark;
end;

local B=TweenInfo.new(0.1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out);

function ae:Display()
local C=ad.Suffix or'';
if ad.Compact then
A.Text=ad.Text..': '..ae.Value..C
elseif ad.HideMax then
A.Text=string.format('%s',ae.Value..C)
else
A.Text=string.format('%s/%s',ae.Value..C,ae.Max..C);
end

ae.MaxSize=aj.AbsoluteSize.X;
local D=math.ceil(s:MapValue(ae.Value,ae.Min,ae.Max,0,ae.MaxSize));


i:Create(ak,B,{
Size=UDim2.new(0,D,1,0);
}):Play();

am.Visible=not(D==ae.MaxSize or D==0);
end;


function ae:OnChanged(C)
ae.Changed=C;
C(ae.Value);
end;
local function C(D)
if ae.Rounding==0 then
return math.floor(D)
end;


return tonumber(string.format('%.'..ae.Rounding..'f',D))
end;
function ae:GetMaxSize()
return aj.AbsoluteSize.X
end
function ae:GetValueFromXOffset(D)
return C(s:MapValue(D,0,ae:GetMaxSize(),ae.Min,ae.Max))
end
function ae:SetValue(D)
local E=tonumber(D);
if(not E)then
return
end;

E=math.clamp(E,ae.Min,ae.Max);

ae.Value=E;
ae:Display();

s:SafeCallback(ae.Callback,ae.Value);
s:SafeCallback(ae.Changed,ae.Value);
end;
aj.InputBegan:Connect(function(D)
if(D.UserInputType==Enum.UserInputType.MouseButton1 or D.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then

local function E(F)
local G=aj.AbsolutePosition.X
local H=ae:GetMaxSize()

local I=F-G
local J=math.clamp(I,0,H)

local K=ae:GetValueFromXOffset(J)
local L=ae.Value

ae.Value=K
ae:Display()

if K~=L then
s:SafeCallback(ae.Callback,ae.Value)
s:SafeCallback(ae.Changed,ae.Value)
end
end

E(D.Position.X)

local F=c.InputChanged:Connect(function(F)
if F.UserInputType==Enum.UserInputType.MouseMovement or F==D then
E(F.Position.X)
end
end)

local G
G=c.InputEnded:Connect(function(H)
if H==D or H.UserInputType==Enum.UserInputType.Touch then
F:Disconnect()
G:Disconnect()
s:AttemptSave()
end
end)
end;
end);

ae:Display();
af:AddBlank(ad.BlankSize or 6);
af:Resize();

r[ac]=ae;

return ae
end;
function ab:AddDropdown(ac,ad)
if ad.SpecialType=='Player'then
ad.Values=v();
ad.AllowNull=true;
elseif ad.SpecialType=='Team'then
ad.Values=w();
ad.AllowNull=true;
end;

assert(ad.Values,'AddDropdown: Missing dropdown value list.');
assert(ad.AllowNull or ad.Default,'AddDropdown: Missing default value. Pass `AllowNull` as true if this was intentional.')

if(not ad.Text)then
ad.Compact=true;
end;

local ae={
Values=ad.Values;
Value=ad.Multi and{};
Multi=ad.Multi;
Type='Dropdown';
SpecialType=ad.SpecialType;
Callback=ad.Callback or function(ae)end;
};

local af=self;
local ah=af.Container;

local ai=0;
if not ad.Compact then
local aj=s:CreateLabel({
Size=UDim2.new(1,0,0,10);
TextSize=16;
Text=ad.Text;
TextXAlignment=Enum.TextXAlignment.Left;
TextYAlignment=Enum.TextYAlignment.Bottom;
ZIndex=5;
Parent=ah;
});
af:AddBlank(3);
end

for aj,ak in next,ah:GetChildren()do
if not ak:IsA('UIListLayout')then
ai=ai+ak.Size.Y.Offset;
end;
end;

local aj=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-4,0,20);
ZIndex=5;
Parent=ah;
});
s:AddToRegistry(aj,{
BorderColor3='Black';
});
local ak=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=6;
Parent=aj;
});
s:AddToRegistry(ak,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=ak;
});
local al=s:Create('ImageLabel',{
AnchorPoint=Vector2.new(0,0.5);
BackgroundTransparency=1;
ImageColor3=s.AccentColor;
Position=UDim2.new(1,-16,0.5,0);
Size=UDim2.new(0,12,0,12);
Image='http://www.roblox.com/asset/?id=6282522798';
ZIndex=8;
Parent=ak;
});
s:AddToRegistry(al,{
ImageColor3='AccentColor';
});
local am=s:CreateLabel({
Position=UDim2.new(0,5,0,0);
Size=UDim2.new(1,-5,1,0);
TextSize=16;
Text='--';
TextXAlignment=Enum.TextXAlignment.Left;
TextWrapped=true;
ZIndex=7;
Parent=ak;
});
s:OnHighlight(aj,aj,
{BorderColor3='AccentColor'},
{BorderColor3='Black'}
);
if type(ad.Tooltip)=='string'then
s:AddToolTip(ad.Tooltip,aj)
end

local A=8;
local B=s:Create('Frame',{
BackgroundColor3=Color3.new(0,0,0);
BorderColor3=Color3.new(0,0,0);
ZIndex=20;
Visible=false;
Parent=p;
});
local function C()
B.Position=UDim2.fromOffset(aj.AbsolutePosition.X,aj.AbsolutePosition.Y+aj.Size.Y.Offset+1);
end;

local function D(E)
B.Size=UDim2.fromOffset(aj.AbsoluteSize.X,E or(A*20+2))
end;
C();
D();

aj:GetPropertyChangedSignal('AbsolutePosition'):Connect(C);

local E=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
BorderSizePixel=0;
Size=UDim2.new(1,0,1,0);
ZIndex=21;
Parent=B;
});
s:AddToRegistry(E,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
local F=s:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
CanvasSize=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,1,0);
ZIndex=21;
Parent=E;

TopImage='rbxasset://textures/ui/Scroll/scroll-middle.png',
BottomImage='rbxasset://textures/ui/Scroll/scroll-middle.png',

ScrollBarThickness=3,
ScrollBarImageColor3=s.AccentColor,
});
s:AddToRegistry(F,{
ScrollBarImageColor3='AccentColor'
})

s:Create('UIListLayout',{
Padding=UDim.new(0,0);
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=F;
});
function ae:Display()
local G=ae.Values;
local H='';

if ad.Multi then
for I,J in next,G do
if ae.Value[J]then
H=H..J..', ';
end;
end;

H=H:sub(1,#H-2);
else
H=ae.Value or'';
end;

am.Text=(H==''and'--'or H);
end;
function ae:GetActiveValues()
if ad.Multi then
local G={};
for H,I in next,ae.Value do
table.insert(G,H);
end;

return G
else
return ae.Value and 1 or 0
end;
end;

function ae:BuildDropdownList()
local G=ae.Values;
local H={};

for I,J in next,F:GetChildren()do
if not J:IsA('UIListLayout')then
J:Destroy();
end;
end;

local I=0;

for J,K in next,G do
local L={};
I=I+1;

local M=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Middle;
Size=UDim2.new(1,-1,0,20);
ZIndex=23;
Active=true,
Parent=F;
});
s:AddToRegistry(M,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
local N=s:CreateLabel({
Active=false;
Size=UDim2.new(1,-6,1,0);
Position=UDim2.new(0,6,0,0);
TextSize=16;
Text=K;
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=25;
Parent=M;
});

s:OnHighlight(M,M,
{BorderColor3='AccentColor',ZIndex=24},
{BorderColor3='OutlineColor',ZIndex=23}
);
local O;

if ad.Multi then
O=ae.Value[K];
else
O=ae.Value==K;
end;

function L:UpdateButton()
if ad.Multi then
O=ae.Value[K];
else
O=ae.Value==K;
end;

N.TextColor3=O and s.AccentColor or s.FontColor;
s.RegistryMap[N].Properties.TextColor3=O and'AccentColor'or'FontColor';
end;
N.InputBegan:Connect(function(P)
if(P.UserInputType==Enum.UserInputType.MouseButton1 or P.UserInputType==Enum.UserInputType.Touch)then
local Q=not O;

if ae:GetActiveValues()==1 and(not Q)and(not ad.AllowNull)then
else
if ad.Multi then
O=Q;

if O then
ae.Value[K]=true;
else
ae.Value[K]=nil;
end;
else
O=Q;

if O then
ae.Value=K;
else
ae.Value=nil;
end;

for R,S in next,H do
S:UpdateButton();
end;
end;

L:UpdateButton();
ae:Display();

s:SafeCallback(ae.Callback,ae.Value);
s:SafeCallback(ae.Changed,ae.Value);

s:AttemptSave();
end;
end;
end);

L:UpdateButton();
ae:Display();

H[M]=L;
end;
F.CanvasSize=UDim2.fromOffset(0,(I*20)+1);

local J=math.clamp(I*20,0,A*20)+1;
D(J);
end;

function ae:SetValues(G)
if G then
ae.Values=G;
end;

ae:BuildDropdownList();
end;

function ae:OpenDropdown()
B.Visible=true;
s.OpenedFrames[B]=true;
al.Rotation=180;
end;

function ae:CloseDropdown()
B.Visible=false;
s.OpenedFrames[B]=nil;
al.Rotation=0;
end;

function ae:OnChanged(G)
ae.Changed=G;
G(ae.Value);
end;

function ae:SetValue(G)
if ae.Multi then
local H={};
for I,J in next,G do
if table.find(ae.Values,I)then
H[I]=true
end;
end;

ae.Value=H;
else
if(not G)then
ae.Value=nil;
elseif table.find(ae.Values,G)then
ae.Value=G;
end;
end;

ae:BuildDropdownList();

s:SafeCallback(ae.Callback,ae.Value);
s:SafeCallback(ae.Changed,ae.Value);
end;

aj.InputBegan:Connect(function(G)
if(G.UserInputType==Enum.UserInputType.MouseButton1 or G.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then
if B.Visible then
ae:CloseDropdown();
else
ae:OpenDropdown();
end;
end;
end);
c.InputBegan:Connect(function(G)
if(G.UserInputType==Enum.UserInputType.MouseButton1 or G.UserInputType==Enum.UserInputType.Touch)then
local H,I=B.AbsolutePosition,B.AbsoluteSize;

if m.X<H.X or m.X>H.X+I.X
or m.Y<(H.Y-20-1)or m.Y>H.Y+I.Y then

ae:CloseDropdown();
end;
end;
end);
ae:BuildDropdownList();
ae:Display();

local G={}

if type(ad.Default)=='string'then
local H=table.find(ae.Values,ad.Default)
if H then
table.insert(G,H)
end
elseif type(ad.Default)=='table'then
for H,I in next,ad.Default do
local J=table.find(ae.Values,I)
if J then
table.insert(G,J)
end
end
elseif type(ad.Default)=='number'and ae.Values[ad.Default]~=nil then
table.insert(G,ad.Default)
end

if next(G)then
for H=1,#G do
local I=G[H]
if ad.Multi then
ae.Value[ae.Values[I] ]=true
else
ae.Value=ae.Values[I];
end

if(not ad.Multi)then break end
end

ae:BuildDropdownList();
ae:Display();
end

af:AddBlank(ad.BlankSize or 5);
af:Resize();

r[ac]=ae;

return ae
end;
function ab:AddDependencyBox()
local ac={
Dependencies={};
};

local ad=self;
local ae=ad.Container;

local af=s:Create('Frame',{
BackgroundTransparency=1;
Size=UDim2.new(1,0,0,0);
Visible=false;
Parent=ae;
});
local ah=s:Create('Frame',{
BackgroundTransparency=1;
Size=UDim2.new(1,0,1,0);
Visible=true;
Parent=af;
});
local ai=s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=ah;
});
function ac:Resize()
af.Size=UDim2.new(1,0,0,ai.AbsoluteContentSize.Y);
ad:Resize();
end;

ai:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
ac:Resize();
end);
af:GetPropertyChangedSignal('Visible'):Connect(function()
ac:Resize();
end);
function ac:Update()
for aj,ak in next,ac.Dependencies do
local al=ak[1];
local am=ak[2];

if al.Type=='Toggle'and al.Value~=am then
af.Visible=false;
ac:Resize();
return
end;
end;

af.Visible=true;
ac:Resize();
end;

function ac:SetupDependencies(aj)
for ak,al in next,aj do
assert(type(al)=='table','SetupDependencies: Dependency is not of type `table`.');
assert(al[1],'SetupDependencies: Dependency is missing element argument.');
assert(al[2]~=nil,'SetupDependencies: Dependency is missing value argument.');
end;

ac.Dependencies=aj;
ac:Update();
end;

ac.Container=ah;

setmetatable(ac,aa);

table.insert(s.DependencyBoxes,ac);

return ac
end;

aa.__index=ab;
aa.__namecall=function(ac,ad,...)
return ab[ad](...)
end;
end;
do
s.NotificationArea=s:Create('Frame',{
BackgroundTransparency=1;
Position=UDim2.new(0,s.NotifyConfig.PositionX,0,s.NotifyConfig.PositionY);
Size=UDim2.new(0,300,1,-s.NotifyConfig.PositionY);
ZIndex=100;
Parent=p;
});
s.NotifLayout=s:Create('UIListLayout',{
Padding=UDim.new(0,4);
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=s.NotificationArea;
});
local function ab()
local ac=s.NotifyConfig
local ad=s.NotificationArea
local ae=s.NotifLayout

ad.Position=UDim2.new(0,ac.PositionX,0,ac.PositionY)
ad.Size=UDim2.new(0,300,1,-ac.PositionY)

local af=ac.Alignment or'Left'
if af=='Left'then
ae.HorizontalAlignment=Enum.HorizontalAlignment.Left
ad.AnchorPoint=Vector2.new(0,0)
elseif af=='Right'then
ae.HorizontalAlignment=Enum.HorizontalAlignment.Right
ad.AnchorPoint=Vector2.new(0,0)
elseif af=='Center'then
ae.HorizontalAlignment=Enum.HorizontalAlignment.Center
ad.AnchorPoint=Vector2.new(0,0)
end
end
s.UpdateNotifAlignment=ab
ab()

local ac=s:Create('Frame',{

BorderSizePixel=1;
BorderColor3=Color3.new(0,0,0);
Position=UDim2.new(0.5,0,0,8);
Size=UDim2.new(0,0,0,20);
AutomaticSize=Enum.AutomaticSize.X;
ZIndex=200;
Visible=false;
Parent=p;
});

local ad=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.AccentColor;
BorderSizePixel=0;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=201;
Parent=ac;
});
s:AddToRegistry(ad,{
BorderColor3='AccentColor';
});




























local ae=s:Create('Frame',{
BackgroundColor3=Color3.new(1,1,1);
BorderSizePixel=0;
Position=UDim2.new(0,1,0,1);
Size=UDim2.new(1,-2,1,-2);
ZIndex=202;
Parent=ad;
});
local af=s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,s:GetDarkerColor(s.MainColor)),
ColorSequenceKeypoint.new(1,s.MainColor),
});
Rotation=-90;
Parent=ae;
});
s:AddToRegistry(af,{
Color=function()
return ColorSequence.new({
ColorSequenceKeypoint.new(0,s:GetDarkerColor(s.MainColor)),
ColorSequenceKeypoint.new(1,s.MainColor),
})
end
});
local ah=s:CreateLabel({
Position=UDim2.new(0,18,0,-1);
Size=UDim2.new(0,0,1,0);
AutomaticSize=Enum.AutomaticSize.X;
TextSize=s.FontSize-1;
TextXAlignment=Enum.TextXAlignment.Left;
RichText=true;
ZIndex=203;
Parent=ae;
});
local ai=s:Create('ImageLabel',{
AnchorPoint=Vector2.new(0,0.5);
BackgroundTransparency=1;
ImageColor3=s.AccentColor;
Position=UDim2.new(0,-8,0,9);
Size=UDim2.new(0,35,0,35);
Image='http://www.roblox.com/asset/?id=94332135815092';
ZIndex=204;
Parent=ae;
});
s:AddToRegistry(ai,{
ImageColor3='AccentColor';
});
s:Create('UIPadding',{
PaddingRight=UDim.new(0,8);
Parent=ah;
});
s.Watermark=ac;
s.WatermarkText=ah;
s:MakeDraggable(s.Watermark);

local aj=s:Create('Frame',{
AnchorPoint=Vector2.new(0,0.5);
BorderColor3=Color3.new(0,0,0);
Position=UDim2.new(0,10,0.5,0);
Size=UDim2.new(0,210,0,20);
Visible=false;
ZIndex=100;
Parent=p;
});
s:ApplyGlow(aj);

local ak=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=101;
Parent=aj;
});
s:AddToRegistry(ak,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
},true);
local al=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
Position=UDim2.fromOffset(0,15);
Size=UDim2.new(1,0,0,2);
ZIndex=102;
Parent=ak;
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1));
ColorSequenceKeypoint.new(0.5,Color3.new(1,1,1));
ColorSequenceKeypoint.new(1,Color3.new(1,1,1));
});
Transparency=NumberSequence.new({
NumberSequenceKeypoint.new(0,1);
NumberSequenceKeypoint.new(0.2,0.6);
NumberSequenceKeypoint.new(0.5,0);
NumberSequenceKeypoint.new(0.8,0.6);
NumberSequenceKeypoint.new(1,1);
});
Rotation=0;
Parent=al;
});
s:AddToRegistry(al,{
BackgroundColor3='AccentColor';
},true);
local am=s:CreateLabel({
Size=UDim2.new(1,0,0,20);
Position=UDim2.fromOffset(0,-5),
TextXAlignment=Enum.TextXAlignment.Center,
TextSize=s.FontSize+1;
Text='Keybinds';
ZIndex=104;
Parent=ak;
});
local A=s:Create('Frame',{
BackgroundTransparency=1;
Size=UDim2.new(1,0,1,-20);
Position=UDim2.new(0,0,0,17);
ZIndex=1;
Parent=ak;
});
s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=A;
});
s:Create('UIPadding',{
PaddingLeft=UDim.new(0,5),
Parent=A,
})

s.KeybindFrame=aj;
s.KeybindContainer=A;
s:MakeDraggable(aj);
end;

function s:SetKeybindMode(ab)
assert(ab=='All'or ab=='Active'or ab=='Toggled',
"SetKeybindMode: Mode must be 'All', 'Active', or 'Toggled'")
s.KeybindMode=ab
s:RefreshKeybinds()
end

function s:RefreshKeybinds()
for ab,ac in ipairs(s.KeyPickerList)do
if not ac.NoUI then
pcall(function()ac:Update()end)
end
end
end

function s:SetWatermarkVisibility(ab)
s.Watermark.Visible=ab;
end;

function s:SetWatermark(ab)
local ac=s.Watermark.Position.Y
s.WatermarkText.Text=ab
s.Watermark.AnchorPoint=Vector2.new(0.5,0)
s.Watermark.Position=UDim2.new(0.5,0,ac.Scale,ac.Offset)
s:SetWatermarkVisibility(true)
end
function s:Notify(ab,ac)
local ad=s.NotifyConfig
local ae=ad.BarSide or'Left'
local af=ad.Alignment or'Left'

local ah,ai=s:GetTextBounds(ab,s.Font,s.FontSize)
ai=ai+7

local aj=3
local ak=3

local al=(ae=='Left')and 1 or 1
local am=(ae=='Top')and ak or 1
local A=(ae=='Left'or ae=='Right')and-2 or-2
local B=(ae=='Top'or ae=='Bottom')and-(ak+1)or-2

local C=(ae=='Left')and aj+2 or 4
local D=(ae=='Left'or ae=='Right')and-(aj+4)or-4

local E=Vector2.new(0,0)
local F=0
if af=='Center'then
E=Vector2.new(0.5,0)
F=0
elseif af=='Right'then
E=Vector2.new(1,0)
F=0
end

local G=s:Create('Frame',{
BackgroundTransparency=1;
AnchorPoint=E;
BorderColor3=Color3.new(0,0,0);
Position=(af=='Center')
and UDim2.new(0.5,0,0,0)
or(af=='Right'and UDim2.new(1,0,0,0)or UDim2.new(0,0,0,0));
Size=UDim2.new(0,0,0,ai);
ClipsDescendants=true;
ZIndex=100;
Parent=s.NotificationArea;
});
local H=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,0);
ZIndex=101;
Parent=G;
});
s:AddToRegistry(H,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
},true);
local I=s:Create('Frame',{
BackgroundColor3=Color3.new(1,1,1);
BorderSizePixel=0;
Position=UDim2.new(0,al,0,am);
Size=UDim2.new(1,A,1,B);
ZIndex=102;
Parent=H;
});
local J=s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,s:GetDarkerColor(s.MainColor)),
ColorSequenceKeypoint.new(1,s.MainColor),
});
Rotation=-90;
Parent=I;
});
s:AddToRegistry(J,{
Color=function()
return ColorSequence.new({
ColorSequenceKeypoint.new(0,s:GetDarkerColor(s.MainColor)),
ColorSequenceKeypoint.new(1,s.MainColor),
})
end
});
local K=s:CreateLabel({
Position=UDim2.new(0,C,0,0);
Size=UDim2.new(1,D,1,0);
Text=ab;
TextXAlignment=(af=='Center')
and Enum.TextXAlignment.Center
or Enum.TextXAlignment.Left;
TextSize=s.FontSize;
ZIndex=103;
Parent=I;
});
local L=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
ZIndex=104;
Parent=G;
});
if ae=='Left'then
L.Position=UDim2.new(0,-1,0,-1)
L.Size=UDim2.new(0,aj,1,2)
elseif ae=='Right'then
L.Position=UDim2.new(1,-aj+1,0,-1)
L.Size=UDim2.new(0,aj,1,2)
elseif ae=='Top'then
L.Position=UDim2.new(0,-1,0,-1)
L.Size=UDim2.new(1,2,0,ak)
elseif ae=='Bottom'then
L.Position=UDim2.new(0,-1,1,-ak+1)
L.Size=UDim2.new(1,2,0,ak)
end

s:AddToRegistry(L,{
BackgroundColor3='AccentColor';
},true);
local M=ah+8+4
if ae=='Left'or ae=='Right'then
M=M+aj
end
pcall(G.TweenSize,G,
UDim2.new(0,M,0,ai),'Out','Quad',0.4,true);
task.spawn(function()
wait(ac or 5);
pcall(G.TweenSize,G,
UDim2.new(0,0,0,ai),'Out','Quad',0.4,true);
wait(0.4);
G:Destroy();
end);
end;

function s:CreateWindow(...)
local ab={...}
local ac={AnchorPoint=Vector2.zero}

if type(...)=='table'then
ac=...;
else
ac.Title=ab[1]
ac.AutoShow=ab[2]or false;
end

if type(ac.Title)~='string'then ac.Title='No title'end
if type(ac.TabPadding)~='number'then ac.TabPadding=0 end
if type(ac.MenuFadeTime)~='number'then ac.MenuFadeTime=0.2 end

if typeof(ac.Size)~='UDim2'then ac.Size=UDim2.fromOffset(550,600)end
if typeof(ac.Position)~='UDim2'then ac.Position=UDim2.fromOffset(175,50)end

if c.TouchEnabled then
local ad=workspace.CurrentCamera.ViewportSize
local ae=math.min(ac.Size.X.Offset,ad.X-20)

local af=math.min(ac.Size.Y.Offset,ad.Y-60)
ac.Size=UDim2.fromOffset(ae,af)
end

if ac.Center then
ac.AnchorPoint=Vector2.new(0.5,0.5)
ac.Position=UDim2.fromScale(0.5,0.5)
end

if ac.WireframeDrag~=nil then
s.WireframeDrag=ac.WireframeDrag
end

local ad={
Tabs={};
};

local ae=s:Create('Frame',{
AnchorPoint=ac.AnchorPoint,
BackgroundTransparency=1,
BorderSizePixel=0;
Position=ac.Position,
Size=ac.Size,
Visible=false;
ZIndex=1;
Parent=p;
});
s:MakeDraggable(ae,25,true);

if ac.Resizable then
s:MakeResizable(ae,ac.MinSize,ac.MaxSize)
end

local af=s:Create('Frame',{
Name="Inner",
BackgroundColor3=s.MainColor;
BorderColor3=s.Black;
BorderMode=Enum.BorderMode.Outline;
Position=UDim2.new(0,1,0,1);
Size=UDim2.new(1,-2,1,-2);
ZIndex=1;
Parent=ae;
});
s:AddToRegistry(af,{
BackgroundColor3='MainColor';
BorderColor3='Black';
});

local ah=s:CreateLabel({
Position=UDim2.new(0,9,0,4);
Size=UDim2.new(1,0,0,25);
TextSize=16;
Text=ac.Title or'';
RichText=true;
TextXAlignment=Enum.TextXAlignment.Center;
ZIndex=1;
Parent=af;
});

local ai=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
Position=UDim2.new(0,7,0,0);
Size=UDim2.new(1,-14,0,1);
Visible=true;
ZIndex=50;
Parent=af;
});
s:AddToRegistry(ai,{
BackgroundColor3='AccentColor';
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1));
ColorSequenceKeypoint.new(0.5,Color3.new(1,1,1));
ColorSequenceKeypoint.new(1,Color3.new(1,1,1));
});
Transparency=NumberSequence.new({
NumberSequenceKeypoint.new(0,1);
NumberSequenceKeypoint.new(0.2,0.6);
NumberSequenceKeypoint.new(0.5,0);
NumberSequenceKeypoint.new(0.8,0.6);
NumberSequenceKeypoint.new(1,1);
});
Rotation=0;
Parent=ai;
});

local aj=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Position=UDim2.new(0,7,0,27);
Size=UDim2.new(1,-14,0,27);
ZIndex=6;
Parent=af;
});
s:AddToRegistry(aj,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});

local ak=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
Position=UDim2.new(0,0,0,0);
BorderColor3=Color3.new(0,0,0);
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,1,4);
ZIndex=7;
Parent=aj;
});
s:AddToRegistry(ak,{
BackgroundColor3='BackgroundColor';
});

local al=s:Create("UIGradient",{
Rotation=90,
Parent=ak,
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.fromRGB(255,255,255)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100)),
}),
});

local am=s:Create('Frame',{
BackgroundTransparency=1;
Position=UDim2.new(0,4,0,4);
Size=UDim2.new(1,-8,1,-8);
ZIndex=8;
Parent=ak;
});

local A=s:Create('UIListLayout',{
Padding=UDim.new(0,ac.TabPadding);
FillDirection=Enum.FillDirection.Horizontal;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=am;
});



local B=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
Position=UDim2.new(0,8,0,54);
Size=UDim2.new(1,-16,1,-62);
ZIndex=2;
Parent=af;
});
s:AddToRegistry(B,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});

local C=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=Color3.new(0,0,0);
BorderMode=Enum.BorderMode.Inset;
Position=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,1,0);
ZIndex=3;
Parent=B;
});
s:AddToRegistry(C,{
BackgroundColor3='BackgroundColor';
});

local D=s:Create('Frame',{
BackgroundColor3=s.MainColor;
BorderColor3=s.OutlineColor;
Position=UDim2.new(0,8,0,8);
Size=UDim2.new(1,-16,1,-16);
ZIndex=4;
Parent=C;
});
s:AddToRegistry(D,{
BackgroundColor3='MainColor';
BorderColor3='OutlineColor';
});
function ad:SetWindowTitle(E)
ah.Text=E;
end;
function ad:AddTab(E)
local F={
Groupboxes={};
Tabboxes={};
};

local G=s:GetTextBounds(E,s.Font,s.FontSize+2)

local H=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BackgroundTransparency=1;
BorderSizePixel=0;
Size=UDim2.new(0,G+12,1,0);
ZIndex=9;
Parent=am;
})
s:AddToRegistry(H,{
BackgroundColor3='MainColor';
})

local I=s:Create("UIStroke",{
Color=s.AccentColor,
Thickness=1,
LineJoinMode=Enum.LineJoinMode.Miter,
Parent=H,
ApplyStrokeMode=Enum.ApplyStrokeMode.Border,
Enabled=false,
BorderStrokePosition=Enum.BorderStrokePosition.Inner,
})

s:AddToRegistry(I,{
Color='AccentColor';
})

s:OnHighlight(H,I,{
Color="AccentColor",
Enabled=true,
},{
Color="BackgroundColor",
Enabled=false,
})

local J=s:CreateLabel({
Position=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,1,0);
TextSize=16;
TextColor3=s.FontColor;
Text=E;
ZIndex=10;
Parent=H;
})

s:AddToRegistry(J,{
TextColor3='FontColor';
})







































local K=s:Create('Frame',{
Name='TabFrame';
BackgroundTransparency=1;
Position=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,1,0);
Visible=false;
ZIndex=5;
Parent=D;
})

local L=s:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
Position=UDim2.new(0,8-1,0,8-1);
Size=UDim2.new(0.5,-12+2,1,-16);
CanvasSize=UDim2.new(0,0,0,0);
BottomImage='';
TopImage='';
ScrollBarThickness=0;
ZIndex=6;
Parent=K;
})

local M=s:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
Position=UDim2.new(0.5,4+1,0,8-1);
Size=UDim2.new(0.5,-12+2,1,-16);
CanvasSize=UDim2.new(0,0,0,0);
BottomImage='';
TopImage='';
ScrollBarThickness=0;
ZIndex=6;
Parent=K;
})

s:Create('UIListLayout',{
Padding=UDim.new(0,8);
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
HorizontalAlignment=Enum.HorizontalAlignment.Center;
Parent=L;
})
s:Create('UIListLayout',{
Padding=UDim.new(0,8);
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
HorizontalAlignment=Enum.HorizontalAlignment.Center;
Parent=M;
})

for N,O in next,{L,M}do
O:WaitForChild('UIListLayout'):GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
O.CanvasSize=UDim2.fromOffset(0,O.UIListLayout.AbsoluteContentSize.Y)
end)
end


function F:ShowTab()
for N,O in next,ad.Tabs do
O:HideTab()
end

H.BackgroundTransparency=0
H.BackgroundColor3=s.MainColor
s.RegistryMap[H].Properties.BackgroundColor3='MainColor'

K.Visible=true




H.BackgroundTransparency=1
J.TextColor3=s.AccentColor
s.RegistryMap[J].Properties.TextColor3='AccentColor'

end

function F:HideTab()
H.BackgroundTransparency=1
H.BackgroundColor3=s.BackgroundColor
s.RegistryMap[H].Properties.BackgroundColor3='BackgroundColor'

K.Visible=false




H.BackgroundTransparency=1
J.TextColor3=s.FontColor
s.RegistryMap[J].Properties.TextColor3='FontColor'
end
function F:SetLayoutOrder(N)
H.LayoutOrder=N;
A:ApplyLayout();
end;
function F:AddGroupbox(N)
local O={};
local P=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,0,507+2);
ZIndex=2;
Parent=N.Side==1 and L or M;
});
s:AddToRegistry(P,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});
local Q=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-2,1,-2);
Position=UDim2.new(0,1,0,1);
ZIndex=4;
Parent=P;
});
s:AddToRegistry(Q,{
BackgroundColor3='BackgroundColor';
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=Q;
});
local R=s:Create('Frame',{
BackgroundColor3=s.OutlineColor;
BorderSizePixel=0;
Position=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,0,18);
ZIndex=5;
Parent=Q;
});
s:AddToRegistry(R,{
BackgroundColor3='OutlineColor';
});
local S=s:Create("UIGradient",{
Rotation=90,
Parent=R,
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.fromRGB(255,255,255)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100)),
}),
});
local T=s:Create('Frame',{
BackgroundColor3=Color3.fromRGB(0,0,0);
BorderSizePixel=0;
Position=UDim2.new(0,0,0,18);
Size=UDim2.new(1,0,0,1);
ZIndex=6;
Parent=R;
});





























local U=s:CreateLabel({
Size=UDim2.new(1,0,0,18);

Position=UDim2.new(0,1,0,0);
TextSize=16;
Text=N.Name;
TextXAlignment=Enum.TextXAlignment.Left;
ZIndex=5;
Parent=Q;
});
local V=s:Create('Frame',{
BackgroundTransparency=1;
Position=UDim2.new(0,4,0,20);
Size=UDim2.new(1,-4,1,-20);
ZIndex=1;
Parent=Q;
});
s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=V;
});
function O:Resize()
local W=0;
for X,Y in next,O.Container:GetChildren()do
if(not Y:IsA('UIListLayout'))and Y.Visible then
W=W+Y.Size.Y.Offset;
end;
end;

P.Size=UDim2.new(1,0,0,20+W+2+2);
end;

O.Container=V;
setmetatable(O,aa);
O:AddBlank(3);
O:Resize();

F.Groupboxes[N.Name]=O;

return O
end;

function F:AddLeftGroupbox(N)
return F:AddGroupbox({Side=1;Name=N;})
end;

function F:AddRightGroupbox(N)
return F:AddGroupbox({Side=2;Name=N;})
end;

function F:AddTabbox(N)
local O={
Tabs={};
};

local P=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=s.OutlineColor;
BorderMode=Enum.BorderMode.Inset;
Size=UDim2.new(1,0,0,0);
ZIndex=2;
Parent=N.Side==1 and L or M;
});
s:AddToRegistry(P,{
BackgroundColor3='BackgroundColor';
BorderColor3='OutlineColor';
});
local Q=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderColor3=Color3.new(0,0,0);
Size=UDim2.new(1,-2,1,-2);
Position=UDim2.new(0,1,0,1);
ZIndex=4;
Parent=P;
});
s:AddToRegistry(Q,{
BackgroundColor3='BackgroundColor';
});
s:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(212,212,212))
});
Rotation=90;
Parent=Q;
});
local R=s:Create('Frame',{
BackgroundColor3=Color3.fromRGB(0,0,0);
BorderSizePixel=0;
Position=UDim2.new(0,0,0,19);
Size=UDim2.new(1,0,0,1);
ZIndex=6;
Parent=P;
});
local S=s:Create('Frame',{
BackgroundColor3=s.AccentColor;
BorderSizePixel=0;
Position=UDim2.new(0,0,0,0);
Size=UDim2.new(1,0,0,1);
Visible=true;
ZIndex=10;
Parent=Q;
});
s:AddToRegistry(S,{
BackgroundColor3='AccentColor';
});
local T=s:Create('Frame',{
BackgroundTransparency=1;
Position=UDim2.new(0,0,0,1);
Size=UDim2.new(1,0,0,18);
ZIndex=5;
Parent=Q;
});
s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Horizontal;
HorizontalAlignment=Enum.HorizontalAlignment.Left;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=T;
});
function O:AddTab(U)
local V={};
local W=s:Create('Frame',{
BackgroundColor3=s:GetDarkerColor(s.OutlineColor);
Position=UDim2.new(0,0,0,-2);
Size=UDim2.new(0.5,0,1,2);
BorderSizePixel=0;
BorderColor3=Color3.fromRGB(0,0,0);
BorderMode=Enum.BorderMode.Inset;
ZIndex=6;
Parent=T;
});
s:AddToRegistry(W,{
BackgroundColor3=function()return s:GetDarkerColor(s.OutlineColor)end;
});
local X=s:Create("UIGradient",{
Rotation=90,
Parent=W,
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,Color3.fromRGB(255,255,255)),
ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100)),
}),
});


















local Y=s:CreateLabel({
Size=UDim2.new(1,0,1,0);
TextSize=16;
Text=U;
TextXAlignment=Enum.TextXAlignment.Center;
ZIndex=7;
Parent=W;
});
local Z=s:Create('Frame',{
BackgroundColor3=s.BackgroundColor;
BorderSizePixel=0;
Position=UDim2.new(0,0,1,0);
Size=UDim2.new(1,0,0,1);
Visible=false;
ZIndex=9;
Parent=W;
});
s:AddToRegistry(Z,{
BackgroundColor3='BackgroundColor';
});
local _=s:Create('Frame',{
BackgroundTransparency=1;
Position=UDim2.new(0,4,0,20);
Size=UDim2.new(1,-4,1,-20);
ZIndex=1;
Visible=false;
Parent=Q;
});
s:Create('UIListLayout',{
FillDirection=Enum.FillDirection.Vertical;
SortOrder=Enum.SortOrder.LayoutOrder;
Parent=_;
});
function V:Show()
for an,ao in next,O.Tabs do
if ao~=V then
ao:Hide();
end;
end;

_.Visible=true;
Z.Visible=true;

W.BackgroundColor3=s.BackgroundColor;
s.RegistryMap[W].Properties.BackgroundColor3='BackgroundColor';
Y.TextColor3=s.AccentColor;
s.RegistryMap[Y].Properties.TextColor3='AccentColor';
V:Resize();
end;

function V:Hide()
_.Visible=false;
Z.Visible=false;

W.BackgroundColor3=s:GetDarkerColor(s.OutlineColor);
s.RegistryMap[W].Properties.BackgroundColor3=function()return s:GetDarkerColor(s.OutlineColor)end;
Y.TextColor3=s.FontColor;
s.RegistryMap[Y].Properties.TextColor3='FontColor';
end;
function V:Resize()
local an=0;
for ao,ap in next,O.Tabs do
an=an+1;
end;

for ao,ap in next,T:GetChildren()do
if not ap:IsA('UIListLayout')then
ap.Size=UDim2.new(1/an,0,1,0);
end;
end;

if(not _.Visible)then
return
end;

local ao=0;

for ap,aq in next,V.Container:GetChildren()do
if(not aq:IsA('UIListLayout'))and aq.Visible then
ao=ao+aq.Size.Y.Offset;
end;
end;

P.Size=UDim2.new(1,0,0,20+ao+2+2);
end;
W.InputBegan:Connect(function(an)
if(an.UserInputType==Enum.UserInputType.MouseButton1 or an.UserInputType==Enum.UserInputType.Touch)and not s:MouseIsOverOpenedFrame()then
V:Show();
V:Resize();
end;
end);

V.Container=_;
O.Tabs[U]=V;

setmetatable(V,aa);

V:AddBlank(3);
V:Resize();

if#T:GetChildren()==2 then
V:Show();
end;

return V
end;

F.Tabboxes[N.Name or'']=O;

return O
end;
function F:AddLeftTabbox(an)
return F:AddTabbox({Name=an,Side=1;})
end;

function F:AddRightTabbox(an)
return F:AddTabbox({Name=an,Side=2;})
end;

H.InputBegan:Connect(function(an)
if(an.UserInputType==Enum.UserInputType.MouseButton1 or an.UserInputType==Enum.UserInputType.Touch)then
F:ShowTab();
end;
end);
if#D:GetChildren()==1 then
F:ShowTab();
end;
ad.Tabs[E]=F;
return F
end;

local an=s:Create('TextButton',{
BackgroundTransparency=1;
Size=UDim2.new(0,0,0,0);
Visible=true;
Text='';
Modal=false;
Parent=p;
});
function s:Toggle()
s.Toggled=not s.Toggled;
an.Modal=s.Toggled;
ae.Visible=s.Toggled;
if s.Toggled then
task.spawn(function()
local ao=c.MouseIconEnabled;

local ap=Drawing.new('Triangle');
ap.Thickness=1;
ap.Filled=true;
ap.Visible=true;

local aq=Drawing.new('Triangle');
aq.Thickness=1;
aq.Filled=false;
aq.Color=Color3.new(0,0,0);
aq.Visible=true;

while s.Toggled and p.Parent do
c.MouseIconEnabled=false;

local E=c:GetMouseLocation();

ap.Color=s.AccentColor;

ap.PointA=Vector2.new(E.X,E.Y);
ap.PointB=Vector2.new(E.X+16,E.Y+6);
ap.PointC=Vector2.new(E.X+6,E.Y+16);
aq.PointA=ap.PointA;
aq.PointB=ap.PointB;
aq.PointC=ap.PointC;

k:Wait();
end;

c.MouseIconEnabled=ao;

ap:Remove();
aq:Remove();
end);
end;
if s.UseBlur then
if s.Toggled then
s.BlurEffect.Enabled=true
s.BlurEffect.Size=s.BlurSize
else
s.BlurEffect.Size=0
s.BlurEffect.Enabled=false
end
else
s.BlurEffect.Size=0
s.BlurEffect.Enabled=false
end
end

s:GiveSignal(c.InputBegan:Connect(function(ao,ap)
if type(s.ToggleKeybind)=='table'and s.ToggleKeybind.Type=='KeyPicker'then
if ao.UserInputType==Enum.UserInputType.Keyboard and ao.KeyCode.Name==s.ToggleKeybind.Value then
task.spawn(s.Toggle)
end
elseif type(s.ToggleKeybind)=='string'then
if ao.UserInputType==Enum.UserInputType.Keyboard and ao.KeyCode.Name==s.ToggleKeybind then
task.spawn(s.Toggle)
end
elseif ao.KeyCode==Enum.KeyCode.RightControl or(ao.KeyCode==Enum.KeyCode.RightShift and(not ap))then
task.spawn(s.Toggle)
end
end))

if ac.AutoShow then task.spawn(s.Toggle)end

ad.Holder=ae;
return ad
end;

local function ab()
local ac=v();
for ad,ae in next,r do
if ae.Type=='Dropdown'and ae.SpecialType=='Player'then
ae:SetValues(ac);
end;
end;
end;

g.PlayerAdded:Connect(ab);
g.PlayerRemoving:Connect(ab);

if c.TouchEnabled then
local ac=Instance.new("ScreenGui")
ac.Name="LinoriaMobileUI"
ac.ZIndexBehavior=Enum.ZIndexBehavior.Global
n(ac)
ac.Parent=e

local ad,ae=88,30
local af=40

local function ah(ai,aj,ak)
local al=s:Create('Frame',{
Name=ai.."Outer",
BackgroundColor3=s.OutlineColor,
BorderSizePixel=0,
Position=ak,
Size=UDim2.new(0,ad,0,ae),
ZIndex=300,
Parent=ac,
Active=true,
})
s:AddToRegistry(al,{BackgroundColor3='OutlineColor'})

local am=s:Create('Frame',{
Name=ai.."Accent",
BackgroundColor3=s.AccentColor,
BorderSizePixel=0,
Position=UDim2.new(0,1,0,1),
Size=UDim2.new(1,-2,1,-2),
ZIndex=301,
Parent=al,
})
s:AddToRegistry(am,{BackgroundColor3='AccentColor'})

local an=s:Create('Frame',{
Name=ai.."Inner",
BackgroundColor3=Color3.fromRGB(8,8,12),
BorderSizePixel=0,
Position=UDim2.new(0,1,0,1),
Size=UDim2.new(1,-2,1,-2),
ZIndex=302,
Parent=am,
})

local ao=s:Create('Frame',{
Name=ai.."Gradient",
BackgroundColor3=Color3.new(1,1,1),
BorderSizePixel=0,
Size=UDim2.new(1,0,1,0),
ZIndex=303,
Parent=an,
})
s:Create('UIGradient',{
Transparency=NumberSequence.new({
NumberSequenceKeypoint.new(0,0.90),
NumberSequenceKeypoint.new(1,1.0)
}),
Rotation=90,
Parent=ao,
})

local ap=s:Create('TextButton',{
Name=ai.."Btn",
BackgroundTransparency=1,
Size=UDim2.new(1,0,1,0),
Font=Enum.Font.Code,
Text=aj,
TextColor3=Color3.fromRGB(255,255,255),
TextSize=s.FontSize-1,
ZIndex=304,
Parent=an,
Active=true,
})

return al,ap
end

local ai,aj=ah("Toggle","Toggle UI",UDim2.new(0,10,0,10))
local ak,al=ah("Lock","Unlock UI",UDim2.new(0,10,0,10+ae+(af-ae)))

local am=false

local function an(ao,ap,aq)
local A=false
local B=nil
local C=nil
local D=nil
local E=false

ao.InputBegan:Connect(function(F)
if F.UserInputType==Enum.UserInputType.MouseButton1
or F.UserInputType==Enum.UserInputType.Touch then
A=true
E=false
C=F.Position
D=ap.Position
B=F

local G
G=F.Changed:Connect(function()
if F.UserInputState==Enum.UserInputState.End then
A=false
G:Disconnect()
if not E then
aq()
end
end
end)
end
end)

c.InputChanged:Connect(function(F)
if F==B and A then
local G=F.Position-C
if G.Magnitude>3 then
E=true
end
if am and E then
ap.Position=UDim2.new(
D.X.Scale,D.X.Offset+G.X,
D.Y.Scale,D.Y.Offset+G.Y
)
end
end
end)
end

an(aj,ai,function()
s:Toggle()
end)

an(al,ak,function()
am=not am
al.Text=am and"Lock UI"or"Unlock UI"
al.TextColor3=am
and s.AccentColor
or Color3.fromRGB(255,255,255)
end)

local ao=s.UpdateColorsUsingRegistry
s.UpdateColorsUsingRegistry=function(ap)
ao(ap)
end
end

getgenv().Library=s
return s end function a.a():typeof(b())local aa=a.cache.a if not aa then aa={c=b()}a.cache.a=aa end return aa.c end end do local function aa()

local ab=game:GetService('HttpService')
local ac={}do
ac.Folder='LinoriaLibSettings'


ac.Library=nil
ac.BuiltInThemes={
['Default']={1,ab:JSONDecode('{"MainColor":"141414","AccentColor":"b9b8e7","OutlineColor":"2d2d2d","BackgroundColor":"111111","FontColor":"ffffff"}')},
['Dracula']={2,ab:JSONDecode('{"FontColor":"f8f8f2","MainColor":"282a36","AccentColor":"bd93f9","BackgroundColor":"1e1f29","OutlineColor":"44475a"}')},
['NewBlue']={3,ab:JSONDecode('{"MainColor":"202427","AccentColor":"7b88a8","OutlineColor":"202427","BackgroundColor":"131617","FontColor":"ffffff"}')},
['Blue']={4,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"181818","AccentColor":"4777b6","BackgroundColor":"141414","OutlineColor":"1f1f1f"}')},
['Primordial']={5,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"181818","AccentColor":"d7a6b0","BackgroundColor":"1f1f1f","OutlineColor":"2a2a2a"}')},
['BBot']={6,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"1e1e1e","AccentColor":"7e48a3","BackgroundColor":"232323","OutlineColor":"141414"}')},
['Fatality']={7,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"1e1842","AccentColor":"c50754","BackgroundColor":"191335","OutlineColor":"3c355d"}')},
['Jester']={8,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"242424","AccentColor":"db4467","BackgroundColor":"1c1c1c","OutlineColor":"373737"}')},
['Mint']={9,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"242424","AccentColor":"3db488","BackgroundColor":"1c1c1c","OutlineColor":"373737"}')},
['Tokyo Night']={10,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"191925","AccentColor":"6759b3","BackgroundColor":"16161f","OutlineColor":"323232"}')},
['Ubuntu']={11,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"3e3e3e","AccentColor":"e2581e","BackgroundColor":"323232","OutlineColor":"191919"}')},
['Quartz']={12,ab:JSONDecode('{"FontColor":"ffffff","MainColor":"232330","AccentColor":"426e87","BackgroundColor":"1d1b26","OutlineColor":"27232f"}')},
}

function ac:ApplyTheme(ad)
local ae=self:GetCustomTheme(ad)
local af=ae or self.BuiltInThemes[ad]

if not af then return end



local ah=af[2]
for ai,aj in next,ae or ah do
self.Library[ai]=Color3.fromHex(aj)

if Options[ai]then
Options[ai]:SetValueRGB(Color3.fromHex(aj))
end
end

self:ThemeUpdate()
end

function ac:ThemeUpdate()

local ad={"FontColor","MainColor","AccentColor","BackgroundColor","OutlineColor"}
for ae,af in next,ad do
if Options and Options[af]then
self.Library[af]=Options[af].Value
end
end

self.Library.AccentColorDark=self.Library:GetDarkerColor(self.Library.AccentColor);
self.Library:UpdateColorsUsingRegistry()
end

function ac:LoadDefault()
local ad='Default'
local ae=isfile(self.Folder..'/themes/default.txt')and readfile(self.Folder..'/themes/default.txt')

local af=true
if ae then
if self.BuiltInThemes[ae]then
ad=ae
elseif self:GetCustomTheme(ae)then
ad=ae
af=false;
end
elseif self.BuiltInThemes[self.DefaultTheme]then
ad=self.DefaultTheme
end

if af then
Options.ThemeManager_ThemeList:SetValue(ad)
else
self:ApplyTheme(ad)
end
end

function ac:SaveDefault(ad)
writefile(self.Folder..'/themes/default.txt',ad)
end

function ac:CreateThemeManager(ad)
ad:AddLabel('Background color'):AddColorPicker('BackgroundColor',{Default=self.Library.BackgroundColor});
ad:AddLabel('Main color'):AddColorPicker('MainColor',{Default=self.Library.MainColor});
ad:AddLabel('Accent color'):AddColorPicker('AccentColor',{Default=self.Library.AccentColor});
ad:AddLabel('Outline color'):AddColorPicker('OutlineColor',{Default=self.Library.OutlineColor});
ad:AddLabel('Font color'):AddColorPicker('FontColor',{Default=self.Library.FontColor});

local ae={}
for af,ah in next,self.BuiltInThemes do
table.insert(ae,af)
end

table.sort(ae,function(af,ah)return self.BuiltInThemes[af][1]<self.BuiltInThemes[ah][1]end)

ad:AddDivider()
ad:AddDropdown('ThemeManager_ThemeList',{Text='Theme list',Values=ae,Default=1})

ad:AddButton('Set as default',function()
self:SaveDefault(Options.ThemeManager_ThemeList.Value)
self.Library:Notify(string.format('Set default theme to %q',Options.ThemeManager_ThemeList.Value))
end)

Options.ThemeManager_ThemeList:OnChanged(function()
self:ApplyTheme(Options.ThemeManager_ThemeList.Value)
end)

ad:AddDivider()
ad:AddInput('ThemeManager_CustomThemeName',{Text='Custom theme name'})
ad:AddDropdown('ThemeManager_CustomThemeList',{Text='Custom themes',Values=self:ReloadCustomThemes(),AllowNull=true,Default=1})
ad:AddDivider()

ad:AddButton('Save theme',function()
self:SaveCustomTheme(Options.ThemeManager_CustomThemeName.Value)

Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
Options.ThemeManager_CustomThemeList:SetValue(nil)
end):AddButton('Load theme',function()
self:ApplyTheme(Options.ThemeManager_CustomThemeList.Value)
end)

ad:AddButton('Refresh list',function()
Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
Options.ThemeManager_CustomThemeList:SetValue(nil)
end)

ad:AddButton('Set as default',function()
if Options.ThemeManager_CustomThemeList.Value~=nil and Options.ThemeManager_CustomThemeList.Value~=''then
self:SaveDefault(Options.ThemeManager_CustomThemeList.Value)
self.Library:Notify(string.format('Set default theme to %q',Options.ThemeManager_CustomThemeList.Value))
end
end)

ac:LoadDefault()

local function af()
self:ThemeUpdate()
end

Options.BackgroundColor:OnChanged(af)
Options.MainColor:OnChanged(af)
Options.AccentColor:OnChanged(af)
Options.OutlineColor:OnChanged(af)
Options.FontColor:OnChanged(af)
end

function ac:GetCustomTheme(ad)
local ae=self.Folder..'/themes/'..ad
if not isfile(ae)then
return nil
end

local af=readfile(ae)
local ah,ai=pcall(ab.JSONDecode,ab,af)

if not ah then
return nil
end

return ai
end

function ac:SaveCustomTheme(ad)
if ad:gsub(' ','')==''then
return self.Library:Notify('Invalid file name for theme (empty)',3)
end

local ae={}
local af={"FontColor","MainColor","AccentColor","BackgroundColor","OutlineColor"}

for ah,ai in next,af do
ae[ai]=Options[ai].Value:ToHex()
end

writefile(self.Folder..'/themes/'..ad..'.json',ab:JSONEncode(ae))
end

function ac:ReloadCustomThemes()
local ad=listfiles(self.Folder..'/themes')

local ae={}
for af=1,#ad do
local ah=ad[af]
if ah:sub(-5)=='.json'then


local ai=ah:find('.json',1,true)
local aj=ah:sub(ai,ai)

while aj~='/'and aj~='\\'and aj~=''do
ai=ai-1
aj=ah:sub(ai,ai)
end

if aj=='/'or aj=='\\'then
table.insert(ae,ah:sub(ai+1))
end
end
end

return ae
end

function ac:SetLibrary(ad)
self.Library=ad
end

function ac:BuildFolderTree()
local ad={}




local ae=self.Folder:split('/')
for af=1,#ae do
ad[#ad+1]=table.concat(ae,'/',1,af)
end

table.insert(ad,self.Folder..'/themes')
table.insert(ad,self.Folder..'/settings')

for af=1,#ad do
local ah=ad[af]
if not isfolder(ah)then
makefolder(ah)
end
end
end

function ac:SetFolder(ad)
self.Folder=ad
self:BuildFolderTree()
end

function ac:CreateGroupBox(ad)
assert(self.Library,'Must set ThemeManager.Library first!')
return ad:AddLeftGroupbox('Themes')
end

function ac:ApplyToTab(ad)
assert(self.Library,'Must set ThemeManager.Library first!')
local ae=self:CreateGroupBox(ad)
self:CreateThemeManager(ae)
end

function ac:ApplyToGroupbox(ad)
assert(self.Library,'Must set ThemeManager.Library first!')
self:CreateThemeManager(ad)
end

ac:BuildFolderTree()
end

return ac end function a.b():typeof(aa())local ab=a.cache.b if not ab then ab={c=aa()}a.cache.b=ab end return ab.c end end do local function aa()

local ab=game:GetService('HttpService')

local ac={}do
ac.Folder='LinoriaLibSettings'
ac.Ignore={}
ac.Parser={
Toggle={
Save=function(ad,ae)
return{type='Toggle',idx=ad,value=ae.Value}
end,
Load=function(ad,ae)
if Toggles[ad]then
Toggles[ad]:SetValue(ae.value)
end
end,
},
Slider={
Save=function(ad,ae)
return{type='Slider',idx=ad,value=tostring(ae.Value)}
end,
Load=function(ad,ae)
if Options[ad]then
Options[ad]:SetValue(ae.value)
end
end,
},
Dropdown={
Save=function(ad,ae)
return{type='Dropdown',idx=ad,value=ae.Value,mutli=ae.Multi}
end,
Load=function(ad,ae)
if Options[ad]then
Options[ad]:SetValue(ae.value)
end
end,
},
ColorPicker={
Save=function(ad,ae)
return{type='ColorPicker',idx=ad,value=ae.Value:ToHex(),transparency=ae.Transparency}
end,
Load=function(ad,ae)
if Options[ad]then
Options[ad]:SetValueRGB(Color3.fromHex(ae.value),Options[ad].HasTransparency and ae.transparency or 0)
end
end,
},
KeyPicker={
Save=function(ad,ae)
return{type='KeyPicker',idx=ad,mode=ae.Mode,key=ae.Value}
end,
Load=function(ad,ae)
if Options[ad]then
Options[ad]:SetValue({ae.key,ae.mode})
end
end,
},

Input={
Save=function(ad,ae)
return{type='Input',idx=ad,text=ae.Value}
end,
Load=function(ad,ae)
if Options[ad]and type(ae.text)=='string'then
Options[ad]:SetValue(ae.text)
end
end,
},
}

function ac:SetIgnoreIndexes(ad)
for ae,af in next,ad do
self.Ignore[af]=true
end
end

function ac:SetFolder(ad)
self.Folder=ad;
self:BuildFolderTree()
end

function ac:Save(ad)
if(not ad)then
return false,'no config file is selected'
end

local ae=self.Folder..'/settings/'..ad..'.json'

local af={
objects={}
}

for ah,ai in next,Toggles do
if self.Ignore[ah]then continue end

table.insert(af.objects,self.Parser[ai.Type].Save(ah,ai))
end

for ah,ai in next,Options do
if not self.Parser[ai.Type]then continue end
if self.Ignore[ah]then continue end

table.insert(af.objects,self.Parser[ai.Type].Save(ah,ai))
end

local ah,ai=pcall(ab.JSONEncode,ab,af)
if not ah then
return false,'failed to encode data'
end

writefile(ae,ai)
return true
end

function ac:Load(ad)
if(not ad)then
return false,'no config file is selected'
end

local ae=self.Folder..'/settings/'..ad..'.json'
if not isfile(ae)then return false,'invalid file'end

local af,ah=pcall(ab.JSONDecode,ab,readfile(ae))
if not af then return false,'decode error'end

for ai,aj in next,ah.objects do
if self.Parser[aj.type]then
task.spawn(function()self.Parser[aj.type].Load(aj.idx,aj)end)
end
end

return true
end

function ac:IgnoreThemeSettings()
self:SetIgnoreIndexes({
"BackgroundColor","MainColor","AccentColor","OutlineColor","FontColor",
"ThemeManager_ThemeList",'ThemeManager_CustomThemeList','ThemeManager_CustomThemeName',
})
end

function ac:BuildFolderTree()
local ad={
self.Folder,
self.Folder..'/themes',
self.Folder..'/settings'
}

for ae=1,#ad do
local af=ad[ae]
if not isfolder(af)then
makefolder(af)
end
end
end

function ac:RefreshConfigList()
local ad=listfiles(self.Folder..'/settings')

local ae={}
for af=1,#ad do
local ah=ad[af]
if ah:sub(-5)=='.json'then


local ai=ah:find('.json',1,true)
local aj=ai

local ak=ah:sub(ai,ai)
while ak~='/'and ak~='\\'and ak~=''do
ai=ai-1
ak=ah:sub(ai,ai)
end

if ak=='/'or ak=='\\'then
table.insert(ae,ah:sub(ai+1,aj-1))
end
end
end

return ae
end

function ac:SetLibrary(ad)
self.Library=ad
end

function ac:LoadAutoloadConfig()
if isfile(self.Folder..'/settings/autoload.txt')then
local ad=readfile(self.Folder..'/settings/autoload.txt')

local ae,af=self:Load(ad)
if not ae and not SILENT then
return self.Library:Notify('Failed to load autoload config: '..af)
end
if not SILENT then
self.Library:Notify(string.format('Auto loaded config %q',ad))
end
end
end


function ac:BuildConfigSection(ad)
assert(self.Library,'Must set SaveManager.Library')

local ae=ad:AddRightGroupbox('Configuration')

ae:AddInput('SaveManager_ConfigName',{Text='Config name'})
ae:AddDropdown('SaveManager_ConfigList',{Text='Config list',Values=self:RefreshConfigList(),AllowNull=true})

ae:AddDivider()

ae:AddButton('Create config',function()
local af=Options.SaveManager_ConfigName.Value

if af:gsub(' ','')==''then
return self.Library:Notify('Invalid config name (empty)',2)
end

local ah,ai=self:Save(af)
if not ah then
return self.Library:Notify('Failed to save config: '..ai)
end

self.Library:Notify(string.format('Created config %q',af))

Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
Options.SaveManager_ConfigList:SetValue(nil)
end):AddButton('Load config',function()
local af=Options.SaveManager_ConfigList.Value

local ah,ai=self:Load(af)
if not ah then
return self.Library:Notify('Failed to load config: '..ai)
end

self.Library:Notify(string.format('Loaded config %q',af))
end)

ae:AddButton('Overwrite config',function()
local af=Options.SaveManager_ConfigList.Value

local ah,ai=self:Save(af)
if not ah then
return self.Library:Notify('Failed to overwrite config: '..ai)
end

self.Library:Notify(string.format('Overwrote config %q',af))
end)

ae:AddButton('Refresh list',function()
Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
Options.SaveManager_ConfigList:SetValue(nil)
end)

ae:AddButton('Set autoload',function()
local af=Options.SaveManager_ConfigList.Value
if(not af)then
return
end;
writefile(self.Folder..'/settings/autoload.txt',af)
ac.AutoloadLabel:SetText('Current autoload config: '..af)
self.Library:Notify(string.format('Set %q to auto load',af))
end):AddButton('Remove autoload',function()
local af=Options.SaveManager_ConfigList.Value
if(isfile(self.Folder..'/settings/autoload.txt'))then
delfile(self.Folder..'/settings/autoload.txt');
end;
ac.AutoloadLabel:SetText('Current autoload config: none')
self.Library:Notify("removed autoload");
end)

ac.AutoloadLabel=ae:AddLabel('Current autoload config: none',true)

if isfile(self.Folder..'/settings/autoload.txt')then
local af=readfile(self.Folder..'/settings/autoload.txt')
ac.AutoloadLabel:SetText('Current autoload config: '..af)
end

ac:SetIgnoreIndexes({'SaveManager_ConfigList','SaveManager_ConfigName'})
end

ac:BuildFolderTree()
end
getgenv().SaveManager=ac;
return ac end function a.c():typeof(aa())local ab=a.cache.c if not ab then ab={c=aa()}a.cache.c=ab end return ab.c end end do local function aa()




local ab=game:GetService("Players")
local ac=game:GetService("RunService")
local ad=game:GetService("ReplicatedStorage")
local ae=game:GetService("HttpService")

local af=ab.LocalPlayer

local ah=require(ad:WaitForChild("References"))
local ai=require(ah.PlayerScripts:WaitForChild("Secondary"):WaitForChild("TravelHandler"))




local aj=3
local ak=40
local al=40
local am="islandhop_config.json"
local an="https://raw.githubusercontent.com/bananasxyz/satanas/refs/heads/main/banging.lua"

local ao=false
local ap=ak




local aq={
"Mainland","Blizzard Island","Forest Island","Royal Island",
"Desert Island","Glacier Island","Mountain Island","Jungle Island",
"Lunar Islands","Volcano Island",
}

local b={
["Mainland"]=8,["Stable Island"]=1,
["Training Island"]=1,["Royal Island"]=1,
["Volcano Island"]=1,["Blizzard Island"]=1,
["Forest Island"]=1,["Desert Island"]=1,
["Glacier Island"]=1,["Mountain Island"]=1,
["Jungle Island"]=1,["Lunar Islands"]=1,
}

local c={
["Mainland"]={
CFrame.new(665.936,14.998,-201.439,0.160052,0.000000,0.987109,0.000000,1.000000,-0.000000,-0.987109,0.000000,0.160052),
CFrame.new(390.433,19.315,-318.522,0.433055,-0.000000,0.901368,0.000000,1.000000,0.000000,-0.901368,0.000000,0.433055),
CFrame.new(361.149,80.161,-536.547,0.999545,-0.000000,0.030152,0.000000,1.000000,-0.000000,-0.030152,0.000000,0.999545),
CFrame.new(321.848,21.772,-876.733,-0.212789,0.000000,0.977098,0.000000,1.000000,-0.000000,-0.977098,0.000000,-0.212789),
CFrame.new(217.005,104.998,-612.001,-0.733737,-0.000000,-0.679433,-0.000000,1.000000,-0.000000,0.679433,-0.000000,-0.733737),
CFrame.new(43.606,14.998,-61.387,-0.917488,0.000000,0.397764,0.000000,1.000000,-0.000000,-0.397764,-0.000000,-0.917488),
CFrame.new(433.122,33.563,39.955,0.999795,0.000000,-0.020224,-0.000000,1.000000,0.000000,0.020224,-0.000000,0.999795),
CFrame.new(582.080,22.779,150.222,-0.422344,-0.000000,-0.906435,0.000000,1.000000,-0.000000,0.906435,-0.000000,-0.422344),
CFrame.new(861.912,14.998,216.408,-0.386769,0.000000,0.922177,-0.000000,1.000000,-0.000000,-0.922177,-0.000000,-0.386769),
CFrame.new(737.339,14.998,581.619,-0.876427,0.000000,0.481535,0.000000,1.000000,0.000000,-0.481535,0.000000,-0.876427),
CFrame.new(508.405,14.998,816.679,-0.882457,0.000000,0.470393,0.000000,1.000000,-0.000000,-0.470393,-0.000000,-0.882457),
CFrame.new(-20.912,18.419,927.573,0.880280,-0.000000,0.474455,0.000000,1.000000,0.000000,-0.474455,-0.000000,0.880280),
CFrame.new(-380.807,35.123,678.163,0.915341,-0.000000,0.402679,0.000000,1.000000,0.000000,-0.402679,0.000000,0.915341),
CFrame.new(-252.048,39.135,804.587,-0.175896,-0.000000,-0.984409,-0.000000,1.000000,-0.000000,0.984409,0.000000,-0.175896),
CFrame.new(-156.075,133.629,601.806,0.794045,-0.000000,-0.607859,0.000000,1.000000,0.000000,0.607859,-0.000000,0.794045),
CFrame.new(-29.686,193.046,621.971,0.923170,0.000000,0.384391,-0.000000,1.000000,-0.000000,-0.384391,-0.000000,0.923170),
CFrame.new(30.838,122.484,340.240,-0.972864,-0.000000,-0.231379,-0.000000,1.000000,-0.000000,0.231379,-0.000000,-0.972864),
CFrame.new(-770.175,15.971,385.762,0.806249,-0.000000,0.591576,-0.000000,1.000000,0.000000,-0.591576,-0.000000,0.806249),
CFrame.new(-925.341,15.247,65.800,0.998214,-0.000000,-0.059738,0.000000,1.000000,0.000000,0.059738,-0.000000,0.998214),
CFrame.new(-876.237,15.426,-293.784,0.999537,0.000000,-0.030441,-0.000000,1.000000,0.000000,0.030441,-0.000000,0.999537),
CFrame.new(-418.191,14.998,-754.435,0.254495,-0.000000,0.967074,-0.000000,1.000000,0.000000,-0.967074,-0.000000,0.254495),
CFrame.new(-444.135,20.839,-376.701,-0.615930,0.000000,-0.787800,-0.000000,1.000000,0.000000,0.787800,0.000000,-0.615930),
CFrame.new(-483.923,82.639,-177.806,-0.471303,0.000000,0.881971,0.000000,1.000000,-0.000000,-0.881971,0.000000,-0.471303),
CFrame.new(-503.018,116.094,-60.103,-0.965255,0.000000,-0.261309,0.000000,1.000000,0.000000,0.261309,0.000000,-0.965255),
CFrame.new(-629.732,137.707,311.997,-0.199397,0.000000,-0.979919,0.000000,1.000000,0.000000,0.979919,0.000000,-0.199397)
},
["Blizzard Island"]={
CFrame.new(-382.042,92.995,-3522.306,0.460438,0.000000,-0.887692,0.000000,1.000000,0.000000,0.887692,-0.000000,0.460438),
CFrame.new(-758.201,61.008,-3531.670,0.276509,0.000000,0.961011,0.000000,1.000000,-0.000000,-0.961011,0.000000,0.276509),
CFrame.new(-1105.937,109.860,-3519.610,0.726535,-0.000000,-0.687130,-0.000000,1.000000,-0.000000,0.687130,0.000000,0.726535),
CFrame.new(-1257.574,62.161,-3631.305,-0.914364,-0.000000,-0.404892,-0.000000,1.000000,-0.000000,0.404892,-0.000000,-0.914364),
CFrame.new(-967.561,14.404,-3950.552,-0.997720,0.000000,0.067482,0.000000,1.000000,-0.000000,-0.067482,-0.000000,-0.997720),
CFrame.new(-728.238,14.404,-3776.091,-0.382731,0.000000,-0.923860,-0.000000,1.000000,0.000000,0.923860,0.000000,-0.382731),
CFrame.new(-706.962,60.998,-4103.288,-0.236992,0.000000,-0.971512,-0.000000,1.000000,0.000000,0.971512,0.000000,-0.236992),
CFrame.new(-425.512,44.028,-4108.528,-0.182664,-0.000000,-0.983175,-0.000000,1.000000,-0.000000,0.983175,-0.000000,-0.182664),
CFrame.new(-93.559,18.113,-3925.751,-0.259992,-0.000000,-0.965611,-0.000000,1.000000,-0.000000,0.965611,-0.000000,-0.259992),
CFrame.new(146.277,29.452,-3828.152,-0.754581,0.000000,-0.656207,-0.000000,1.000000,0.000000,0.656207,0.000000,-0.754581),
CFrame.new(513.539,35.080,-3722.744,-0.805225,-0.000000,-0.592970,-0.000000,1.000000,-0.000000,0.592970,-0.000000,-0.805225),
CFrame.new(689.042,37.192,-3576.572,-0.837222,-0.000000,-0.546863,-0.000000,1.000000,-0.000000,0.546863,-0.000000,-0.837222),
CFrame.new(869.162,22.305,-3565.883,0.161352,0.000000,-0.986897,-0.000000,1.000000,-0.000000,0.986897,0.000000,0.161352),
CFrame.new(1215.247,19.139,-3584.557,-0.917856,-0.000000,-0.396913,-0.000000,1.000000,-0.000000,0.396913,-0.000000,-0.917856),
CFrame.new(660.312,15.404,-3308.462,0.009147,-0.000000,0.999958,-0.000000,1.000000,0.000000,-0.999958,-0.000000,0.009147),
CFrame.new(178.478,14.537,-3340.854,0.082006,-0.000000,0.996632,-0.000000,1.000000,0.000000,-0.996632,-0.000000,0.082006),
CFrame.new(-165.832,92.998,-3478.214,0.793830,-0.000000,0.608140,0.000000,1.000000,0.000000,-0.608140,-0.000000,0.793830)
},
["Forest Island"]={
CFrame.new(-7472.860,28.764,4463.595,0.551560,0.000000,0.834135,0.000000,1.000000,-0.000000,-0.834135,0.000000,0.551560),
CFrame.new(-7535.876,32.325,4238.738,0.068780,0.000000,0.997632,0.000000,1.000000,-0.000000,-0.997632,0.000000,0.068780),
CFrame.new(-7545.947,37.404,4098.649,-0.198445,-0.000000,0.980112,-0.000000,1.000000,0.000000,-0.980112,0.000000,-0.198445),
CFrame.new(-7684.951,40.998,3959.780,0.147014,0.000000,0.989134,-0.000000,1.000000,-0.000000,-0.989134,0.000000,0.147014),
CFrame.new(-7848.366,45.537,4018.761,-0.623656,0.000000,0.781699,0.000000,1.000000,-0.000000,-0.781699,-0.000000,-0.623656),
CFrame.new(-7997.114,48.987,4179.415,0.798752,0.000000,0.601661,-0.000000,1.000000,-0.000000,-0.601661,0.000000,0.798752),
CFrame.new(-8099.335,34.141,3889.042,0.999798,-0.000000,0.020097,0.000000,1.000000,-0.000000,-0.020097,0.000000,0.999798),
CFrame.new(-8042.408,20.998,3701.553,0.999915,0.000000,0.013039,-0.000000,1.000000,0.000000,-0.013039,-0.000000,0.999915),
CFrame.new(-7689.577,51.595,3598.035,0.535114,0.000000,0.844780,0.000000,1.000000,-0.000000,-0.844780,0.000000,0.535114),
CFrame.new(-7575.774,17.483,3334.014,0.082241,0.000000,-0.996612,0.000000,1.000000,0.000000,0.996612,-0.000000,0.082241),
CFrame.new(-7764.240,20.998,3233.584,-0.684193,-0.000000,0.729301,-0.000000,1.000000,0.000000,-0.729301,-0.000000,-0.684193),
CFrame.new(-7949.454,24.971,3231.008,0.345593,-0.000000,0.938385,0.000000,1.000000,0.000000,-0.938385,-0.000000,0.345593),
CFrame.new(-7901.335,20.971,3084.170,0.578636,0.000000,-0.815586,-0.000000,1.000000,0.000000,0.815586,-0.000000,0.578636),
CFrame.new(-8060.973,24.998,2915.217,0.955658,0.000000,0.294479,0.000000,1.000000,-0.000000,-0.294479,0.000000,0.955658),
CFrame.new(-8177.794,16.998,2739.041,-0.257516,-0.000000,0.966274,0.000000,1.000000,0.000000,-0.966274,0.000000,-0.257516),
CFrame.new(-7852.313,17.017,2649.995,0.726514,0.000000,-0.687151,-0.000000,1.000000,0.000000,0.687151,0.000000,0.726514),
CFrame.new(-7552.338,18.441,2621.408,0.261372,-0.000000,-0.965238,0.000000,1.000000,-0.000000,0.965238,-0.000000,0.261372),
CFrame.new(-7572.040,23.101,2814.098,-0.796524,-0.000000,0.604607,0.000000,1.000000,0.000000,-0.604607,0.000000,-0.796524),
CFrame.new(-8304.572,18.548,3097.513,-0.745587,-0.000000,0.666408,0.000000,1.000000,0.000000,-0.666408,0.000000,-0.745587),
CFrame.new(-8208.253,24.460,3482.972,-0.837417,0.000000,-0.546565,0.000000,1.000000,0.000000,0.546565,0.000000,-0.837417),
CFrame.new(-8240.987,18.951,3839.143,-0.966695,0.000000,0.255932,0.000000,1.000000,-0.000000,-0.255932,-0.000000,-0.966695),
CFrame.new(-8305.184,19.335,4191.374,-0.999586,-0.000000,-0.028773,-0.000000,1.000000,-0.000000,0.028773,-0.000000,-0.999586),
CFrame.new(-8239.147,18.951,4445.735,-0.848891,0.000000,-0.528567,-0.000000,1.000000,0.000000,0.528567,0.000000,-0.848891),
CFrame.new(-7883.821,18.951,4732.082,0.055220,-0.000000,-0.998474,0.000000,1.000000,-0.000000,0.998474,-0.000000,0.055220),
CFrame.new(-7518.997,58.069,3746.335,-0.113768,0.000000,-0.993507,-0.000000,1.000000,0.000000,0.993507,0.000000,-0.113768),
CFrame.new(-7607.815,111.891,3781.745,-0.867879,0.000000,0.496775,-0.000000,1.000000,-0.000000,-0.496775,-0.000000,-0.867879)
},
["Royal Island"]={
CFrame.new(-72.123,81.097,-5076.466,-0.619706,-0.000000,0.784834,-0.000000,1.000000,0.000000,-0.784834,-0.000000,-0.619706),
CFrame.new(-209.632,55.156,-5209.898,-0.154395,0.000000,0.988009,0.000000,1.000000,-0.000000,-0.988009,0.000000,-0.154395),
CFrame.new(27.947,40.998,-5560.313,0.377881,0.000000,-0.925854,-0.000000,1.000000,-0.000000,0.925854,0.000000,0.377881),
CFrame.new(-424.829,74.496,-5903.549,-0.919306,0.000000,0.393543,0.000000,1.000000,0.000000,-0.393543,0.000000,-0.919306),
CFrame.new(-481.518,137.520,-6073.541,0.520545,-0.000000,0.853834,-0.000000,1.000000,0.000000,-0.853834,-0.000000,0.520545),
CFrame.new(-612.957,208.595,-6277.108,0.995154,0.000000,-0.098330,-0.000000,1.000000,-0.000000,0.098330,0.000000,0.995154),
CFrame.new(-640.990,275.037,-6525.042,0.491454,-0.000000,0.870903,0.000000,1.000000,0.000000,-0.870903,0.000000,0.491454),
CFrame.new(-789.431,284.302,-6500.426,0.986291,0.000000,0.165016,-0.000000,1.000000,0.000000,-0.165016,-0.000000,0.986291),
CFrame.new(-478.821,24.998,-6492.742,-0.855233,0.000000,0.518244,0.000000,1.000000,-0.000000,-0.518244,0.000000,-0.855233),
CFrame.new(98.136,57.357,-6496.738,0.462515,0.000000,-0.886611,0.000000,1.000000,0.000000,0.886611,-0.000000,0.462515),
CFrame.new(247.551,24.998,-6520.478,0.132216,-0.000000,-0.991221,0.000000,1.000000,-0.000000,0.991221,0.000000,0.132216),
CFrame.new(183.846,25.728,-6420.287,-0.964619,0.000000,0.263648,0.000000,1.000000,-0.000000,-0.263648,-0.000000,-0.964619),
CFrame.new(292.436,32.998,-6029.453,-0.997315,-0.000000,-0.073233,-0.000000,1.000000,-0.000000,0.073233,-0.000000,-0.997315),
CFrame.new(225.520,56.982,-5711.718,-0.793371,0.000000,-0.608738,0.000000,1.000000,0.000000,0.608738,0.000000,-0.793371),
CFrame.new(514.783,69.082,-5562.985,-0.765907,-0.000000,-0.642951,-0.000000,1.000000,-0.000000,0.642951,-0.000000,-0.765907),
CFrame.new(769.309,60.830,-5532.214,0.919211,0.000000,-0.393766,-0.000000,1.000000,-0.000000,0.393766,0.000000,0.919211),
CFrame.new(953.228,53.977,-5724.733,0.973086,0.000000,-0.230442,0.000000,1.000000,0.000000,0.230442,-0.000000,0.973086),
CFrame.new(1086.085,31.402,-6151.646,0.701825,-0.000000,-0.712349,0.000000,1.000000,-0.000000,0.712349,0.000000,0.701825)
},
["Desert Island"]={
CFrame.new(1057.257,64.998,3448.428,0.089481,0.000000,0.995988,-0.000000,1.000000,-0.000000,-0.995988,-0.000000,0.089481),
CFrame.new(1132.570,15.370,3667.140,-0.879080,0.000000,-0.476674,-0.000000,1.000000,0.000000,0.476674,0.000000,-0.879080),
CFrame.new(1375.584,16.400,3794.933,-0.437227,0.000000,-0.899351,-0.000000,1.000000,0.000000,0.899351,0.000000,-0.437227),
CFrame.new(1480.055,31.172,4003.031,-0.452688,0.000000,0.891669,-0.000000,1.000000,-0.000000,-0.891669,-0.000000,-0.452688),
CFrame.new(1280.563,125.093,3976.117,0.833706,-0.000000,0.552209,0.000000,1.000000,0.000000,-0.552209,-0.000000,0.833706),
CFrame.new(1072.918,124.989,3931.746,-0.265551,0.000000,0.964097,-0.000000,1.000000,-0.000000,-0.964097,-0.000000,-0.265551),
CFrame.new(803.771,125.089,3962.484,-0.258191,-0.000000,0.966094,-0.000000,1.000000,0.000000,-0.966094,0.000000,-0.258191),
CFrame.new(561.074,125.183,4054.734,-0.380536,-0.000000,0.924766,-0.000000,1.000000,0.000000,-0.924766,0.000000,-0.380536),
CFrame.new(547.330,17.074,4279.596,0.902964,-0.000000,0.429716,0.000000,1.000000,0.000000,-0.429716,0.000000,0.902964),
CFrame.new(444.411,15.404,4438.446,-0.999448,0.000000,-0.033216,0.000000,1.000000,0.000000,0.033216,0.000000,-0.999448),
CFrame.new(581.363,45.373,4506.168,0.811399,0.000000,0.584493,-0.000000,1.000000,-0.000000,-0.584493,0.000000,0.811399),
CFrame.new(687.038,124.998,4353.153,0.099777,-0.000000,-0.995010,0.000000,1.000000,-0.000000,0.995010,-0.000000,0.099777),
CFrame.new(966.561,125.118,4342.621,0.264482,-0.000000,-0.964391,-0.000000,1.000000,-0.000000,0.964391,0.000000,0.264482),
CFrame.new(1344.608,103.504,4317.035,-0.869061,0.000000,-0.494705,-0.000000,1.000000,0.000000,0.494705,0.000000,-0.869061),
CFrame.new(1319.893,15.404,4539.259,-0.884852,0.000000,0.465871,0.000000,1.000000,-0.000000,-0.465871,-0.000000,-0.884852),
CFrame.new(1484.599,15.404,4258.654,0.942722,-0.000000,0.333579,0.000000,1.000000,-0.000000,-0.333579,0.000000,0.942722),
CFrame.new(1720.102,63.150,4254.261,-0.034687,-0.000000,-0.999398,-0.000000,1.000000,-0.000000,0.999398,0.000000,-0.034687),
CFrame.new(1004.585,15.431,4625.405,-0.068486,-0.000000,0.997652,0.000000,1.000000,0.000000,-0.997652,0.000000,-0.068486),
CFrame.new(114.073,38.638,4555.410,0.017780,0.002914,0.999838,0.004563,0.999985,-0.002995,-0.999831,0.004616,0.017766),
CFrame.new(-84.351,15.536,4522.342,0.281574,0.000000,0.959540,-0.000000,1.000000,-0.000000,-0.959540,0.000000,0.281574),
CFrame.new(-211.976,77.077,4458.471,0.883683,-0.000000,0.468085,0.000000,1.000000,-0.000000,-0.468085,0.000000,0.883683),
CFrame.new(-425.070,24.569,4420.184,0.493990,0.000000,0.869468,-0.000000,1.000000,-0.000000,-0.869468,-0.000000,0.493990),
CFrame.new(-761.432,17.434,4420.783,0.303166,-0.000000,0.952938,0.000000,1.000000,0.000000,-0.952938,0.000000,0.303166),
CFrame.new(-1081.539,34.009,4359.438,-0.113532,-0.000000,0.993534,-0.000000,1.000000,0.000000,-0.993534,-0.000000,-0.11353),
CFrame.new(-925.370,16.098,4174.521,0.317470,0.000000,-0.948268,0.000000,1.000000,0.000000,0.948268,-0.000000,0.317470),
CFrame.new(-925.370,16.098,4174.521,0.317470,0.000000,-0.948268,0.000000,1.000000,0.000000,0.948268,-0.000000,0.317470),
CFrame.new(-448.702,74.692,3720.310,0.717594,0.000000,-0.696462,-0.000000,1.000000,0.000000,0.696462,0.000000,0.717594),
CFrame.new(-210.401,69.401,3578.062,0.320887,-0.000000,-0.947117,0.000000,1.000000,-0.000000,0.947117,0.000000,0.320887),
CFrame.new(172.042,65.213,3368.040,0.334159,-0.000000,-0.942517,-0.000000,1.000000,-0.000000,0.942517,0.000000,0.334159),
CFrame.new(406.814,47.485,3346.744,-0.148952,-0.000000,-0.988844,0.000000,1.000000,-0.000000,0.988844,-0.000000,-0.148952),
CFrame.new(325.440,15.404,3600.470,-0.519839,0.000000,0.854264,0.000000,1.000000,-0.000000,-0.854264,-0.000000,-0.519839),
CFrame.new(85.659,15.404,3927.667,-0.657984,-0.000000,0.753032,-0.000000,1.000000,0.000000,-0.753032,0.000000,-0.657984),
CFrame.new(-59.720,15.404,4080.630,-0.745152,-0.000000,0.666895,-0.000000,1.000000,0.000000,-0.666895,-0.000000,-0.745152),
CFrame.new(-214.535,15.404,4034.627,-0.180478,-0.000000,0.983579,0.000000,1.000000,0.000000,-0.983579,0.000000,-0.180478),
CFrame.new(-361.872,17.675,4156.984,-0.662197,-0.000000,0.749330,-0.000000,1.000000,0.000000,-0.749330,0.000000,-0.662197),
CFrame.new(-189.597,15.404,4205.576,0.962794,0.000000,0.270236,-0.000000,1.000000,-0.000000,-0.270236,0.000000,0.962794),
CFrame.new(62.058,15.404,4269.560,0.020223,0.000000,-0.999795,0.000000,1.000000,0.000000,0.999795,-0.000000,0.020223),
CFrame.new(476.299,15.404,3826.276,0.890786,-0.000000,-0.454423,-0.000000,1.000000,-0.000000,0.454423,0.000000,0.890786)
},
["Glacier Island"]={
CFrame.new(2400.878,-10.504,-3.798,-0.293389,0.000000,0.955993,-0.000000,1.000000,-0.000000,-0.955993,-0.000000,-0.293389),
CFrame.new(2197.143,25.029,51.597,-0.895651,-0.000000,0.444758,-0.000000,1.000000,-0.000000,-0.444758,-0.000000,-0.895651),
CFrame.new(2993.914,-7.018,89.664,0.920103,0.000000,0.391677,-0.000000,1.000000,0.000000,-0.391677,-0.000000,0.920103),
CFrame.new(3254.097,-7.002,78.091,0.039642,0.000000,-0.999214,-0.000000,1.000000,0.000000,0.999214,-0.000000,0.039642),
CFrame.new(3527.007,-7.002,77.846,-0.293532,-0.000000,-0.955949,0.000000,1.000000,-0.000000,0.955949,-0.000000,-0.293532),
CFrame.new(3183.226,82.592,-231.937,-0.819137,0.000000,0.573598,0.000000,1.000000,-0.000000,-0.573598,0.000000,-0.819137),
CFrame.new(3272.057,82.592,-113.594,-0.837187,-0.000000,-0.546917,0.000000,1.000000,-0.000000,0.546917,-0.000000,-0.837187),
CFrame.new(3364.805,82.592,-71.182,0.110966,-0.000000,-0.993824,-0.000000,1.000000,-0.000000,0.993824,0.000000,0.110966),
CFrame.new(3132.977,82.592,-128.275,0.043340,-0.000000,0.999060,-0.000000,1.000000,0.000000,-0.999060,-0.000000,0.043340),
CFrame.new(3198.475,116.948,-412.867,0.273361,-0.000000,0.961911,-0.000000,1.000000,0.000000,-0.961911,-0.000000,0.273361),
CFrame.new(2849.397,116.998,-367.796,-0.000623,-0.000000,1.000000,0.000000,1.000000,0.000000,-1.000000,0.000000,-0.000623),
CFrame.new(2618.713,116.389,-379.632,0.715373,0.000000,0.698743,-0.000000,1.000000,-0.000000,-0.698743,-0.000000,0.715373),
CFrame.new(2563.353,116.389,-728.223,0.861281,-0.000000,-0.508129,0.000000,1.000000,-0.000000,0.508129,0.000000,0.861281),
CFrame.new(2389.108,116.998,-475.673,-0.162055,-0.000000,0.986782,0.000000,1.000000,0.000000,-0.986782,0.000000,-0.162055),
CFrame.new(2292.616,116.998,-542.623,0.897960,-0.000000,0.440077,-0.000000,1.000000,0.000000,-0.440077,-0.000000,0.897960),
CFrame.new(2183.349,166.967,-405.284,-0.527489,-0.000000,0.849562,0.000000,1.000000,0.000000,-0.849562,0.000000,-0.527489),
CFrame.new(2053.532,173.773,-469.023,-0.073158,-0.000000,0.997320,0.000000,1.000000,0.000000,-0.997320,0.000000,-0.073158),
CFrame.new(2197.752,186.264,-596.579,0.316232,0.000000,-0.948682,0.000000,1.000000,0.000000,0.948682,-0.000000,0.316232),
CFrame.new(2416.479,186.264,-599.388,0.075444,-0.000000,-0.997150,0.000000,1.000000,-0.000000,0.997150,0.000000,0.075444),
CFrame.new(2702.472,193.545,-634.575,-0.066647,0.000000,-0.997777,-0.000000,1.000000,0.000000,0.997777,0.000000,-0.066647),
CFrame.new(2692.565,193.534,-766.698,0.796939,-0.000000,-0.604060,-0.000000,1.000000,-0.000000,0.604060,0.000000,0.796939),
CFrame.new(2768.410,201.882,-1040.510,0.999988,-0.000000,0.004846,0.000000,1.000000,0.000000,-0.004846,-0.000000,0.999988),
CFrame.new(2815.325,200.576,-993.692,-0.470508,0.000000,-0.882396,0.000000,1.000000,0.000000,0.882396,0.000000,-0.470508),
CFrame.new(2662.499,224.998,-1049.144,-0.962307,0.000000,0.271965,0.000000,1.000000,-0.000000,-0.271965,-0.000000,-0.962307),
CFrame.new(2556.227,255.216,-915.980,0.495420,0.000000,0.868654,0.000000,1.000000,-0.000000,-0.868654,0.000000,0.495420),
CFrame.new(2487.840,255.217,-927.118,-0.403209,0.000000,0.915108,0.000000,1.000000,-0.000000,-0.915108,-0.000000,-0.403209),
CFrame.new(2425.859,275.749,-1105.462,0.892337,-0.000000,0.451369,0.000000,1.000000,-0.000000,-0.451369,0.000000,0.892337),
CFrame.new(2358.345,280.232,-1206.390,0.825089,0.000000,0.565002,0.000000,1.000000,-0.000000,-0.565002,0.000000,0.825089),
CFrame.new(2351.815,256.992,-887.120,-0.794428,-0.000000,0.607358,0.000000,1.000000,0.000000,-0.607358,0.000000,-0.794428),
CFrame.new(2135.493,255.326,-878.545,-0.188005,0.000000,0.982168,-0.000000,1.000000,-0.000000,-0.982168,-0.000000,-0.188005),
CFrame.new(2216.487,316.845,-1213.264,-0.910133,0.000000,-0.414316,0.000000,1.000000,0.000000,0.414316,0.000000,-0.910133),
CFrame.new(2070.611,360.757,-1132.188,-0.641953,0.000000,-0.766744,0.000000,1.000000,0.000000,0.766744,0.000000,-0.641953),
CFrame.new(2073.024,329.226,-1267.795,-0.401489,-0.000000,-0.915864,-0.000000,1.000000,-0.000000,0.915864,0.000000,-0.401489),
CFrame.new(3052.000,146.935,-1236.844,0.061776,-0.000000,-0.998090,0.000000,1.000000,-0.000000,0.998090,-0.000000,0.061776),
CFrame.new(3003.948,148.815,-1124.271,-0.980462,0.000000,-0.196708,0.000000,1.000000,-0.000000,0.196708,-0.000000,-0.980462),
CFrame.new(3216.661,146.842,-1222.549,0.482906,0.000000,-0.875672,-0.000000,1.000000,0.000000,0.875672,0.000000,0.482906),
CFrame.new(3172.916,146.842,-997.427,-0.904359,0.000000,0.426773,-0.000000,1.000000,-0.000000,-0.426773,-0.000000,-0.904359),
CFrame.new(3027.217,154.722,-680.357,-0.751371,0.000000,0.659880,-0.000000,1.000000,-0.000000,-0.659880,-0.000000,-0.751371),
CFrame.new(2987.953,147.525,-861.088,0.656116,0.000000,-0.754660,0.000000,1.000000,0.000000,0.754660,-0.000000,0.656116),
CFrame.new(3392.629,147.122,-737.173,-0.744118,-0.000000,0.668049,-0.000000,1.000000,-0.000000,-0.668049,-0.000000,-0.744118),
CFrame.new(3160.088,147.395,-597.373,-0.905875,0.000000,-0.423546,0.000000,1.000000,0.000000,0.423546,-0.000000,-0.905875),
CFrame.new(3007.762,28.994,-757.581,-0.616086,-0.000000,0.787679,0.000000,1.000000,0.000000,-0.787679,0.000000,-0.616086),
CFrame.new(2821.434,31.060,-722.676,-0.161401,0.000000,-0.986889,-0.000000,1.000000,0.000000,0.986889,0.000000,-0.161401),
CFrame.new(2885.220,27.186,-620.042,0.929208,-0.000000,0.369558,0.000000,1.000000,0.000000,-0.369558,-0.000000,0.929208),
CFrame.new(3200.757,-7.745,-755.416,-0.109131,-0.000000,0.994027,-0.000000,1.000000,0.000000,-0.994027,-0.000000,-0.109131),
CFrame.new(3150.148,-7.018,-989.735,0.789689,0.000000,0.613507,-0.000000,1.000000,0.000000,-0.613507,-0.000000,0.789689),
CFrame.new(2523.225,-7.002,-503.811,-0.941150,0.000000,0.337988,0.000000,1.000000,0.000000,-0.337988,0.000000,-0.941150),
CFrame.new(2536.531,-7.002,-396.331,-0.998193,-0.000000,-0.060086,-0.000000,1.000000,0.000000,0.060086,0.000000,-0.998193),
CFrame.new(2378.865,-7.018,-490.483,-0.807392,0.000000,0.590016,0.000000,1.000000,-0.000000,-0.590016,-0.000000,-0.807392)
},
["Mountain Island"]={
CFrame.new(-7214.773,32.530,287.288,0.491617,0.000000,-0.870812,-0.000000,1.000000,0.000000,0.870812,-0.000000,0.491617),
CFrame.new(-7006.402,126.644,165.479,0.217954,-0.000000,-0.975959,-0.000000,1.000000,-0.000000,0.975959,0.000000,0.217954),
CFrame.new(-6841.190,165.338,185.724,0.867930,-0.000000,0.496686,-0.000000,1.000000,0.000000,-0.496686,-0.000000,0.867930),
CFrame.new(-6797.834,228.663,62.782,0.984083,-0.000000,0.177709,0.000000,1.000000,-0.000000,-0.177709,0.000000,0.984083),
CFrame.new(-6774.866,276.932,-98.017,0.214142,0.000000,-0.976802,-0.000000,1.000000,-0.000000,0.976802,0.000000,0.214142),
CFrame.new(-6607.875,372.998,-286.469,-0.661203,0.000000,0.750207,0.000000,1.000000,0.000000,-0.750207,0.000000,-0.661203),
CFrame.new(-6666.319,320.369,-264.341,0.773879,0.000000,-0.633334,-0.000000,1.000000,-0.000000,0.633334,0.000000,0.773879),
CFrame.new(-6524.053,276.998,-69.672,0.499362,0.000000,-0.866393,-0.000000,1.000000,0.000000,0.866393,-0.000000,0.499362),
CFrame.new(-6669.441,224.874,70.515,-0.119839,-0.000000,0.992793,-0.000000,1.000000,0.000000,-0.992793,-0.000000,-0.119839),
CFrame.new(-6757.290,9.018,415.531,0.980146,-0.000000,0.198276,0.000000,1.000000,0.000000,-0.198276,-0.000000,0.980146),
CFrame.new(-6487.353,46.925,288.247,0.512483,-0.000000,-0.858697,0.000000,1.000000,-0.000000,0.858697,0.000000,0.512483),
CFrame.new(-6236.704,32.997,149.049,0.930303,-0.000000,-0.366792,0.000000,1.000000,-0.000000,0.366792,0.000000,0.930303),
CFrame.new(-5920.666,32.998,18.426,-0.061217,-0.000000,-0.998124,-0.000000,1.000000,-0.000000,0.998124,-0.000000,-0.061217),
CFrame.new(-5488.082,45.074,-88.013,0.799946,-0.000000,-0.600072,0.000000,1.000000,-0.000000,0.600072,0.000000,0.799946),
CFrame.new(-5615.074,93.555,-178.742,0.565122,-0.000000,-0.825007,-0.000000,1.000000,-0.000000,0.825007,0.000000,0.565122),
CFrame.new(-5720.867,137.505,-170.928,-0.990899,0.000000,0.134610,0.000000,1.000000,-0.000000,-0.134610,-0.000000,-0.990899),
CFrame.new(-6008.147,170.706,-124.588,-0.904283,0.000000,0.426933,0.000000,1.000000,0.000000,-0.426933,0.000000,-0.904283),
CFrame.new(-5866.254,188.998,-324.966,0.671534,-0.000000,-0.740974,0.000000,1.000000,-0.000000,0.740974,0.000000,0.671534),
CFrame.new(-6095.032,196.998,-396.765,-0.112491,-0.000000,0.993653,-0.000000,1.000000,0.000000,-0.993653,-0.000000,-0.112491),
CFrame.new(-6151.569,220.829,-269.242,-0.998729,0.000000,-0.050399,0.000000,1.000000,0.000000,0.050399,0.000000,-0.998729),
CFrame.new(-6252.735,169.552,-517.140,0.560569,-0.000000,0.828108,0.000000,1.000000,0.000000,-0.828108,-0.000000,0.560569),
CFrame.new(-6603.163,12.105,-890.028,0.627388,-0.000000,0.778707,0.000000,1.000000,0.000000,-0.778707,-0.000000,0.627388),
CFrame.new(-6878.203,13.671,-1044.889,0.525436,-0.000000,0.850833,0.000000,1.000000,0.000000,-0.850833,-0.000000,0.525436),
CFrame.new(-7200.269,109.276,-1177.566,0.067147,-0.000000,0.997743,0.000000,1.000000,0.000000,-0.997743,-0.000000,0.067147),
CFrame.new(-7228.098,147.970,-1137.700,-0.723252,0.000057,0.690584,0.000005,1.000000,-0.000077,-0.690584,-0.000052,-0.723252),
CFrame.new(-7332.619,105.113,-1024.005,-0.723782,0.000000,-0.690029,0.000000,1.000000,0.000000,0.690029,0.000000,-0.723782),
CFrame.new(-7128.717,127.300,-971.255,-0.120213,0.000000,-0.992748,0.000000,1.000000,0.000000,0.992748,-0.000000,-0.120213),
CFrame.new(-7073.085,124.991,-733.655,0.177600,-0.000000,0.984103,0.000000,1.000000,0.000000,-0.984103,-0.000000,0.177600),
CFrame.new(-7255.609,113.053,-641.237,-0.558652,-0.000000,0.829402,0.000000,1.000000,0.000000,-0.829402,0.000000,-0.558652),
CFrame.new(-7478.791,13.166,-746.285,-0.017462,0.000000,0.999848,-0.000000,1.000000,-0.000000,-0.999848,-0.000000,-0.017462),
CFrame.new(-7391.350,33.294,-1053.469,0.910577,-0.000000,0.413340,0.000000,1.000000,-0.000000,-0.413340,0.000000,0.910577),
CFrame.new(-7555.564,19.813,-452.911,-0.941290,-0.000000,-0.337600,-0.000000,1.000000,-0.000000,0.337600,-0.000000,-0.941290),
CFrame.new(-7409.248,55.013,-427.185,0.999950,-0.000000,-0.010043,0.000000,1.000000,-0.000000,0.010043,0.000000,0.999950),
CFrame.new(-7095.168,176.324,-455.400,0.885353,-0.000000,0.464919,0.000000,1.000000,0.000000,-0.464919,-0.000000,0.885353),
CFrame.new(-7163.932,240.998,-684.626,-0.427335,-0.000000,-0.904093,-0.000000,1.000000,-0.000000,0.904093,-0.000000,-0.427335),
CFrame.new(-7154.404,217.219,-930.967,0.688617,-0.000000,-0.725125,-0.000000,1.000000,-0.000000,0.725125,0.000000,0.688617),
CFrame.new(-7256.463,200.997,-1074.188,0.712506,0.000000,0.701667,-0.000000,1.000000,-0.000000,-0.701667,0.000000,0.712506),
CFrame.new(-7039.147,272.293,-690.777,-0.928318,0.000000,-0.371788,-0.000000,1.000000,0.000000,0.371788,0.000000,-0.928318),
CFrame.new(-6896.775,265.244,-379.438,-0.907239,-0.000000,-0.420616,0.000000,1.000000,-0.000000,0.420616,-0.000000,-0.907239),
CFrame.new(-6858.708,263.932,-253.161,-0.963729,0.000000,-0.266883,-0.000000,1.000000,0.000000,0.266883,0.000000,-0.963729)
},
["Jungle Island"]={
CFrame.new(3242.791,18.754,2630.970,-0.903268,0.000000,-0.429076,0.000000,1.000000,0.000000,0.429076,0.000000,-0.903268),
CFrame.new(3498.283,46.975,2810.441,-0.892794,0.000000,0.450466,0.000000,1.000000,-0.000000,-0.450466,0.000000,-0.892794),
CFrame.new(3649.556,17.309,2995.035,-0.748639,-0.000000,-0.662978,0.000000,1.000000,-0.000000,0.662978,-0.000000,-0.748639),
CFrame.new(3544.737,46.949,3248.570,-0.991336,0.000000,-0.131350,0.000000,1.000000,-0.000000,0.131350,-0.000000,-0.991336),
CFrame.new(3316.892,45.557,3499.239,-0.796393,-0.000000,0.604780,-0.000000,1.000000,0.000000,-0.604780,0.000000,-0.796393),
CFrame.new(2939.932,68.979,3562.033,-0.556870,0.000000,0.830600,0.000000,1.000000,-0.000000,-0.830600,-0.000000,-0.556870),
CFrame.new(2937.843,28.430,3325.126,0.971778,-0.000000,-0.235897,0.000000,1.000000,0.000000,0.235897,-0.000000,0.971778),
CFrame.new(3700.291,43.296,3469.382,-0.582111,0.000000,-0.813109,0.000000,1.000000,0.000000,0.813109,0.000000,-0.582111),
CFrame.new(3812.564,18.167,3650.845,-0.950636,0.000000,-0.310308,0.000000,1.000000,0.000000,0.310308,0.000000,-0.950636),
CFrame.new(3936.433,18.818,3570.950,0.107361,0.000000,-0.994220,0.000000,1.000000,0.000000,0.994220,-0.000000,0.107361),
CFrame.new(3936.433,18.818,3570.950,0.107361,0.000000,-0.994220,0.000000,1.000000,0.000000,0.994220,-0.000000,0.107361),
CFrame.new(3715.697,15.403,4417.625,-0.930706,0.000000,-0.365767,0.000000,1.000000,0.000000,0.365767,0.000000,-0.930706),
CFrame.new(3980.756,15.404,4302.282,-0.026813,-0.000000,-0.999640,-0.000000,1.000000,-0.000000,0.999640,0.000000,-0.026813),
CFrame.new(4201.835,120.998,4027.481,0.988359,-0.000000,-0.152141,-0.000000,1.000000,-0.000000,0.152141,0.000000,0.988359),
CFrame.new(4253.639,120.998,3701.066,0.973689,-0.000000,-0.227881,-0.000000,1.000000,-0.000000,0.227881,0.000000,0.973689),
CFrame.new(4250.476,120.998,3375.971,0.998921,0.000000,0.046446,-0.000000,1.000000,-0.000000,-0.046446,0.000000,0.998921),
CFrame.new(4243.121,15.577,2533.673,0.850525,-0.000000,-0.525935,0.000000,1.000000,-0.000000,0.525935,0.000000,0.850525),
CFrame.new(4300.399,15.404,2449.880,0.297536,-0.000000,-0.954711,0.000000,1.000000,-0.000000,0.954711,-0.000000,0.297536),
CFrame.new(3826.826,38.406,2469.223,0.534162,-0.000000,0.845382,0.000000,1.000000,0.000000,-0.845382,0.000000,0.534162),
CFrame.new(3734.817,21.552,2657.648,-0.968356,-0.000000,0.249575,0.000000,1.000000,0.000000,-0.249575,0.000000,-0.968356),
CFrame.new(3997.381,120.998,2186.975,0.999624,-0.000000,-0.027403,0.000000,1.000000,-0.000000,0.027403,0.000000,0.999624),
CFrame.new(3957.398,127.028,1994.268,0.370745,-0.000000,0.928735,-0.000000,1.000000,0.000000,-0.928735,-0.000000,0.370745),
CFrame.new(3582.243,45.969,2037.649,0.970824,0.000000,0.239791,-0.000000,1.000000,-0.000000,-0.239791,0.000000,0.970824),
CFrame.new(4194.147,15.404,1539.364,-0.514991,-0.000000,0.857196,0.000000,1.000000,0.000000,-0.857196,0.000000,-0.514991),
CFrame.new(4145.227,15.404,1209.603,0.930009,-0.000000,0.367536,-0.000000,1.000000,0.000000,-0.367536,-0.000000,0.930009),
CFrame.new(3646.035,120.998,1108.057,0.073999,-0.000000,0.997258,-0.000000,1.000000,0.000000,-0.997258,-0.000000,0.073999),
CFrame.new(3560.289,132.848,707.446,0.977682,-0.000000,0.210090,-0.000000,1.000000,0.000000,-0.210090,-0.000000,0.977682),
CFrame.new(3380.594,120.998,434.143,0.623593,-0.000000,0.781749,-0.000000,1.000000,0.000000,-0.781749,-0.000000,0.623593),
CFrame.new(3283.650,15.404,902.778,0.945511,-0.000000,0.325591,0.000000,1.000000,0.000000,-0.325591,0.000000,0.945511),
CFrame.new(3169.369,36.436,1101.109,-0.406628,-0.000000,0.913594,0.000000,1.000000,0.000000,-0.913594,0.000000,-0.406628),
CFrame.new(3185.811,73.948,1215.060,-0.962469,0.000000,-0.271392,0.000000,1.000000,-0.000000,0.271392,-0.000000,-0.962469),
CFrame.new(3179.302,86.346,1346.582,0.643434,0.000000,0.765502,-0.000000,1.000000,0.000000,-0.765502,-0.000000,0.643434),
CFrame.new(3329.037,29.141,1537.636,-0.916502,-0.000000,0.400030,0.000000,1.000000,0.000000,-0.400030,0.000000,-0.916502),
CFrame.new(3527.567,33.152,1496.425,-0.917907,0.000000,-0.396795,0.000000,1.000000,-0.000000,0.396795,-0.000000,-0.917907),
CFrame.new(3447.305,16.998,1782.471,-0.830702,0.000000,0.556717,0.000000,1.000000,-0.000000,-0.556717,-0.000000,-0.830702),
CFrame.new(2908.122,66.767,2025.434,-0.258916,-0.000000,0.965900,-0.000000,1.000000,0.000000,-0.965900,-0.000000,-0.258916)
},
["Lunar Islands"]={
CFrame.new(-3848.084,13.858,-2062.724,0.954432,-0.000000,0.298429,-0.000000,1.000000,0.000000,-0.298429,-0.000000,0.954432),
CFrame.new(-3615.102,29.024,-2338.882,0.619280,0.000000,-0.785170,0.000000,1.000000,0.000000,0.785170,-0.000000,0.619280),
CFrame.new(-3615.102,28.968,-2338.882,0.619280,-0.000000,-0.785170,-0.000000,1.000000,-0.000000,0.785170,0.000000,0.619280),
CFrame.new(-3325.056,15.404,-2555.110,0.962541,0.000000,0.271135,-0.000000,1.000000,-0.000000,-0.271135,0.000000,0.962541),
CFrame.new(-2988.803,129.599,-2990.796,0.638774,0.000000,-0.769394,0.000000,1.000000,0.000000,0.769394,-0.000000,0.638774),
CFrame.new(-2895.019,204.949,-2927.568,-0.737933,0.000000,-0.674874,-0.000000,1.000000,0.000000,0.674874,0.000000,-0.737933),
CFrame.new(-2527.140,180.883,-3042.093,0.246609,0.000000,-0.969115,0.000000,1.000000,0.000000,0.969115,-0.000000,0.246609),
CFrame.new(-2600.961,185.327,-3373.617,0.859072,0.000000,0.511855,-0.000000,1.000000,0.000000,-0.511855,-0.000000,0.859072),
CFrame.new(-2750.094,63.254,-3574.267,-0.692174,0.000000,0.721731,-0.000000,1.000000,-0.000000,-0.721731,-0.000000,-0.692174),
CFrame.new(-2836.744,15.404,-3682.515,0.519055,0.000000,-0.854741,0.000000,1.000000,0.000000,0.854741,-0.000000,0.519055),
CFrame.new(-3159.135,55.030,-3768.750,-0.752918,0.000000,0.658115,0.000000,1.000000,-0.000000,-0.658115,-0.000000,-0.752918),
CFrame.new(-3259.411,129.574,-4054.177,0.993008,0.000000,0.118048,-0.000000,1.000000,-0.000000,-0.118048,0.000000,0.993008),
CFrame.new(-3402.317,127.905,-4239.881,0.625049,0.000000,0.780585,-0.000000,1.000000,-0.000000,-0.780585,-0.000000,0.625049),
CFrame.new(-3688.774,146.333,-4278.259,-0.041952,-0.000000,0.999120,0.000000,1.000000,0.000000,-0.999120,0.000000,-0.041952),
CFrame.new(-3627.743,147.442,-3693.322,-0.368103,0.000000,-0.929785,-0.000000,1.000000,0.000000,0.929785,0.000000,-0.368103),
CFrame.new(-3535.550,12.240,-4048.549,0.901751,-0.000000,-0.432257,0.000000,1.000000,-0.000000,0.432257,0.000000,0.901751),
CFrame.new(-3386.680,149.475,-3687.410,0.574820,0.000000,-0.818280,0.000000,1.000000,0.000000,0.818280,-0.000000,0.574820),
CFrame.new(-3391.785,20.899,-3454.754,0.169722,0.000000,0.985492,-0.000000,1.000000,-0.000000,-0.985492,-0.000000,0.169722),
CFrame.new(-3148.866,15.275,-3034.570,0.948468,0.000000,-0.316873,-0.000000,1.000000,-0.000000,0.316873,0.000000,0.948468),
CFrame.new(-3325.533,25.653,-2425.765,-0.823623,0.000000,-0.567137,-0.000000,1.000000,0.000000,0.567137,0.000000,-0.823623),
CFrame.new(-3553.793,17.142,-1642.600,-0.912142,0.000000,-0.409875,-0.000000,1.000000,0.000000,0.409875,0.000000,-0.912142),
CFrame.new(-3397.513,27.252,-1431.277,0.993105,-0.000000,0.117232,0.000000,1.000000,-0.000000,-0.117232,0.000000,0.993105),
CFrame.new(-3292.648,35.424,-893.182,-0.600277,0.000000,-0.799792,0.000000,1.000000,0.000000,0.799792,-0.000000,-0.600277),
CFrame.new(-3026.764,24.863,-790.847,-0.250915,0.000000,-0.968009,0.000000,1.000000,0.000000,0.968009,-0.000000,-0.250915),
CFrame.new(-2707.063,56.112,-745.312,-0.129162,-0.000000,-0.991623,0.000000,1.000000,-0.000000,0.991623,-0.000000,-0.129162),
CFrame.new(-2345.119,106.512,-931.376,-0.187430,0.000000,-0.982278,0.000000,1.000000,0.000000,0.982278,-0.000000,-0.187430),
CFrame.new(-2167.360,134.201,-1231.910,0.912997,0.000000,-0.407966,-0.000000,1.000000,-0.000000,0.407966,0.000000,0.912997),
CFrame.new(-2071.372,226.236,-1517.424,0.969284,0.000000,-0.245943,-0.000000,1.000000,-0.000000,0.245943,0.000000,0.969284),
CFrame.new(-2105.362,228.242,-1751.023,0.995166,-0.000000,-0.098209,0.000000,1.000000,-0.000000,0.098209,0.000000,0.995166),
CFrame.new(-2308.253,64.788,-1936.170,0.972440,0.000000,-0.233152,-0.000000,1.000000,0.000000,0.233152,-0.000000,0.972440),
CFrame.new(-2268.345,106.998,-1341.150,-0.481596,0.000000,-0.876393,0.000000,1.000000,0.000000,0.876393,-0.000000,-0.481596)
},
["Volcano Island"]={
CFrame.new(2730.048,29.027,-7504.669,0.998885,0.000000,-0.047207,-0.000000,1.000000,0.000000,0.047207,-0.000000,0.998885),
CFrame.new(2806.406,137.020,-8512.783,-0.610555,-0.000000,-0.791974,-0.000000,1.000000,0.000000,0.791974,0.000000,-0.610555),
CFrame.new(2871.342,32.027,-8414.449,0.279599,0.000000,-0.960117,-0.000000,1.000000,0.000000,0.960117,0.000000,0.279599),
CFrame.new(3555.607,24.998,-8890.222,0.065099,-0.000000,-0.997879,-0.000000,1.000000,-0.000000,0.997879,0.000000,0.065099),
CFrame.new(4200.558,30.585,-8863.275,0.997366,0.000000,0.072534,-0.000000,1.000000,-0.000000,-0.072534,-0.000000,0.997366),
CFrame.new(4860.976,25.462,-7930.800,-0.989602,-0.000000,0.143834,-0.000000,1.000000,0.000000,-0.143834,0.000000,-0.989602),
CFrame.new(4721.313,34.848,-6873.854,-0.148959,-0.000000,-0.988843,-0.000000,1.000000,-0.000000,0.988843,0.000000,-0.148959),
CFrame.new(3700.177,44.611,-6779.036,-0.526967,-0.000000,0.849886,-0.000000,1.000000,0.000000,-0.849886,0.000000,-0.526967)
},
["Forest Island (Throwback)"]={
CFrame.new(-2645.7587890625,19.09952163696289,3957.66552734375,0.900820255279541,-8.90305607015307e-10,-0.43419215083122253,0.019455183297395706,0.9989956021308899,0.040363747626543045,0.4337560534477234,-0.04480776935815811,0.8999155163764954),
CFrame.new(-2811.169921875,32.69327163696289,3581.891357421875,0.9338151216506958,-5.614745335336124e-10,0.3577558994293213,-0.016030246391892433,0.9989956021308899,0.041842181235551834,-0.35739657282829285,-0.04480777680873871,0.932877242565155),
CFrame.new(-3028.420654296875,19.09952163696289,3233.06640625,0.8672376871109009,-1.2765652979140896e-09,-0.49789437651634216,0.02230953797698021,0.9989956021308899,0.038858987390995026,0.49739429354667664,-0.04480776935815811,0.8663666844367981),
CFrame.new(-3125.812744140625,58.78828811645508,3278.41015625,-0.9544302225112915,6.415117326241671e-10,-0.29843413829803467,0.03564987704157829,0.9928394556045532,-0.11401282995939255,0.29629719257354736,-0.11945643275976181,-0.9475959539413452),
CFrame.new(-3039.51611328125,55.556095123291016,3900.78076171875,-0.992558479309082,1.947409522529142e-08,0.12176892906427383,-0.0415569506585598,0.9399626851081848,-0.3387376666069031,-0.11445825546979904,-0.3412773013114929,-0.9329679012298584),
CFrame.new(-3122.237060546875,56.693267822265625,4369.5048828125,-0.04829072952270508,2.356704553729827e-10,-0.9988332986831665,0.044755496084690094,0.9989956021308899,-0.0021637999452650547,0.9978300929069519,-0.04480777308344841,-0.0482422299683094),
CFrame.new(-2704.343994140625,19.041061401367188,4454.23974609375,-0.8978657126426697,-1.1989987891425358e-09,0.44026947021484375,-0.01718595065176487,0.9992378354072571,-0.03504824638366699,-0.4399339258670807,-0.039035066962242126,-0.8971813917160034)
},
["Mountain Island (Throwback)"]={
CFrame.new(-2817.66162109375,175.23265075683594,1821.020263671875,-0.7795168161392212,-5.440058403749504e-10,0.6263812780380249,-1.0135109640430073e-08,1,-1.1744416106296285e-08,-0.6263812780380249,-1.550341366396424e-08,-0.7795168161392212),
CFrame.new(-3494.836181640625,18.852523803710938,2294.5029296875,0.9679929614067078,2.1962889285020992e-09,0.25097736716270447,0.004178429953753948,0.9998614192008972,-0.016115767881274223,-0.250942587852478,0.016648640856146812,0.9678587913513184),
CFrame.new(-2755.1767578125,18.86957359313965,2257.945068359375,-0.04128270223736763,2.072863235225153e-11,0.9991475343704224,0.013371532782912254,0.9999104738235474,0.00055248400894925,-0.9990580081939697,0.013382941484451294,-0.041279006749391556),
CFrame.new(-1706.5648193359375,18.86957359313965,1683.26904296875,0.9380949139595032,-5.677184833352555e-10,-0.34637826681137085,-0.004635559860616922,0.9999104738235474,-0.01255447044968605,0.34634724259376526,0.013382942415773869,0.9380109310150146),
CFrame.new(-3510.778564453125,117.78726196289062,2045.1370849609375,-0.688940167427063,-3.1397658517562377e-07,0.724818229675293,0.053280286490917206,0.997294545173645,0.050643377006053925,-0.7228572964668274,0.07350877672433853,-0.6870762705802917),
CFrame.new(-4112.453125,18.86957359313965,1498.7515869140625,-0.8605156540870667,-1.5045772272515023e-07,-0.5094240307807922,-0.0068177212961018085,0.9999104738235474,0.011516153812408447,0.5093783736228943,0.013382941484451294,-0.8604385852813721),
CFrame.new(-3558.8271484375,18.86957359313965,321.7713317871094,-0.9888119101524353,1.2208082322828773e-09,0.14916753768920898,0.001996301580220461,0.9999104738235474,0.013233211822807789,-0.1491541862487793,0.013382941484451294,-0.9887233972549438)
},
["Lunar Islands (Throwback)"]={
CFrame.new(-2692.972412109375,81.2981185913086,-3013.7421875,0.8737332820892334,1.1835785684866096e-09,-0.4864053428173065,0.07269331812858582,0.9887692928314209,0.13057951629161835,0.48094263672828674,-0.14945007860660553,0.8639206290245056),
CFrame.new(-3565.741455078125,15.51860237121582,-3990.25,-0.8327977061271667,1.6515574463582539e-09,0.5535774230957031,0.00791101437062025,0.9998978972434998,0.011901266872882843,-0.5535208582878113,0.014290707185864449,-0.832712709903717),
CFrame.new(-3758.672119140625,58.03699493408203,-3903.278564453125,0.42879927158355713,0.0000022030583295418182,-0.9033997654914856,0.22922514379024506,0.9672732353210449,0.10880421847105026,0.8738346695899963,-0.25373712182044983,0.41476553678512573),
CFrame.new(-3344.911376953125,27.45481300354004,-3538.0556640625,-0.18251392245292664,-7.660114209606661e-10,0.9832032918930054,0.2527621388435364,0.966390073299408,0.04692072421312332,-0.950157880783081,0.2570802569389343,-0.17637963593006134),
CFrame.new(-3667.556396484375,14.357109069824219,-1678.4263916015625,0.8500844836235046,0.0000012958216757397167,-0.5266463160514832,-0.18660850822925568,0.9351198077201843,-0.30121123790740967,0.49247702956199646,0.3543316721916199,0.7949311137199402),
CFrame.new(-3152.14111328125,53.04074478149414,-818.3273315429688,-0.5497733354568481,-3.9937089746366894e-10,0.8353138566017151,-0.01294289343059063,0.9998799562454224,-0.008518543094396591,-0.8352136015892029,-0.01549464650452137,-0.5497073531150818),
CFrame.new(-2589.01318359375,45.34171676635742,-732.80322265625,0.3039243221282959,8.568468388148176e-07,0.9526962041854858,-0.1402074247598648,0.9891113638877869,0.044727373868227005,-0.9423226118087769,-0.1471688151359558,0.300615131855011),
CFrame.new(-2201.864013671875,53.50882339477539,-1301.504150390625,0.9891817569732666,6.338255253979241e-09,0.14669498801231384,-0.008047064766287804,0.9984942674636841,0.054262276738882065,-0.14647410809993744,-0.054855719208717346,0.9876923561096191),
CFrame.new(-2212.5947265625,37.42900085449219,-1867.304443359375,0.9936606287956238,1.0158131003024096e-09,-0.11242134124040604,-0.03296387568116188,0.9560458660125732,-0.29135844111442566,0.10747995227575302,0.2932172417640686,0.9499850869178772)
},

["Wild Island (Throwback)"]={
CFrame.new(3432.30810546875,20.46332359313965,-4193.63134765625,0.98481285572052,-1.3883409699655402e-10,0.1736193299293518,0.002323537366464734,0.9999104738235474,-0.013179692439734936,-0.17360377311706543,0.013382941484451294,0.9847246408462524),
CFrame.new(4005.9033203125,20.812761306762695,-3152.5869140625,-0.7392423748970032,-6.944985697643347e-10,0.6734394431114197,0.004857037216424942,0.9999740123748779,0.005331627558916807,-0.6734219193458557,0.007212285418063402,-0.7392231822013855),
CFrame.new(4504.66259765625,92.46330261230469,-3211.67138671875,0.9078524708747864,5.18271122018632e-07,-0.419289767742157,-0.00561180105432868,0.9999104738235474,-0.012149520218372345,0.4192522168159485,0.013382942415773869,0.9077711701393127),
CFrame.new(3038.34619140625,34.11455154418945,-3864.0322265625,-0.6413583755493164,1.1108452824259984e-08,0.7672414779663086,0.14720547199249268,0.9814216494560242,0.12305311858654022,-0.7529873847961426,0.1918632835149765,-0.6294429898262024)

},
}

local d={
["Mainland"]=CFrame.new(34.923,14.990,-470.256,-0.987316,0,0.158766,0,1,0,-0.158766,0,-0.987316),
["Blizzard Island"]=CFrame.new(-395.354,15.404,-3828.735,-0.763117,0,0.646261,0,1,0,-0.646261,0,-0.763117),
["Forest Island"]=CFrame.new(-7436.085,28.764,4508.391,-0.889721,0,0.456505,0,1,0,-0.456505,0,-0.889721),
["Royal Island"]=CFrame.new(705.109,20.252,-5111.376,0.731477,0,0.681866,0,1,0,-0.681866,0,0.731477),
["Desert Island"]=CFrame.new(826.356,40.998,3601.169,0.420382,0,-0.907347,0,1,0,0.907347,0,0.420382),
["Glacier Island"]=CFrame.new(2702.397,-7.018,-40.926,-0.079052,0.000000,0.996871,0.000000,1.000000,-0.000000,-0.996871,0.000000,-0.079052),
["Mountain Island"]=CFrame.new(-7155.097,9.071,460.569,0.286330,0,-0.958131,0,1,0,0.958131,0,0.286330),
["Jungle Island"]=CFrame.new(3074.222,52.998,2183.594,0.172924,0,-0.984935,0,1,0,0.984935,0,0.172924),
["Lunar Islands"]=CFrame.new(-3615.954,15.423,-1817.699,-0.521454,0,0.853279,0,1,0,-0.853279,0,-0.521454),
["Volcano Island"]=CFrame.new(2858.625,24.998,-7019.049,0.672071,-0.000000,0.740487,0.000000,1.000000,0.000000,-0.740487,0.000000,0.672071)
}

local e={
["Mainland"]=CFrame.new(-50.854,13.037,-941.285,0.818569,0,0.574408,0,1,0,-0.574408,0,0.818569),
["Blizzard Island"]=CFrame.new(-318.315,15.842,-3194.496,-0.997635,0,-0.068731,0,1,0,0.068731,0,-0.997635),
["Forest Island"]=CFrame.new(-7091.160,19.167,4562.860,-0.464963,0,-0.885330,0,1,0,0.885330,0,-0.464963),
["Royal Island"]=CFrame.new(886.675,13.871,-4781.455,-0.996450,0,-0.084187,0,1,0,0.084187,0,-0.996450),
["Desert Island"]=CFrame.new(744.558,9.802,3289.691,0.941137,0,0.338026,0,1,0,-0.338026,0,0.941137),
["Glacier Island"]=CFrame.new(2662.256,-6.468,329.880,-0.753561,-0.000000,0.657378,0.000000,1.000000,0.000000,-0.657378,0.000000,-0.753561),
["Mountain Island"]=CFrame.new(-7530.906,9.253,222.176,0.745782,0,0.666190,0,1,0,-0.666190,0,0.745782),
["Jungle Island"]=CFrame.new(2758.440,15.277,2194.426,-0.396159,0,0.918182,0,1,0,-0.918182,0,-0.396159),
["Lunar Islands"]=CFrame.new(-3508.798,15.270,-1886.841,0.208355,0,-0.978053,0,1,0,0.978053,0,0.208355),
["Volcano Island"]=CFrame.new(2419.245,18.078,-6687.550,-0.860906,0.000000,-0.508763,0.000000,1.000000,-0.000000,0.508763,-0.000000,-0.860906)
}




local f={
["Mainland"]=false,
["Blizzard Island"]=false,
["Forest Island"]=false,
["Royal Island"]=false,
["Desert Island"]=false,
["Glacier Island"]=false,
["Mountain Island"]=false,
["Jungle Island"]=false,
["Lunar Islands"]=false,
["Volcano Island"]=false,
}

local g=false
local h=false
local i=false
local j=0




local k=false
local l={}
local m=1
local n=false
local o=false
local p=nil




if isfile and isfile(am)then
local q,r=pcall(function()
return ae:JSONDecode(readfile(am))
end)
if q and type(r)=="table"and r.__islandhop then
k=true
g=r.autotravel or false
h=r.wildherd or false
ap=r.idleLimit or ak
ao=true

if type(r.islands)=="table"then
for s,t in pairs(r.islands)do
if f[s]~=nil then
f[s]=t
end
end
end

if type(r.remaining)=="table"then
p=r.remaining
end


pcall(function()writefile(am,"")end)
end
end




local q=120
local r=0.5




local s=nil
local t=nil
local u=nil
local v=nil
local w=nil
local x=nil




local function y(z,A)
if not z then return end
for B,C in ipairs(z:GetDescendants())do
if C:IsA("BasePart")then
C.CanCollide=not A
end
end
end

task.spawn(function()
while true do
ac.Heartbeat:Wait()
if not ao then continue end
local z=af.Character
if not z then continue end
local A=z:FindFirstChild("UpperTorso")
local B=z:FindFirstChild("LowerTorso")
if A and A.CanCollide then A.CanCollide=false end
if B and B.CanCollide then B.CanCollide=false end
end
end)




local function z()
if u then u:Disconnect();u=nil end
if t then t:Destroy();t=nil end
if s then s:Destroy();s=nil end
if v then
v.PlatformStand=false
v=nil
end
w=nil
x=nil
end




local function A(B,C)
z()

local D=af.Character
local E=D and D:FindFirstChild("HumanoidRootPart")
if not E then return end

w=B:FindFirstAncestorOfClass("Model")
x=C

local F=D:FindFirstChildOfClass("Humanoid")
if F then
F.PlatformStand=true
v=F
end

y(D,true)

s=Instance.new("Attachment")
s.Parent=E

t=Instance.new("LinearVelocity")
t.Attachment0=s
t.MaxForce=1e6
t.RelativeTo=Enum.ActuatorRelativeTo.World
t.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
t.VectorVelocity=Vector3.zero
t.Parent=E

u=ac.Heartbeat:Connect(function()
if not B or not B.Parent then
local G=x
z()
if G then G()end
return
end

local G=af.Character and af.Character:FindFirstChild("HumanoidRootPart")
if not G or not t then return end

local H=B.Position+Vector3.new(0,aj,0)
local I=H-G.Position
local J=I.Magnitude

if J<r then
t.VectorVelocity=Vector3.zero
return
end

t.VectorVelocity=I.Unit*math.min(J*60,q)
end)
end




af.CharacterAdded:Connect(function(B)
z()
task.wait(1.5)
y(B,false)
i=false
if ao then
y(B,true)
end
end)




local function B(C)
if not C then return end
local D=b[C]or 1
pcall(function()ai.Travel(C,D)end)
end




local function C()
local D=workspace:FindFirstChild("Islands")
if not D then return nil end
for E,F in ipairs(D:GetChildren())do
if F:FindFirstChild(af.Name)then return F end
end
return nil
end




local function D(E,F)
if not F then return end
if i then return end

local G=c[F.Name]
if not G or#G==0 then return end

local H=G[math.random(1,#G)].Position
local I=af.Character
if not I then return end
local J=I:FindFirstChildOfClass("Humanoid")

i=true
j=tick()

local K=15
local L=40
local M=2

local N=t~=nil
local O,P=t,s
local Q,R=nil,nil

if not N then
if J then J.PlatformStand=true end
R=Instance.new("Attachment")
R.Parent=E
Q=Instance.new("LinearVelocity")
Q.Attachment0=R
Q.MaxForce=1e6
Q.RelativeTo=Enum.ActuatorRelativeTo.World
Q.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
Q.VectorVelocity=Vector3.zero
Q.Parent=E
O,P=Q,R
end

local function S()
if Q then Q:Destroy()end
if R then R:Destroy()end
if J and not N then
J.PlatformStand=false
end
i=false
end

local T
T=ac.Heartbeat:Connect(function()
if tick()-j>K then
T:Disconnect()
S()
return
end

local U=af.Character
local V=U and U:FindFirstChild("HumanoidRootPart")

if not V or not O or not O.Parent then
T:Disconnect()
S()
return
end

local W=H-V.Position
local X=W.Magnitude

if X<M then
O.VectorVelocity=Vector3.zero
T:Disconnect()
S()
else
O.VectorVelocity=W.Unit*math.min(X*8,L)
end
end)
end




local function E(F,G)
local H=nil
local I=math.huge

if h then
local J=G:FindFirstChild("WildHerdSpawners")
if J then
for K,L in ipairs(J:GetChildren())do
for M,N in ipairs(L:GetDescendants())do
if N:IsA("Model")then
local O=N:FindFirstChild("HumanoidRootPart")
if O then
local P=(F.Position-O.Position).Magnitude
if P<I then
I=P
H=O
end
end
end
end
end
end
return H
end

for J,K in ipairs(G:GetDescendants())do
if K:IsA("Model")then
local L=K:FindFirstChild("HumanoidRootPart")
local M=K:FindFirstChild("CaptureProgress",true)
if L and M then
local N=(F.Position-L.Position).Magnitude
if N<I then I=N;H=L end
end
end
end

return H
end

local function F(G)
local H={}
for I,J in ipairs(aq)do
if f[J]then table.insert(H,J)end
end
if#H==0 then return nil end
if#H==1 then return H[1]end
for I,J in ipairs(H)do
if J==G then return H[(I%#H)+1]end
end
return H[1]
end




local function G()
local H=workspace:FindFirstChild("Islands")
if not H then return end
local I=H:FindFirstChild("Volcano Island")
if not I then return end
local J=I:FindFirstChild("LavaParts")
if not J then return end
for K,L in ipairs(J:GetDescendants())do
if L:IsA("TouchTransmitter")then L:Destroy()end
end
end

do
local H=nil
ac.Heartbeat:Connect(function()
local I=C()
if I and I.Name=="Volcano Island"and H~="Volcano Island"then
H="Volcano Island"
G()
elseif not I or I.Name~="Volcano Island"then
H=I and I.Name or nil
end
end)
task.spawn(function()
while true do G();task.wait(3)end
end)
end




local function H()
l={}
m=1
for I,J in ipairs(aq)do
if f[J]then
table.insert(l,J)
end
end
end

local function I(J)
local K={}
for L,M in pairs(f)do K[L]=M end
return ae:JSONEncode({
__islandhop=true,
islands=K,
idleLimit=ap,
autotravel=g,
wildherd=h,
remaining=J,
})
end





local function J(K,L)
if o then return end
o=true

task.spawn(function()

print("[IslandHop] Cycle complete — heading to Training Island...")
pcall(function()ai.Travel("Training Island",1)end)

local M=tick()+30
repeat
task.wait(1)
local N=C()
if N and N.Name=="Training Island"then break end
until tick()>M

task.wait(2)



local N=I(K)
pcall(function()writefile(am,N)end)
print("[IslandHop] Config saved — remaining: "..#K.." islands")



local O=string.format(
'loadstring(game:HttpGet("%s"))()',
an
)
queueonteleport(O)
print("[IslandHop] Script queued for post-teleport execution")


pcall(function()ai.DoSamePlaceTravel()end)


task.wait(5)
o=false
if L then L()end
end)
end




do
local K=5

task.spawn(function()
local L=0
local M=nil
local N=false
local O=0
local P=0
local Q=false
local R={}

local function S(T)
if not T then return false end
if not T.Parent then return false end
local U=T:FindFirstAncestorOfClass("Model")
if not U then return false end
if not U.Parent then return false end
return true
end

local function T()
z()
M=nil
P=0
end

local function U(V)
if N then return end
N=true
L=0
O=0
T()

local W=tick()
while i do
if tick()-W>20 then
i=false
break
end
task.wait(0.2)
end
task.wait(0.5)

B(V)
local X=tick()+30
repeat
task.wait(1)
local Y=C()
if Y and Y.Name==V then break end
until tick()>X
task.wait(2)
N=false
end

local function V(W)
if N then return end
local X=C()
if not X then return end
D(W,X)
end

local function W()
for X,Y in ipairs(l)do
if not R[Y]then return false end
end
return true
end

local function X()
for Y,Z in ipairs(l)do
if not R[Z]then return Z end
end
return nil
end



local function Y()
local Z={}
for _,ar in ipairs(l)do
if not R[ar]then
table.insert(Z,ar)
end
end
return Z
end

while true do
task.wait(0.4)

if i and tick()-j>20 then
i=false
end

if ao~=Q then
Q=ao
local ar=af.Character
if ar then y(ar,ao)end
if not ao then
T()
L=0
O=0
R={}
end
end

if not ao then continue end
if N then continue end
if o then continue end

local ar=af.Character
local Z=ar and ar:FindFirstChild("HumanoidRootPart")
if not Z then continue end

local _=C()
if not _ then continue end


if k then

if#l==0 then
H()
R={}


if p then
local as={}
for at,au in ipairs(p)do
as[au]=true
end
for at,au in ipairs(l)do
if f[au]and not as[au]then
R[au]=true
end
end
p=nil
end
end

if#l==0 then continue end


if not f[_.Name]then
local as=X()
if as then U(as)end
continue
end


if M then
if S(M)then
P=0
else
P+=1
if P>=K then T()end
end
end

if not M then
local as=E(Z,_)
if as then
M=as
P=0
A(M,function()
M=nil
P=0
end)
end
end

if M then
L=0
O=0
else
L+=1
O+=0.4

if L>=(tonumber(ap)or ak)then
V(Z)
L=0
end

if O>=al then
R[_.Name]=true
O=0
L=0

if W()then


local as={}
for at,au in ipairs(l)do
table.insert(as,au)
end
J(as,function()
H()
R={}
end)
else

local as=X()
if as then U(as)end
end
end
end

continue
end


if g and not f[_.Name]then
local as=F(_.Name)
if as then U(as)end
continue
end

if M then
if S(M)then
P=0
else
P+=1
if P>=K then T()end
end
end

if not M then
local as=E(Z,_)
if as then
M=as
P=0
A(M,function()
M=nil
P=0
end)
end
end

if M then
L=0
O=0
else
L+=1
O+=0.4

if g and O>=al then
local as=F(_.Name)
if as and as~=_.Name then
U(as)
else
V(Z)
O=0
L=0
end
continue
end

if L>=(tonumber(ap)or ak)then
V(Z)
L=0
end
end
end
end)
end




return{
setEnabled=function(ar)ao=ar end,
setWildherd=function(ar)h=ar end,
setAutotravel=function(ar)g=ar end,
setIdleLimit=function(ar)ap=ar end,
setIsland=function(ar,as)
if f[ar]~=nil then
f[ar]=as
end
end,
getIslands=function()
local ar={}
for as,at in pairs(f)do ar[as]=at end
return ar
end,
setIslandHop=function(ar)
k=ar
if not ar then
l={}
m=1
n=false
o=false
end
end,
isIslandHop=function()return k end,
}end function a.d():typeof(aa())local ab=a.cache.d if not ab then ab={c=aa()}a.cache.d=ab end return ab.c end end do local function aa()






local ab=game:GetService("ReplicatedStorage")
local ac=game:GetService("Players")
local ad=game:GetService("RunService")

local ae=ac.LocalPlayer
local af=require(ab:WaitForChild("References"))
local ah=af.Utilities
local ai=require(ae.PlayerScripts.Priority:WaitForChild("Data"))
local aj=require(ae.PlayerScripts.Priority:WaitForChild("InventoryHandler"))








local ak={

{id=401,name="Prismatic Lasso",strength=30,craftable=true},
{id=32,name="Perfect Lasso",strength=25,craftable=false},
{id=1349,name="Rescue Lasso",strength=25,craftable=false},
{id=31,name="Moonstone Lasso",strength=24,craftable=true},
{id=30,name="Obsidian Lasso",strength=22,craftable=true},
{id=29,name="Amethyst Lasso",strength=20,craftable=true},
{id=28,name="Emerald Lasso",strength=18,craftable=true},
{id=27,name="Topaz Lasso",strength=16,craftable=true},
{id=26,name="Sapphire Lasso",strength=14,craftable=true},
{id=25,name="Diamond Lasso",strength=12,craftable=true},
{id=1243,name="Ice Lasso",strength=11,craftable=true},
{id=24,name="Ruby Lasso",strength=10,craftable=true},
{id=325,name="Clear Quartz Lasso",strength=7,craftable=true},
{id=21,name="Iron Lasso",strength=7,craftable=true},
{id=22,name="Silver Lasso",strength=7,craftable=true},
{id=23,name="Gold Lasso",strength=8,craftable=true},
{id=20,name="Bronze Lasso",strength=6,craftable=true},
{id=19,name="Copper Lasso",strength=5,craftable=true},
{id=18,name="Tin Lasso",strength=4,craftable=true},
{id=191,name="Stone Lasso",strength=3,craftable=true},
{id=17,name="Wooden Lasso",strength=2,craftable=true},
{id=1029,name="Corrupt Lasso",strength=20,craftable=false},
{id=1163,name="Lasso",strength=1,craftable=false},
}


local al={}
for am,an in ipairs(ak)do
al[an.id]=am
end







local am={
[17]={{id=102,qty=1},{id=70,qty=1}},
[18]={{id=102,qty=1},{id=35,qty=3}},
[19]={{id=102,qty=1},{id=34,qty=3}},
[20]={{id=102,qty=1},{id=36,qty=3}},
[21]={{id=102,qty=1},{id=37,qty=3}},
[22]={{id=102,qty=1},{id=38,qty=3}},
[23]={{id=102,qty=1},{id=39,qty=3}},
[24]={{id=102,qty=1},{id=40,qty=2}},
[25]={{id=102,qty=1},{id=41,qty=2}},
[26]={{id=102,qty=1},{id=42,qty=2}},
[27]={{id=102,qty=1},{id=43,qty=2}},
[28]={{id=102,qty=1},{id=44,qty=2}},
[29]={{id=102,qty=1},{id=45,qty=2}},
[30]={{id=102,qty=1},{id=46,qty=2}},
[31]={{id=102,qty=1},{id=47,qty=2}},
[191]={{id=102,qty=1},{id=33,qty=3}},
[325]={{id=102,qty=1},{id=323,qty=3}},
[401]={{id=102,qty=1},{id=381,qty=2}},
[1243]={{id=102,qty=1},{id=1238,qty=2}},
}






local an={
[102]={shop="Mainland Shop",slot=1},
[70]={shop="Mainland Shop",slot=8},
[33]={shop="Mainland Shop",slot=9},
[35]={shop="Mainland Shop",slot=10},
[34]={shop="Blizzard Island Shop",slot=8},
[36]={shop="Blizzard Island Shop",slot=9},
[37]={shop="Forest Island Shop",slot=8},
[38]={shop="Forest Island Shop",slot=9},
[39]={shop="Desert Island Shop",slot=8},
[40]={shop="Desert Island Shop",slot=9},
[41]={shop="Mountain Island Shop",slot=8},
[42]={shop="Mountain Island Shop",slot=9},
[43]={shop="Jungle Island Shop",slot=9},
[44]={shop="Jungle Island Shop",slot=10},
[45]={shop="Jungle Island Shop",slot=11},
[46]={shop="Lunar Islands Shop",slot=8},
[47]={shop="Lunar Islands Shop",slot=9},
[323]={shop="Royal Island Shop",slot=7},
[381]={shop="Volcano Island Shop",slot=7},
[1238]={shop="Glacier Island Shop",slot=6},
}




local ao={
autoEquipEnabled=false,
autoCraftEnabled=false,
selectedLassoId=191,
restockThreshold=50,
restockAmount=100,
pollInterval=0.5,
craftPollInterval=3,
buyCooldown=0.4,
craftCooldown=1.0,
confirmTimeout=5,
confirmInterval=0.1,
}

local ap=250
local aq=1000




local ar=false
local as=false
local at=0
local au=0.8
local b=1.2
local c=0.05
local d=3




local function e(f)
local g,h=pcall(aj.GetAmountOf,f)
return(g and tonumber(h))or 0
end

local function f()
return ai.GetLocal({"quickEquipment","Lasso"})
end

local function g()
return ai.GetLocal({"temporary","equippedEquipment"})
end

local function h()
local i=f()
if not i then return false end
return tostring(g())==tostring(i)
end






local i=1163
local j=0
local k=2.0

local function l()
local m=tick()
if m-j<k then return i end
j=m
for n,o in ipairs(ak)do
if e(o.id)>0 then
i=o.id
return i
end
end
i=1163
return i
end






local function m()
ah.Network:FireServer("QuickEquipment","Use","Lasso")
local n=tick()+b
while tick()<n do
task.wait(c)
if h()then return true end
end
return false
end

local function n(o)


local p=aj.GetGuidsOfItemId(o)
if not p or#p==0 then return false end

ah.Network:FireServer("QuickEquipment","Set","Lasso",p[1])
task.wait(0.3)
return true
end

local function o()
if not ao.autoEquipEnabled then return end
if ar then return end

local p=l()
local q=f()

local r=q and aj.GetGuidsOfItemId(p)
local s=false
if r then
for t,u in ipairs(r)do
if u==q then
s=true
break
end
end
end

if h()and s then return end

local t=tick()
if(t-at)<au then return end

if not s then
local u=n(p)
if not u then return end
end

ar=true
at=tick()

local u,v=pcall(function()
for u=1,d do
if m()then break end
task.wait(au*u)
end
end)

if not u then warn("[AutoLasso] Equip error:",v)end
ar=false
end




local function p(q,r)
local s=tick()+ao.confirmTimeout
while tick()<s do
task.wait(ao.confirmInterval)
if e(q)>r then return true end
end
return false
end

local function q(r,s)
local t=an[r]
if not t then
warn(string.format("[AutoLasso] No shop source for material id=%d",r))
return 0
end

s=math.min(s,aq)
if s<=0 then return 0 end

local u=e(r)
ah.Network:FireServer("Shopping","BuyShopItem",t.shop,t.slot,s,nil)

local v=p(r,u)
if not v then
warn(string.format("[AutoLasso] Buy timed out id=%d shop=%s slot=%d qty=%d",
r,t.shop,t.slot,s))
return 0
end
return e(r)-u
end

local function r(s,t)
t=math.min(t,ap)
if t<=0 then return 0 end

local u=e(s)
ah.Network:FireServer("Crafting","Craft",{
id=s,
variants={},
amt=t,
})

local v=p(s,u)
if not v then
warn(string.format("[AutoLasso] Craft timed out id=%d qty=%d",s,t))
return 0
end
return e(s)-u
end

local function s(t,u)
local v=am[t]
if not v then
warn(string.format("[AutoLasso] No recipe for lasso id=%d",t))
return
end

u=math.min(u,ap)


for w,x in ipairs(v)do
local y=x.qty*u
local z=e(x.id)
local A=math.max(0,y-z)
if A>0 then
local B=q(x.id,A)
if B<A then
warn(string.format("[AutoLasso] Material short: id=%d needed=%d got=%d",
x.id,A,B))
end
task.wait(ao.buyCooldown)
end
end


local w=u
for x,y in ipairs(v)do
local z=e(y.id)
local A=math.floor(z/y.qty)
w=math.min(w,A)
end

w=math.min(w,ap)
if w<=0 then
warn("[AutoLasso] No materials to craft after buying — aborting.")
return
end

task.wait(ao.craftCooldown)

local x=r(t,w)
if x>0 then


local y=l()
local z=al[t]or 999
local A=al[y]or 999
if z<=A then
n(t)
task.wait(0.3)
ah.Network:FireServer("QuickEquipment","Use","Lasso")
end
else
warn("[AutoLasso] Craft fired but no lassos received.")
end
end






task.spawn(function()
while true do
task.wait(ao.pollInterval)
o()
end
end)


task.spawn(function()
while true do
task.wait(ao.craftPollInterval)
if not ao.autoCraftEnabled then continue end
if as then continue end

local t=ao.selectedLassoId
if e(t)<=ao.restockThreshold then
as=true
local u,v=pcall(s,t,ao.restockAmount)
if not u then warn("[AutoLasso] Restock error:",v)end
as=false
end
end
end)


ae.CharacterAdded:Connect(function()
task.wait(1.5)
ar=false
at=0
as=false
end)




return{

setEnabled=function(t)
ao.autoEquipEnabled=t
if not t then
ar=false
end
end,


setCraftEnabled=function(t)
ao.autoCraftEnabled=t
if not t then as=false end
end,


setSelectedLasso=function(t)
ao.selectedLassoId=tonumber(t)or 191
end,


setRestockThreshold=function(t)
ao.restockThreshold=tonumber(t)or 50
end,


setRestockAmount=function(t)
ao.restockAmount=math.min(tonumber(t)or 100,ap)
end,


getCraftableLassos=function()
local t={}
for u,v in ipairs(ak)do
if v.craftable then
table.insert(t,{id=v.id,name=v.name,strength=v.strength})
end
end
return t
end,


getStatus=function()
local t=l()
return{
autoEquipEnabled=ao.autoEquipEnabled,
autoCraftEnabled=ao.autoCraftEnabled,
isCycleRunning=as,
bestLassoId=t,
bestLassoName=(function()
for u,v in ipairs(ak)do
if v.id==t then return v.name end
end
return"Unknown"
end)(),
selectedCraftId=ao.selectedLassoId,
selectedCraftCount=e(ao.selectedLassoId),
threshold=ao.restockThreshold,
}
end,


triggerRestock=function()
if as then return end
as=true
local t,u=pcall(s,ao.selectedLassoId,ao.restockAmount)
as=false
if not t then warn("Manual restock error:",u)end
end,
}end function a.e():typeof(aa())local ab=a.cache.e if not ab then ab={c=aa()}a.cache.e=ab end return ab.c end end do local function aa()






local ab=require(game:GetService("ReplicatedStorage").References)
local ac=ab.Utilities
local ad=require(ab.PlayerScripts.Priority.Data)
local ae=game:GetService("Players")
local af=ae.LocalPlayer
local ah=require(ab.PlayerScripts.Secondary:WaitForChild("EquipmentHandler"))

local ai=false
local aj=0.1

local ak=game:GetService("CollectionService")

local function al()
local am=af.Character and af.Character:FindFirstChild("HumanoidRootPart")
if not am then return nil end

local an,ao=nil,math.huge

for ap,aq in ipairs(ak:GetTagged("LassoCapture"))do
if not aq:IsDescendantOf(workspace)then continue end

local ar=aq:FindFirstChild("HumanoidRootPart")
or aq:FindFirstChildWhichIsA("BasePart")
if not ar then continue end

local as=(am.Position-ar.Position).Magnitude
if as<ao then
ao=as
an=aq
end
end

return an
end

task.spawn(function()
while true do
task.wait(aj)
if not ai then continue end

local am=ah.object
if not am then continue end

local an=al()
if not an then continue end

pcall(function()am:Activate()end)
end
end)

return{
setEnabled=function(am)ai=am end,
setDuration=function(am)aj=tonumber(am)or 0.1 end,
isEnabled=function()return ai end,
}end function a.f():typeof(aa())local ab=a.cache.f if not ab then ab={c=aa()}a.cache.f=ab end return ab.c end end do local function aa()




local ab=game:GetService("Players")
local ac=ab.LocalPlayer




local ad=0
local ae=0
local af=0
local ah=0
local ai=nil


local aj={
mismatchHairColour=0,
summer2026=0,
naturallyDyedHairColour=0,
islandUniqueCoat=0,
islandUniqueHorn=0,
islandUniqueHairColour=0,
specialIslandUniqueCoat=0,
specialCoat=0,
specialHair=0,
horned=0,
rareCoat=0,
}




local function ak(al)
if not al or al==""then return nil end
al=al:gsub(",",""):gsub("%s+","")
local am=1
if al:sub(1,1)=="-"then am=-1;al=al:sub(2)
elseif al:sub(1,1)=="+"then al=al:sub(2)end
local an=tonumber(al)
return an and(am*math.floor(an))or nil
end

local function al()
local am=ac:FindFirstChild("PlayerGui");if not am then return nil end
local an=am:FindFirstChild("HUD");if not an then return nil end
local ao=an:FindFirstChild("TopBar");if not ao then return nil end
local ap=ao:FindFirstChild("Tokens");if not ap then return nil end
return ap:FindFirstChild("ChangeLabel")
end

local am=0
local an=10

local function ao()
if ai then ai:Disconnect();ai=nil end

local ap=al()
if not ap then
am+=1
if am<an then
task.delay(3,ao)
else
warn("[Counter] ChangeLabel not found after max retries — coin tracking disabled")
end
return
end

am=0
ai=ap:GetPropertyChangedSignal("Text"):Connect(function()
local aq=ak(ap.Text)
if aq and aq>0 then
ae+=aq
end
end)
end




local ap={}

function ap.recordSell()
ad+=1
af+=1
end


function ap.recordLock(aq)
ad+=1
ah+=1
if aq and aj[aq]~=nil then
aj[aq]+=1
end
end

function ap.getStats()

local aq={}
for ar,as in pairs(aj)do
aq[ar]=as
end
return{
captures=ad,
sold=af,
locked=ah,
coins=ae,

lockedByReason=aq,
}
end

function ap.reset()
ad=0
ae=0
af=0
ah=0
for aq in pairs(aj)do
aj[aq]=0
end
end

function ap.destroy()
if ai then ai:Disconnect();ai=nil end
end

ao()

return ap end function a.g():typeof(aa())local ab=a.cache.g if not ab then ab={c=aa()}a.cache.g=ab end return ab.c end end do local function aa()

local ab=game:GetService("ReplicatedStorage")
local ac=game:GetService("HttpService")
local ad=require(ab.References.HorseVariants)
local ae=a.g()
local af=game:GetService("Players")
local ah=af.LocalPlayer
local ai=require(ab.References.Utilities).Network
local aj=require(ah.PlayerScripts.Priority:WaitForChild("InventoryHandler"))
local ak=require(ab:WaitForChild("References"))
local al=require(ak.PlayerScripts.Priority:WaitForChild("AnimalHandler"))

local am={

"mismatchHairColour",
"naturallyDyedHairColour",
"islandUniqueHairColour",

"rareCoat",
"islandUniqueCoat",
"specialIslandUniqueCoat",
"specialCoat",
"hybridUniqueCoat",
"breedingUnique",
"cobreedingUnique",
"legacyCoat",
"collectionLogReward",
"volcanicMineralsUnique",
"trainingReceiptsUnique",
"goldenAppleBasket",

"horned",
"islandUniqueHorn",

"specialHair",

"event",
"throwback",
"summer2022",
"summer2023",
"summer2024",
"summer2025",
"summer2026",
"autumn2022",
"autumn2023",
"autumn2024",
"autumn2025",
"winter2022",
"winter2023",
"winter2024",
"winter2025",
"spring2023",
"spring2024",
"spring2025",
"spring2026",
"halloween",
"halloween2022",
"halloween2023",
"halloween2024",
"halloween2025",
"holiday",
"holiday2023",
"holiday2024",
"holiday2025",
"valentines",
"valentines2022",
"valentines2023",
"valentines2024",
"valentines2025",
"valentines2026",
"easter2022",
"stpatricksday2022",
"stpatricksday2025",
"stpatricksday2026",
"competitionHubUnique",

"summer",
"autumn",
"winter",
"spring",
"halloween",
"holiday",
"valentines",
"stpatricksday",
"throwback",
"trainingReceiptsUnique",
"competitionHubUnique",
"islandUniqueHairColour",
"unreleasedEventCoat",
}


do
local an={}
local ao={}
for ap,aq in ipairs(am)do
if not an[aq]then
an[aq]=true
table.insert(ao,aq)
end
end
am=ao
end

local an={}
for ao,ap in ipairs(am)do
an[ap]=false
end

local ao=0.975
local ap=0.5




local aq=""
local ar=false

local as={
mismatchHairColour="Mismatched Hair",
naturallyDyedHairColour="Naturally Dyed Hair",
islandUniqueHairColour="Island Unique Hair Colour",
specialHair="Special Hair",
rareCoat="Rare Coat",
islandUniqueCoat="Island Unique Coat",
specialIslandUniqueCoat="Special Island Unique Coat",
specialCoat="Special Coat",
hybridUniqueCoat="Hybrid Unique Coat",
breedingUnique="Breeding Unique",
cobreedingUnique="Co-Breeding Unique",
legacyCoat="Legacy Coat",
collectionLogReward="Collection Log Reward",
volcanicMineralsUnique="Volcanic Minerals Unique",
trainingReceiptsUnique="Training Receipts Unique",
goldenAppleBasket="Golden Apple Basket",
horned="Horned",
islandUniqueHorn="Island Unique Horn",
event="Event",
throwback="Throwback",
summer2022="Summer 2022",
summer2023="Summer 2023",
summer2024="Summer 2024",
summer2025="Summer 2025",
summer2026="Summer 2026",
autumn2022="Autumn 2022",
autumn2023="Autumn 2023",
autumn2024="Autumn 2024",
autumn2025="Autumn 2025",
winter2022="Winter 2022",
winter2023="Winter 2023",
winter2024="Winter 2024",
winter2025="Winter 2025",
spring2023="Spring 2023",
spring2024="Spring 2024",
spring2025="Spring 2025",
spring2026="Spring 2026",
halloween="Halloween",
halloween2022="Halloween 2022",
halloween2023="Halloween 2023",
halloween2024="Halloween 2024",
halloween2025="Halloween 2025",
holiday="Holiday",
holiday2023="Holiday 2023",
holiday2024="Holiday 2024",
holiday2025="Holiday 2025",
valentines="Valentines",
valentines2022="Valentines 2022",
valentines2023="Valentines 2023",
valentines2024="Valentines 2024",
valentines2025="Valentines 2025",
valentines2026="Valentines 2026",
easter2022="Easter 2022",
stpatricksday2022="St. Patrick's Day 2022",
stpatricksday2025="St. Patrick's Day 2025",
stpatricksday2026="St. Patrick's Day 2026",
competitionHubUnique="Competition Hub Unique",
summer="Summer Hair",
autumn="Autumn Hair",
winter="Winter Hair",
spring="Spring Hair",
stpatricksday="St. Patrick's Day Hair",
unreleasedEventCoat="Unreleased Event Coat",
}

local at={
["sandy"]=true,["pearly purple"]=true,["pearly gold"]=true,["clear sea glass"]=true,
["blessed"]=true,["iceyBlue"]=true,["iceyWhite"]=true,["iceyPink"]=true,
["iceyBlack"]=true,["iceyGreen"]=true,["winterStreaks"]=true,["flowery"]=true,
["pearlyGreen"]=true,["leafy"]=true,["pearly"]=true,["pastelStreaks"]=true,
["leathery"]=true,["royalStreaks"]=true,["cowPrint"]=true,["pearlyPink"]=true,
["clearQuartz"]=true,["zebraStripes"]=true,["dustyFade"]=true,["limestone"]=true,
["sandstone"]=true,["ruby"]=true,["pearlyRed"]=true,["glacierGreen"]=true,
["glacierPink"]=true,["glacierBlue"]=true,["snowyGlacierFade"]=true,
["pinkCowPrint"]=true,["diamond"]=true,["sapphire"]=true,["pearlyOrange"]=true,
["sunriseStreaks"]=true,["amethyst"]=true,["topaz"]=true,["jaguarSpots"]=true,
["mossy"]=true,["tigerStripes"]=true,["emerald"]=true,["deepSpace"]=true,
["neonBlue"]=true,["obsidian"]=true,["neonPurple"]=true,["moonstone"]=true,
["pearlyBlue"]=true,["volcanicOrange"]=true,["pearlyBlack"]=true,
["crackedLavaFade"]=true,["volcanicBlack"]=true,["prismatic"]=true,
}




local function au(b,c,d)
if not ar or aq==""then return end

local e=c and c.variants or{}
local f={}

table.insert(f,{
name="Lock Reason",
value=as[d]or d or"Unknown",
inline=true,
})

if e.colour then
local g=ad.colour[e.colour]
table.insert(f,{
name="Coat",
value=(g and g.name)or e.colour,
inline=true,
})
if g then
table.insert(f,{
name="Rarity",
value=tostring(math.floor((g.rarityFloat or 0)*100)).."%",
inline=true,
})
end
end

if e.maneColour then
local g=ad.maneAndTailColour[e.maneColour]
table.insert(f,{
name="Mane Colour",
value=(g and g.name)or e.maneColour,
inline=true,
})
end

if e.tailColour then
local g=ad.maneAndTailColour[e.tailColour]
table.insert(f,{
name="Tail Colour",
value=(g and g.name)or e.tailColour,
inline=true,
})
end

if e.maneColour and e.tailColour
and e.maneColour~=e.tailColour then
table.insert(f,{name="Mismatch",value="Yes",inline=true})
end

if e.hornStyle and e.hornStyle~=""then
table.insert(f,{name="Horn Style",value=e.hornStyle,inline=true})
end

if e.hornColour and e.hornColour~=""then
table.insert(f,{name="Horn Colour",value=e.hornColour,inline=true})
end

if c.isNatDyed then
table.insert(f,{name="Naturally Dyed",value="Yes",inline=true})
end

if e.maneStyle then
table.insert(f,{name="Mane Style",value=e.maneStyle,inline=true})
end

if e.tailStyle then
table.insert(f,{name="Tail Style",value=e.tailStyle,inline=true})
end

table.insert(f,{
name="GUID",
value="`"..tostring(b).."`",
inline=false,
})

local g=ae.getStats()
table.insert(f,{
name="Session Stats",
value=string.format("Sold: %d | Locked: %d | Coins: %d",
g.sold,g.locked,g.coins),
inline=false,
})

local h=ac:JSONEncode({
embeds={{
title="Horse Locked — "..(as[d]or d or"Unknown"),
color=0xF5A623,
fields=f,
footer={text="coconut.xyz • "..os.date("!%Y-%m-%d %H:%M:%S UTC")},
timestamp=os.date("!%Y-%m-%dT%H:%M:%SZ"),
}}
})

task.spawn(function()
local i,j=pcall(function()
local i=(syn and syn.request)or http_request or request
if not i then warn("[AutoSell] Webhook failed: no http function available")return end
i({Url=aq,Method="POST",Headers={["Content-Type"]="application/json"},Body=h})
end)
if not i then warn("[AutoSell] Webhook failed:",j)end
end)
end




local function b(c)
if not c then return false,nil end
local d=c.variants
if not d then return false,nil end

if an["horned"]==true then
if(d.hornStyle and d.hornStyle~="")
or(d.hornColour and d.hornColour~="")then
return true,"horned"
end
end

if d.colour then
local e=ad.colour[d.colour]
if e then
local f=e.specialItemIndicator
if f and an[f]==true then
return true,f
end
if an["rareCoat"]==true
and(e.rarityFloat or 0)>=ao then
return true,"rareCoat"
end
end
end

for e,f in ipairs({"maneColour","tailColour"})do
local g=d[f]
if g then
local h=ad.maneAndTailColour[g]
if h then
local i=h.specialItemIndicator
if i and an[i]==true then
return true,i
end
end
if an["islandUniqueHairColour"]==true
and at[g]==true then
return true,"islandUniqueHairColour"
end
end
end

if an["mismatchHairColour"]==true then
local e=d.maneColour
local f=d.tailColour
if e and f and e~=f then return true,"mismatchHairColour"end
end

if an["naturallyDyedHairColour"]==true
and c.isNatDyed==true then
return true,"naturallyDyedHairColour"
end

if c.specialItemIndicator
and an[c.specialItemIndicator]==true then
return true,c.specialItemIndicator
end

return false,nil
end




local c=false
local d=nil

local function e(f)
c=(f~=nil)and f or(not c)

if c then
if not d then
d=aj.Bind("Added",function(g,h)
local i,j=pcall(function()
local i,j=b(h)
task.delay(ap,function()
if not c then return end
if i then
ai:FireServer("Inventory","Lock",g)
au(g,h,j)
ae.recordLock(j)


task.delay(ap,function()
if not c then return end
local k=al.GetFollowerInstance()
if k then
al.Interact(k,"Pickup")
else
task.delay(0.3,function()
local l=al.GetFollowerInstance()
if l then
al.Interact(l,"Pickup")
else
warn("[AutoSell] Auto-pickup failed — follower not found for guid:",g)
end
end)
end
end)
else
ai:FireServer("Shopping","QuickSellItem",g)
ae.recordSell()
end
end)
end)
if not i then warn("[AutoSell] Bind callback error:",j)end
end)
end
else
if d then
aj.Unbind(d)
d=nil
end
end
end




return{
setEnabled=function(f)e(f)end,
isEnabled=function()return c end,

getStats=ae.getStats,
resetCounters=ae.reset,
snapshotBalance=ae.snapshotBalance,

setWebhook=function(f)aq=f or""end,
setWebhookEnabled=function(f)ar=f==true end,
testWebhook=function()
au("{test-0000-0000-0000-000000000000}",{
isNatDyed=true,
specialItemIndicator=nil,
variants={
colour="buckskinAp2",
maneColour="grey",
tailColour="brown",
maneStyle="shortAp2",
tailStyle="ratAp2",
hornStyle="forestFlower",
hornColour="pinkForestFlower",
},
},"horned")
end,

setLockOption=function(f,g)
local h=an[f]~=nil
if not h then
for i,j in ipairs(am)do
if j==f then h=true break end
end
end
if not h then warn("[AutoSell] Unknown lock option:",f)return end
an[f]=g
end,
getLockOption=function(f)return an[f]==true end,
getAllLockOptions=function()
local f={}
for g,h in ipairs(am)do f[h]=an[h]==true end
return f
end,
pickupFollower=function()
local f=al.GetFollowerInstance()
if f then al.Interact(f,"Pickup")end
end,

setRareThreshold=function(f)ao=tonumber(f)or ao end,
getRareThreshold=function()return ao end,
setActionDelay=function(f)ap=tonumber(f)or ap end,
getActionDelay=function()return ap end,
checkHorse=function(f)return b(f)end,
getLockOptionNames=function()return am end,
}end function a.h():typeof(aa())local ab=a.cache.h if not ab then ab={c=aa()}a.cache.h=ab end return ab.c end end do local function aa()
local function ab(ac,ad)
local ae=game:GetService("ReplicatedStorage")
local af=game:GetService("RunService")
local ah=game:GetService("Players")
local ai=ah.LocalPlayer

local aj=require(ae.References)
local ak=aj.Utilities
local al=require(aj.PlayerScripts.Priority.Data)
local am=require(aj.PlayerScripts.Secondary:WaitForChild("EquipmentHandler"))

local an={
CFrame.new(3536.414,20.998,-8541.338,-0.986777,0.000000,0.162083,0.000000,1.000000,-0.000000,-0.162083,-0.000000,-0.986777),
CFrame.new(3081.965,20.998,-8160.560,0.430185,0.000000,-0.902741,0.000000,1.000000,0.000000,0.902741,-0.000000,0.430185),
CFrame.new(3040.943,21.278,-7674.188,0.183440,0.000000,-0.983031,0.000000,1.000000,0.000000,0.983031,-0.000000,0.183440),
CFrame.new(3216.217,31.396,-7417.089,-0.624938,0.000000,-0.780675,0.000000,1.000000,-0.000000,0.780675,-0.000000,-0.624938),
CFrame.new(3437.597,20.998,-7198.555,-0.661883,-0.000000,-0.749607,0.000000,1.000000,-0.000000,0.749607,-0.000000,-0.661883),
CFrame.new(3773.379,20.998,-7077.844,-0.083580,-0.000000,-0.996501,-0.000000,1.000000,-0.000000,0.996501,0.000000,-0.083580),
CFrame.new(4060.921,20.998,-7068.069,-0.231128,0.000000,-0.972923,0.000000,1.000000,0.000000,0.972923,-0.000000,-0.231128),
CFrame.new(4364.559,20.998,-7172.213,0.760222,-0.000000,-0.649663,-0.000000,1.000000,-0.000000,0.649663,0.000000,0.760222),
CFrame.new(4582.479,20.998,-7443.499,0.967904,0.000000,0.251320,0.000000,1.000000,-0.000000,-0.251320,0.000000,0.967904),
CFrame.new(4611.561,20.998,-7778.034,0.980821,0.000000,-0.194913,-0.000000,1.000000,-0.000000,0.194913,0.000000,0.980821),
CFrame.new(4505.688,21.692,-8218.074,-0.065622,0.000000,0.997845,-0.000000,1.000000,-0.000000,-0.997845,-0.000000,-0.065622),
CFrame.new(4175.913,21.119,-8487.763,0.747902,0.000000,0.663810,-0.000000,1.000000,0.000000,-0.663810,-0.000000,0.747902),
CFrame.new(3854.704,20.998,-8574.500,0.874451,0.000000,-0.485114,-0.000000,1.000000,0.000000,0.485114,-0.000000,0.874451),
CFrame.new(3382.399,20.998,-8496.440,-0.170347,0.000000,0.985384,-0.000000,1.000000,-0.000000,-0.985384,-0.000000,-0.170347),
CFrame.new(3150.118,27.933,-8297.297,-0.923549,-0.000000,0.383481,-0.000000,1.000000,0.000000,-0.383481,0.000000,-0.923549),
CFrame.new(3583.769,524.847,-7929.497,-0.971305,0.000000,-0.237838,-0.000000,1.000000,0.000000,0.237838,0.000000,-0.971305),
CFrame.new(3740.703,547.900,-8010.287,0.990457,0.000000,-0.137825,-0.000000,1.000000,-0.000000,0.137825,0.000000,0.990457),
CFrame.new(3907.742,526.523,-7917.505,-0.471205,-0.000000,-0.882024,-0.000000,1.000000,-0.000000,0.882024,0.000000,-0.471205),
}


local ao=false
local ap=false
local aq=false
local ar=0.05
local as=5
local at=false
local au=false
local b=false
local c=false

local d={
["Rock"]=false,
["Tin Rock"]=false,
["Copper Rock"]=false,
["Bronze Rock"]=false,
["Iron Rock"]=false,
["Random Crystal"]=false,
["Random Rock"]=false,
["Silver Rock"]=false,
["Gold Rock"]=false,
["Ruby Crystal"]=false,
["Frozen Crystal"]=false,
["Clear Quartz Crystal"]=false,
["Archaeological Deposit"]=false,
["Diamond Crystal"]=false,
["Sapphire Crystal"]=false,
["Topaz Crystal"]=false,
["Amethyst Crystal"]=false,
["Emerald Crystal"]=false,
["Obsidian Rock"]=false,
["Moonstone Rock"]=false,
["Prismatic Crystal"]=false,
["Erupted Deposit"]=false,
}

local e={
"Rock","Tin Rock","Copper Rock","Bronze Rock","Iron Rock",
"Random Crystal","Random Rock","Silver Rock","Gold Rock",
"Ruby Crystal","Frozen Crystal","Clear Quartz Crystal",
"Archaeological Deposit","Diamond Crystal","Sapphire Crystal",
"Topaz Crystal","Amethyst Crystal","Emerald Crystal",
"Obsidian Rock","Moonstone Rock","Prismatic Crystal","Erupted Deposit",
}

local f="itemName"
local g="health"

local h=Vector3.zero
local i=tick()


local j=Instance.new("Highlight")
j.FillColor=Color3.fromRGB(0,255,255)
j.OutlineColor=Color3.fromRGB(255,255,255)
j.FillTransparency=0.5
j.OutlineTransparency=0
j.Parent=game:GetService("CoreGui")
j.Enabled=false


local k=Instance.new("Attachment")
local l=Instance.new("LinearVelocity")
l.Attachment0=k
l.MaxForce=1e6
l.RelativeTo=Enum.ActuatorRelativeTo.World
l.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
l.VectorVelocity=Vector3.zero

local m=120
local n=1.5

local function o(p)
if k.Parent~=p then
k.Parent=p
l.Parent=p
end
end

local function p()
l.VectorVelocity=Vector3.zero
k.Parent=nil
l.Parent=nil
end




local q=nil
local r=false

local function s(t,u,v)
if r then return end
r=true

local w=200
local x=3
local y=12

local z=tick()
local A=t.Parent and t.Parent:FindFirstChildOfClass("Humanoid")
if A then A.PlatformStand=true end

o(t)

if q then q:Disconnect()end
q=af.Heartbeat:Connect(function()
local B=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")

if not B or not l.Parent then
if q then q:Disconnect();q=nil end
if A then A.PlatformStand=false end
p()
r=false
return
end


if tick()-z>y then
q:Disconnect();q=nil
if A then A.PlatformStand=false end
p()
r=false
if v then v()end
return
end

local C=u-B.Position
local D=C.Magnitude

if D<=x then
l.VectorVelocity=Vector3.zero
q:Disconnect();q=nil
if A then A.PlatformStand=false end
p()
r=false
if v then v()end
else
l.VectorVelocity=C.Unit*math.min(D*15,w)
end
end)
end


local t=nil

local function u()
local v=ai.Character
if not v then return end
for w,x in ipairs(v:GetDescendants())do
if x:IsA("BasePart")then
x.CanCollide=false
end
end
end

local function v()
local w=ai.Character
if not w then return end
for x,y in ipairs(w:GetDescendants())do
if y:IsA("BasePart")then
y.CanCollide=true
end
end
end

local function w(x)
c=x
if t then
t:Disconnect()
t=nil
end
if x then



t=af.Heartbeat:Connect(function()
local y=ai.Character
if not y then return end
for z,A in ipairs(y:GetDescendants())do
if A:IsA("BasePart")and A.CanCollide then
A.CanCollide=false
end
end
end)
else
v()
end
end


ai.CharacterAdded:Connect(function()
if c then
task.wait(0.1)



u()
end
end)


local function x()
pcall(function()
if not at then return end
local y=al.GetLocal({"quickEquipment","Harvester"})
if not y then return end
if not au then
au=true
ak.Network:FireServer("QuickEquipment","Use","Harvester")
end
end)
end

local function y()
au=false
task.wait(1.5)
x()
end

am.ObjectChanged:Connect(function(z)
local A=al.GetLocal({"quickEquipment","Harvester"})
local B=al.GetLocal({"temporary","equippedEquipment"})
if B~=A then
au=false
task.wait(0.1)
x()
end
end)

if ai.Character then y()end
ai.CharacterAdded:Connect(y)


local function z()
local A=workspace:FindFirstChild("Islands")
if not A then return nil end
for B,C in ipairs(A:GetChildren())do
if C:FindFirstChild(ai.Name)then return C end
end
return nil
end

local A=nil
local B=nil

local function C()
A=nil
end

local function D(E)
if A and B==E then return A end
local F={}
for G,H in ipairs(E:GetDescendants())do
local I=H:GetAttribute(f)
if H:IsA("Model")and d[I]~=nil then
table.insert(F,H)
end
end
A=F
B=E
E.DescendantRemoving:Connect(C)
return F
end

local function E(F)
local G=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not G then return nil end
local H,I=nil,math.huge
for J,K in ipairs(D(F))do
local L=K:GetAttribute(f)
if d[L]==true then
local M=K:GetAttribute(g)
if M and M>0 then
local N=K.PrimaryPart or K:FindFirstChildWhichIsA("BasePart")
if N then
local O=(G.Position-N.Position).Magnitude
if O<I then
I=O
H=K
end
end
end
end
end
return H
end





local F=nil

local function G()
local H=am.object
if not H or not H.controller then return false end
if H.controller._silentAimInstalled then return true end

local I=H.controller.GetTarget
H.controller.GetTarget=function(J)
if F and F.Parent then
local K=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if K and(K.Position-F:GetPivot().Position).Magnitude<=10 then
local L=F.PrimaryPart
or F:FindFirstChildWhichIsA("BasePart")
return F,{Position=L and L.Position or F:GetPivot().Position}
end
end
return I(J)
end

H.controller._silentAimInstalled=true
return true
end

local function H(I)
local J=am.object
if not J or not J.controller then return false end

F=I
G()

local K=pcall(function()J:Activate()end)
return K
end


task.spawn(function()
while true do
task.wait(1)

if not ao or not aq or b then
i=tick()
continue
end

local I=ai.Character
local J=I and I:FindFirstChild("HumanoidRootPart")
if not J then continue end

if(J.Position-h).Magnitude>2 then
h=J.Position
i=tick()
elseif tick()-i>as then
local K=z()
if K then
local L=(K.Name=="Volcano Island")
and an
or(ad and ad[K.Name])
if L and#L>0 then
local M=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if M then

local N=L[math.random(1,#L)]
s(M,N.Position,nil)
end
end
end
i=tick()
end
end
end)


local I=nil

local function J()
if I then
I:Disconnect()
I=nil
end
p()
b=false
F=nil
j.Enabled=false
end

task.spawn(function()
while true do
task.wait(0.3)

if not ao then
J()
continue
end

local K=z()
if not K then continue end

local L=E(K)
if not L then
J()
continue
end

local M=L.PrimaryPart or L:FindFirstChildWhichIsA("BasePart")
if not M then continue end

if ap then
j.Adornee=L
j.Enabled=true
end

local N=L:GetAttribute(f)=="Erupted Deposit"
local O=N and Vector3.new(0,10,0)or Vector3.new(0,2,2)

J()




if N then
task.wait(3)
if not L.Parent or(L:GetAttribute(g)or 0)<=0 then continue end
end

b=true


I=af.Heartbeat:Connect(function()
local P=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not P then J();return end

if not L or not L.Parent or(L:GetAttribute(g)or 0)<=0 then
J()
return
end

local Q=M.Position+O
o(P)

local R=Q-P.Position
local S=R.Magnitude

if S<=n then
l.VectorVelocity=Vector3.zero
else
l.VectorVelocity=R.Unit*math.min(S*10,m)
end
end)


local P=L:GetAttribute(g)

while ao
and L.Parent
and(L:GetAttribute(g)or 0)>0
and d[L:GetAttribute(f)]==true
do
H(L)


local Q=tick()
while tick()-Q<5 do
task.wait(0.05)
if not L.Parent or(L:GetAttribute(g)or 0)<=0 then break end
local R=L:GetAttribute(g)
if R~=P then
P=R
break
end
end

if ar>0 then
task.wait(ar)
end
end

J()
end
end)


return{
setEnabled=function(K)
ao=K
if not K then
J()
w(false)
end
end,
setPickaxeEnabled=function(K)
at=K
if K then x()end
end,
isEnabled=function()return ao end,
setHighlight=function(K)
ap=K
if not K then j.Enabled=false end
end,
setRandomTeleport=function(K)aq=K end,
setIdleThreshold=function(K)as=tonumber(K)or 5 end,
setClickCooldown=function(K)ar=tonumber(K)or 0.05 end,
setOreTarget=function(K,L)
if d[K]~=nil then d[K]=L end
end,
getOreValues=function()return e end,
setNoclip=function(K)w(K)end,
isNoclipEnabled=function()return c end,
}
end

return ab end function a.i():typeof(aa())local ab=a.cache.i if not ab then ab={c=aa()}a.cache.i=ab end return ab.c end end do local function aa()
local ab=game:GetService("RunService")
local ac=game:GetService("Players")

local ad=ac.LocalPlayer

getgenv().WalkSpeed_Enabled=false
getgenv().WalkSpeed_Value=16
getgenv().JumpPower_Enabled=false
getgenv().JumpPower_Value=50

local ae=nil
local af=nil
local ah=nil
local ai=false
local aj=false





local function ak()
local al=ad.Character
return al and al:FindFirstChildOfClass("Humanoid")
end

local function al(am)
if am==ah then return end


if ah and ah.Parent then
if ae then
ah.WalkSpeed=ae
end
if af then
ah.JumpPower=af
ah.UseJumpPower=false
end
end

if am then
ae=am.WalkSpeed
af=am.JumpPower
else
ae=nil
af=nil
end

ah=am
end




ab.Heartbeat:Connect(function()
local am=ak()
if not am then return end

al(am)

local an=getgenv().WalkSpeed_Enabled
local ao=getgenv().JumpPower_Enabled


if an then
am.WalkSpeed=getgenv().WalkSpeed_Value
elseif ai and not an then
if ae then
am.WalkSpeed=ae
end
end


if ao then
am.UseJumpPower=true
am.JumpPower=getgenv().JumpPower_Value
elseif aj and not ao then
am.UseJumpPower=false
if af then
am.JumpPower=af
end
end

ai=an
aj=ao
end)




return{
setEnabled=function(am)getgenv().WalkSpeed_Enabled=am end,
setValue=function(am)getgenv().WalkSpeed_Value=am end,
setJumpEnabled=function(am)getgenv().JumpPower_Enabled=am end,
setJumpValue=function(am)getgenv().JumpPower_Value=am end,
}end function a.j():typeof(aa())local ab=a.cache.j if not ab then ab={c=aa()}a.cache.j=ab end return ab.c end end do local function aa()
local ab=game:GetService("RunService")
local ac=game:GetService("Players").LocalPlayer

getgenv().MountSpeedEnabled=false
getgenv().MountJumpEnabled=false
getgenv().MountSpeedValue=16
getgenv().MountJumpValue=50

local ad=nil
local ae=nil
local af=nil
local ah=nil


local ai=false
local aj=false




task.spawn(function()
while true do
local ak=ac.Character
local al=ak and ak:FindFirstChild("HumanoidRootPart")
local am=workspace:FindFirstChild("Islands")

if al and am then
local an=nil
local ao=250

for ap,aq in ipairs(am:GetChildren())do
if aq:FindFirstChild(ac.Name)then
for ar,as in ipairs(aq:GetDescendants())do
if as:IsA("Model")then
local at=as:FindFirstChild("HumanoidRootPart")
local au=as:FindFirstChildOfClass("Humanoid")

if at and au and(
as:FindFirstChildWhichIsA("AlignPosition")or
as:FindFirstChildWhichIsA("AlignOrientation")
)then
local b=(al.Position-at.Position).Magnitude
if b<ao then
ao=b
an=au
end
end
end
end
end
end

if an~=ah then

if ah and ah.Parent then
if ae then ah.WalkSpeed=ae end
if af then
ah.UseJumpPower=true
ah.JumpPower=af
end
end

if an then
ae=an.WalkSpeed
af=an.JumpPower
else
ae=nil
af=nil
end

ah=an
ad=an
end
end

task.wait(1)
end
end)



ab.Heartbeat:Connect(function()
local ak=ad
if not ak or not ak.Parent then return end

local al=getgenv().MountSpeedEnabled
local am=getgenv().MountJumpEnabled


if al then
ak.WalkSpeed=getgenv().MountSpeedValue
elseif ai and not al then

if ae then
ak.WalkSpeed=ae
end
end


if am then
ak.UseJumpPower=true
ak.JumpPower=getgenv().MountJumpValue
elseif aj and not am then

if af then
ak.UseJumpPower=true
ak.JumpPower=af
end
end

ai=al
aj=am
end)

return{
setEnabled=function(ak)getgenv().MountSpeedEnabled=ak end,
setValue=function(ak)getgenv().MountSpeedValue=ak end,
setJumpEnabled=function(ak)getgenv().MountJumpEnabled=ak end,
setJumpValue=function(ak)getgenv().MountJumpValue=ak end,
}end function a.k():typeof(aa())local ab=a.cache.k if not ab then ab={c=aa()}a.cache.k=ab end return ab.c end end do local function aa()









local function ab()


local ac=false
local ad=42
local ae=6
local af=20
local ah=5
local ai=0.6
local aj=1.5
local ak=false
local al=false


local am=game:GetService("RunService")
local an=game:GetService("Players")
local ao=an.LocalPlayer


local ap,aq,ar,as,at,au

local function b()
if aq then return true end

local c=game:GetService("ReplicatedStorage")
local d,e=pcall(require,c.References)
if not d then warn("[CC] References not found:",e);return false end

ap=e
aq=ap.Utilities.Network
ar=ap.Utilities.Time

local f=ao.PlayerScripts:FindFirstChild("RidingHandler",true)
if not f then warn("[CC] RidingHandler not found");return false end
as=require(f)

local g=ao.PlayerScripts:FindFirstChild("CheckpointActivityHandler",true)
if not g then warn("[CC] CheckpointActivityHandler not found");return false end
at=require(g)

local h,i=pcall(function()
return workspace.Islands["Training Island"]["Cross Country"].CheckpointActivity
end)
if not h then warn("[CC] CheckpointActivity model not found:",i);return false end
au=i

return true
end


local c={"Collision","BallCollision","HumanoidRootPart"}
local d={"LowerTorso","UpperTorso"}
local e={}

local function f(g)
for h,i in e do pcall(function()i:Disconnect()end)end
e={}
if not ak or not g then return end

for h,i in c do
local j=g.instance:FindFirstChild(i,true)
if j and j:IsA("BasePart")then
j.CanCollide=false
table.insert(e,j:GetPropertyChangedSignal("CanCollide"):Connect(function()
if ak and j.CanCollide then j.CanCollide=false end
end))
end
end

local h=ao.Character
if h then
for i,j in d do
local k=h:FindFirstChild(j,true)
if k and k:IsA("BasePart")then
k.CanCollide=false
table.insert(e,k:GetPropertyChangedSignal("CanCollide"):Connect(function()
if ak and k.CanCollide then k.CanCollide=false end
end))
end
end
end
end

local function g(h)
for i,j in e do pcall(function()j:Disconnect()end)end
e={}
if not h then return end
for i,j in c do
local k=h.instance:FindFirstChild(j,true)
if k and k:IsA("BasePart")then k.CanCollide=true end
end
local i=ao.Character
if i then
for j,k in d do
local l=i:FindFirstChild(k,true)
if l and l:IsA("BasePart")then l.CanCollide=true end
end
end
end


local function h(i)
local j=Instance.new("Attachment")
j.Parent=i
local k=Instance.new("LinearVelocity")
k.Attachment0=j
k.MaxForce=1e6
k.RelativeTo=Enum.ActuatorRelativeTo.World
k.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
k.VectorVelocity=Vector3.zero
k.Parent=i
return k,j
end


local function i()
return as and as.GetCurrentMount and as.GetCurrentMount()
end

local function j(k)
return k and k.instance and k.instance:FindFirstChild("HumanoidRootPart")
end

local function k()
local l={}
for m,n in workspace:GetDescendants()do
if n:IsA("Part")
and n.Shape==Enum.PartType.Ball
and n.Transparency==1
and n.Anchored
and not n.CanCollide
and n:FindFirstChildOfClass("TouchTransmitter")
and n:GetFullName():find("Cross Country")then
table.insert(l,n)
end
end
local m=workspace:FindFirstChild("Part")
if m and m:IsA("Part")and m:FindFirstChildOfClass("TouchTransmitter")then
table.insert(l,m)
end
return l
end

local function l(m,n)
local o,p=nil,math.huge
for q,r in n do
local s=(r.Position-m).Magnitude
if s<p then o,p=r,s end
end
return o,p
end

local function m(n,o)
if o:FindFirstChildOfClass("TouchTransmitter")then
firetouchinterest(n,o,0)
end
end


local n,o,p,q,r,s
local t=false

local function u()
t=false
if p then pcall(function()p:Disconnect()end);p=nil end
if q then pcall(function()q:Disconnect()end);q=nil end
if n then
pcall(function()n.VectorVelocity=Vector3.zero;n:Destroy()end)
n=nil
end
if o then
pcall(function()o:Destroy()end)
o=nil
end
g(s)
s=nil
end

local function v()
aq:FireServer("CheckpointActivity","TriggerInteractable",au)
task.wait(2)
r=0
end

local function w()
if t then return end
if not b()then
warn("[CC] Cannot start — refs failed to resolve")
return
end

local x=i()
if not x then warn("[CC] Mount up before enabling");return end
local y=j(x)
if not y then warn("[CC] No HumanoidRootPart on mount");return end

s=x
t=true
r=0
n,o=h(y)

f(x)
v()


q=at.ActivityChanged:Connect(function()
if not t then return end
task.wait(0.3)
if at.currentObject==nil then
if n then n.VectorVelocity=Vector3.zero end
task.wait(aj)
if t and ac then
local z=i()
if z then f(z)end
v()
end
end
end)


p=am.Heartbeat:Connect(function()
if not ac or not t then
if n then n.VectorVelocity=Vector3.zero end
return
end

local z=i()
if not z then if n then n.VectorVelocity=Vector3.zero end;return end
local A=j(z)
if not A then if n then n.VectorVelocity=Vector3.zero end;return end


local B=os.clock()
if B-r>=ai then
r=B
z.lastTimeJumped=ar.Get()
end

local C=A.Position
local D=k()
local E,F=l(C,D)

if not E then
if n then n.VectorVelocity=Vector3.zero end
return
end

if al then
end

local G=E.Position
local H=F<=af and ae or 0
local I=(Vector3.new(G.X,G.Y+H,G.Z)-C).Unit
n.VectorVelocity=I*ad

if F<=ah then
m(A,E)
end
end)

end


return{
setEnabled=function(x)
ac=x
if x and not t then
w()
elseif not x and t then
u()
end
end,

setNoclip=function(x)
ak=x
if t then
local y=i()
if x then f(y)else g(y)end
end
end,

setMoveSpeed=function(x)ad=x end,
setYBias=function(x)ae=x end,
setCloseDist=function(x)af=x end,
setTriggerDist=function(x)ah=x end,
setJumpInterval=function(x)ai=x end,
setRetriggerDelay=function(x)aj=x end,
setDebug=function(x)al=x end,
stop=u,
}
end

return ab end function a.l():typeof(aa())local ab=a.cache.l if not ab then ab={c=aa()}a.cache.l=ab end return ab.c end end do local function aa()









local function ab()


local ac=false
local ad=0.15
local ae=0.5
local af=3
local ah=0.3
local ai=30
local aj=8
local ak=0.1
local al=120
local am=1.5
local an=false
local ao=false


local ap=game:GetService("RunService")
local aq=game:GetService("Players")
local ar=game:GetService("VirtualUser")
local as=aq.LocalPlayer
local at=workspace.CurrentCamera


local au=false
local b=nil
local c=false
local d=0
local e={}


local f,g,h,i

local function j()
if g then return true end

local k,l=pcall(require,game:GetService("ReplicatedStorage").References)
if not k then warn("[AT] References failed:",l);return false end
f=l
g=f.Utilities

local m=as.PlayerScripts:FindFirstChild("Data",true)
if not m then warn("[AT] Data not found");return false end
local n,o=pcall(require,m)
if not n then warn("[AT] Data require failed:",o);return false end
h=o

local p=as.PlayerScripts:FindFirstChild("CharacterHandler",true)
if not p then warn("[AT] CharacterHandler not found");return false end
local q,r=pcall(require,p)
if not q then warn("[AT] CharacterHandler require failed:",r);return false end
i=r

return true
end


local k={"LowerTorso","UpperTorso","HumanoidRootPart"}

local function l()
for m,n in e do pcall(function()n:Disconnect()end)end
e={}
if not an then return end

local m=as.Character
if not m then return end

for n,o in k do
local p=m:FindFirstChild(o,true)
if p and p:IsA("BasePart")then
p.CanCollide=false
table.insert(e,p:GetPropertyChangedSignal("CanCollide"):Connect(function()
if an and p.CanCollide then
p.CanCollide=false
end
end))
end
end
end

local function m()
for n,o in e do pcall(function()o:Disconnect()end)end
e={}

local n=as.Character
if not n then return end
for o,p in k do
local q=n:FindFirstChild(p,true)
if q and q:IsA("BasePart")then q.CanCollide=true end
end
end


as.CharacterAdded:Connect(function()
task.wait(1.5)
c=false
d=0
if an then l()end
end)


local function n(o)
local p=as.Character
if not p then return end
local q=p:FindFirstChild("HumanoidRootPart")
if not q then return end

local r=p:FindFirstChildOfClass("Humanoid")
if r then r.PlatformStand=true end

local s=Instance.new("Attachment")
s.Parent=q

local t=Instance.new("LinearVelocity")
t.Attachment0=s
t.MaxForce=1e6
t.RelativeTo=Enum.ActuatorRelativeTo.World
t.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
t.VectorVelocity=Vector3.zero
t.Parent=q

local u=false

local v=ap.Heartbeat:Connect(function()
local v=as.Character and as.Character:FindFirstChild("HumanoidRootPart")
if not v or not t.Parent then u=true;return end

local w=o-v.Position
local x=w.Magnitude

if x<am then
t.VectorVelocity=Vector3.zero
u=true
else
t.VectorVelocity=w.Unit*math.min(x*8,al)
end
end)

local w=tick()+8
while not u and tick()<w do
task.wait(0.05)
end

v:Disconnect()
t:Destroy()
s:Destroy()
if r then r.PlatformStand=false end


if an then l()end
end

local function o(p)
n(p+Vector3.new(0,4,0))
end


local function p()
if c then return end
local q=tick()
if(q-d)<ae then return end

local r,s=pcall(function()
local r=h.GetLocal({"quickEquipment","Tool"})
if not r then return end

local s=h.GetLocal({"temporary","equippedEquipment"})
if tostring(s)==tostring(r)then return end

c=true
d=tick()

for t=1,af do
g.Network:FireServer("QuickEquipment","Use","Tool")
task.wait(ah)
local u=h.GetLocal({"temporary","equippedEquipment"})
if tostring(u)==tostring(r)then break end
end

c=false
end)

if not r then
c=false
warn("[AT] equipShovel error:",s)
end
end

local function q()
pcall(function()
g.Network:FireServer(
"Inventory","Use",
h.GetLocal({"quickEquipment","Tool"}),
"Unequip"
)
end)
task.wait(0.4)
c=false
d=0
p()
task.wait(0.4)
end


local function r()
local s,t=pcall(function()
return g.Network:InvokeServer("BuriedTreasure","GetPoint")
end)
if s and t then return t end
return nil
end



local function s(t)
pcall(function()
local u=at.ViewportSize
local v=Vector2.new(u.X/2,u.Y/2)
for w=1,t do
ar:ClickButton1(v,at.CFrame)
task.wait(ak)
end
end)
end


local function t()
au=true

if an then l()end

b=task.spawn(function()
while au and ac do
task.wait(ad)


p()
task.wait(0.6)


local u=nil
local v=0

while not u and v<10 do
if not au or not ac then break end
u=r()
if not u then
v+=1
q()
task.wait(0.5)
end
end

if not u then
if ao then print("[AT] No treasure point found — retrying")end
task.wait(1)
continue
end

if ao then print("[AT] Treasure at",tostring(u))end


o(u)


if not i.object:InRange(u,ai)then
if ao then print("[AT] Not in range — re-teleporting")end
o(u)
end


q()
q()
task.wait(0.3)


s(aj)
task.wait(1.5)


q()
task.wait(1)
end

au=false
if ao then print("[AT] Loop exited.")end
end)
end

local function u()
au=false
b=nil
m()
end


return{
setEnabled=function(v)
ac=v
if v and not au then
if not j()then
warn("[AT] Cannot start — refs failed")
ac=false
return
end
t()
elseif not v and au then
u()
end
end,

setNoclip=function(v)
an=v
if au then
if v then l()else m()end
end
end,

setDigClicks=function(v)aj=v end,
setDigClickDelay=function(v)ak=v end,
setDigRange=function(v)ai=v end,
setTeleSpeed=function(v)al=v end,
setPollInterval=function(v)ad=v end,
setEquipCooldown=function(v)ae=v end,
setDebug=function(v)ao=v end,
stop=u,
}
end

return ab end function a.m():typeof(aa())local ab=a.cache.m if not ab then ab={c=aa()}a.cache.m=ab end return ab.c end end do local function aa()









local function ab()


local ac=false
local ad=42
local ae=6
local af=20
local ah=4
local ai=0.6
local aj=1.5
local ak=false
local al=false


local am=game:GetService("RunService")
local an=game:GetService("Players")
local ao=an.LocalPlayer


local ap,aq,ar,as
local at

local function au()
if ap then return true end

local b=game:GetService("ReplicatedStorage")
local c,d=pcall(require,b.References)
if not c then warn("[SS] References failed:",d);return false end

ap=d.Utilities.Network
aq=d.Utilities.Time

local e=ao.PlayerScripts:FindFirstChild("RidingHandler",true)
if not e then warn("[SS] RidingHandler not found");return false end
ar=require(e)

local f=ao.PlayerScripts:FindFirstChild("CheckpointActivityHandler",true)
if not f then warn("[SS] CheckpointActivityHandler not found");return false end
as=require(f)


local g,h=pcall(function()
return workspace.Islands["Training Island"]["Outdoor Arena"]
.DynamicArena._LAYOUT.CheckpointActivity
end)
if not g or not h then
warn("[SS] CheckpointActivity instance not found:",h)
return false
end
at=h

return true
end


local b={"Collision","BallCollision","HumanoidRootPart"}
local c={"LowerTorso","UpperTorso"}
local d={}

local function e(f)
for g,h in d do pcall(function()h:Disconnect()end)end
d={}
if not ak or not f then return end

for g,h in b do
local i=f.instance:FindFirstChild(h,true)
if i and i:IsA("BasePart")then
i.CanCollide=false
table.insert(d,i:GetPropertyChangedSignal("CanCollide"):Connect(function()
if ak and i.CanCollide then i.CanCollide=false end
end))
end
end

local g=ao.Character
if g then
for h,i in c do
local j=g:FindFirstChild(i,true)
if j and j:IsA("BasePart")then
j.CanCollide=false
table.insert(d,j:GetPropertyChangedSignal("CanCollide"):Connect(function()
if ak and j.CanCollide then j.CanCollide=false end
end))
end
end
end
end

local function f(g)
for h,i in d do pcall(function()i:Disconnect()end)end
d={}
if not g then return end
for h,i in b do
local j=g.instance:FindFirstChild(i,true)
if j and j:IsA("BasePart")then j.CanCollide=true end
end
local h=ao.Character
if h then
for i,j in c do
local k=h:FindFirstChild(j,true)
if k and k:IsA("BasePart")then k.CanCollide=true end
end
end
end


local function g(h)
local i=Instance.new("Attachment")
i.Parent=h
local j=Instance.new("LinearVelocity")
j.Attachment0=i
j.MaxForce=1e6
j.RelativeTo=Enum.ActuatorRelativeTo.World
j.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
j.VectorVelocity=Vector3.zero
j.Parent=h
return j,i
end


local function h()
return ar and ar.GetCurrentMount and ar.GetCurrentMount()
end

local function i(j)
return j and j.instance and j.instance:FindFirstChild("HumanoidRootPart")
end






local function j()
local k=as and as.currentObject
if not k then return nil end


local l=k.actualCheckpointStep or k.checkpointStep
if not l then return nil end


local m={}
for n,o in ipairs(k.checkpoints or{})do
for p,q in ipairs(o.checkpoints or{})do
if q.position then
table.insert(m,{
position=q.position,
size=q.size or 7.5,
})
end
end
end

if#m==0 then return nil end
return m,l
end



local function k()
local l=as and as.currentObject
if not l then return nil end

local m,n=j()
if not m then return nil end

local o=((n-1)%#m)+1
return m[o],o,#m
end


local l,m,n,o,p,q
local r=false

local function s()
r=false
if n then pcall(function()n:Disconnect()end);n=nil end
if o then pcall(function()o:Disconnect()end);o=nil end
if l then
pcall(function()l.VectorVelocity=Vector3.zero;l:Destroy()end)
l=nil
end
if m then pcall(function()m:Destroy()end);m=nil end
f(q)
q=nil
end

local function t()
ap:FireServer("CheckpointActivity","TriggerInteractable",at)
task.wait(2)
p=0
end

local function u()
if r then return end
if not au()then
warn("[SS] Cannot start — refs failed")
return
end

local v=h()
if not v then warn("[SS] Mount up before enabling");return end
local w=i(v)
if not w then warn("[SS] No HumanoidRootPart on mount");return end

q=v
r=true
p=0
l,m=g(w)

e(v)


if not as.currentObject then
t()
end


o=as.ActivityChanged:Connect(function()
if not r then return end
task.wait(0.3)
if as.currentObject==nil then
if l then l.VectorVelocity=Vector3.zero end
task.wait(aj)
if r and ac then
local x=h()
if x then e(x)end
t()
end
end
end)


n=am.Heartbeat:Connect(function()
if not ac or not r then
if l then l.VectorVelocity=Vector3.zero end
return
end

local x=h()
if not x then
if l then l.VectorVelocity=Vector3.zero end
return
end
local y=i(x)
if not y then
if l then l.VectorVelocity=Vector3.zero end
return
end


local z=os.clock()
if z-p>=ai then
p=z
x.lastTimeJumped=aq.Get()
end


if not as.currentObject then
if l then l.VectorVelocity=Vector3.zero end
return
end

local A,B,C=k()
if not A then
if l then l.VectorVelocity=Vector3.zero end
return
end

local D=y.Position
local E=A.position
local F=(E-D).Magnitude
local G=F<=af and ae or 0
local H=(Vector3.new(E.X,E.Y+G,E.Z)-D).Unit

l.VectorVelocity=H*ad

if al then
print(string.format("[SS] cp %d/%d | dist=%.1f | target=%s",B,C,F,tostring(E)))
end
end)
end


return{
setEnabled=function(v)
ac=v
if v and not r then
u()
elseif not v and r then
s()
end
end,

setNoclip=function(v)
ak=v
if r then
local w=h()
if v then e(w)else f(w)end
end
end,

setMoveSpeed=function(v)ad=v end,
setYBias=function(v)ae=v end,
setCloseDist=function(v)af=v end,
setTriggerDist=function(v)ah=v end,
setJumpInterval=function(v)ai=v end,
setRetriggerDelay=function(v)aj=v end,
setDebug=function(v)al=v end,
stop=s,
}
end

return ab end function a.n():typeof(aa())local ab=a.cache.n if not ab then ab={c=aa()}a.cache.n=ab end return ab.c end end end
local aa=os.clock()

local ab=game:GetService("Players")
local ac=ab.LocalPlayer
local ad=ac:WaitForChild("PlayerGui")


local ae=ad:FindFirstChild("Loading")
if ae then
local af=ae:WaitForChild("Main"):WaitForChild("Bottom"):WaitForChild("Play")
repeat task.wait(0.1)until af.Visible==true
firesignal(af.Activated)

else

end


local af=a.a()
local ah=a.b()
local ai=a.c()
local aj=a.d()
local ak=a.e()
local al=a.f()
local am=a.h()
local an=a.i()
local ao=an(m_References,IslandTeleports)
local ap=a.j()
local aq=a.k()
local ar=a.g()
local as=a.l()
local at=as()
local au=a.m()
local b=au()
local c=a.n()
local d=c()

local e=getgenv().Options
local f=getgenv().Toggles

af.ShowToggleFrameInKeybinds=true
af.ShowCustomCursor=true
af.NotifySide="Left"

local g=af:CreateWindow({









Title="fantasyhaxx",
Center=true,
AutoShow=false,
Resizable=true,
ShowCustomCursor=true,
UnlockMouseWhileOpen=true,
NotifySide="Left",
TabPadding=8,
Size=UDim2.new(0,556,0,480),
MenuFadeTime=0.2
})

local h=game:GetService("Players").LocalPlayer
local i=game:GetService("VirtualUser")

h.Idled:Connect(function()
i:Button2Down(Vector2.new(0,0),workspace.CurrentCamera.CFrame)
task.wait(1)
i:Button2Up(Vector2.new(0,0),workspace.CurrentCamera.CFrame)
end)







local j={

HomePageLowkey=g:AddTab("Home"),
Main=g:AddTab("Automation"),
HorseRender=g:AddTab("Render"),
Misc=g:AddTab("Misc"),
["UI Settings"]=g:AddTab("UI Settings"),
}








local k=j.HomePageLowkey:AddRightGroupbox("Session Information")

local l=tick()
local m=k:AddLabel('Time Played: 0s')

task.spawn(function()
while true do
task.wait(1)

local n=math.floor(tick()-l)

local o=math.floor(n/3600)
local p=math.floor((n%3600)/60)
local q=n%60

m:SetText(string.format(
"Time Played: %02dh %02dm %02ds",
o,p,q
))
end
end)

local n=k:AddLabel('Coins Earned: 0')

task.spawn(function()
while true do
task.wait(2)
local o=am.getStats()
n:SetText(string.format("Coins Earned: %d",o.coins))
end
end)

local o=k:AddLabel('Horses Captured: 0')

task.spawn(function()
while true do
task.wait(2)
local p=am.getStats()
o:SetText(string.format("Horses Captured: %d",p.captures))
end
end)

local p=j.Main:AddLeftTabbox()

local q=p:AddTab("Horses")
local r=p:AddTab("Sell")
local s=p:AddTab("Lassos")

q:AddToggle('Autofarm_Enable',{
Text='Enable',
Default=false,
Tooltip='Enables Autofarm',

Callback=function(t)
aj.setEnabled(t)
end
})
























q:AddToggle("test",{
Text="test",
Default=false,
Tooltip="sloppy joe",
Callback=function(t)
aj.setIslandHop(t)
end,
})

q:AddToggle("AutoLasso",{
Text="Lasso",
Default=false,
Tooltip="sloppy joe",
Callback=function(t)
ak.setEnabled(t)
end,
})

q:AddToggle('AutoCapture',{
Text='Capture',
Default=false,
Tooltip='Captures the horse',

Callback=function(t)
al.setEnabled(t)
end
})

q:AddSlider('CaptureRate',{
Text='Capture Rate',
Default=1,
Min=1,
Max=5,
Rounding=2,
Compact=true,
HideMax=true,

Callback=function(t)
al.setDuration(t)
end
})















q:AddToggle('autotravel',{
Text='Travel',
Default=false,
Tooltip='Enables autotravel',

Callback=function(t)
aj.setAutotravel(t)
end
})




local t={
"Mainland",
"Blizzard Island",
"Forest Island",
"Royal Island",
"Desert Island",
"Glacier Island",
"Mountain Island",
"Jungle Island",
"Lunar Islands",
"Volcano Island",
}

q:AddDropdown("IslandSelection",{
Text="Select Islands",
Values=t,
Default={},
Multi=true,
Tooltip="Select which islands to farm",

Callback=function(u)

for v,w in ipairs(t)do
aj.setIsland(w,false)
end

for v,w in pairs(u)do
if w then
aj.setIsland(v,true)
end
end
end,

Disabled=false,
Visible=true,
})

r:AddToggle('Autosell',{
Text='Auto Sell',
Default=false,
Tooltip='Automatically sells horses',

Callback=function(u)
am.setEnabled(u)
end
})

local u={

["Mismatched Hair Colour"]="mismatchHairColour",
["Naturally Dyed Hair"]="naturallyDyedHairColour",
["Island Unique Hair Colour"]="islandUniqueHairColour",
["Special Hair"]="specialHair",



["Special Island Unique Coat"]="specialIslandUniqueCoat",
["Special Coat"]="specialCoat",
["Hybrid Unique Coat"]="hybridUniqueCoat",
["Breeding Unique"]="breedingUnique",
["Co-Breeding Unique"]="cobreedingUnique",
["Legacy Coat"]="legacyCoat",
["Collection Log Reward"]="collectionLogReward",
["Volcanic Minerals Unique"]="volcanicMineralsUnique",
["Training Receipts Unique"]="trainingReceiptsUnique",
["Golden Apple Basket"]="goldenAppleBasket",

["Horned"]="horned",
["Island Unique Horn"]="islandUniqueHorn",

["Event"]="event",

["Summer 2022"]="summer2022",
["Summer 2023"]="summer2023",
["Summer 2024"]="summer2024",
["Summer 2025"]="summer2025",
["Summer 2026"]="summer2026",
["Autumn 2022"]="autumn2022",
["Autumn 2023"]="autumn2023",
["Autumn 2024"]="autumn2024",
["Autumn 2025"]="autumn2025",
["Winter 2022"]="winter2022",
["Winter 2023"]="winter2023",
["Winter 2024"]="winter2024",
["Winter 2025"]="winter2025",
["Spring 2023"]="spring2023",
["Spring 2024"]="spring2024",
["Spring 2025"]="spring2025",
["Spring 2026"]="spring2026",
["Halloween"]="halloween",
["Halloween 2022"]="halloween2022",
["Halloween 2023"]="halloween2023",
["Halloween 2024"]="halloween2024",
["Halloween 2025"]="halloween2025",
["Holiday"]="holiday",
["Holiday 2023"]="holiday2023",
["Holiday 2024"]="holiday2024",
["Holiday 2025"]="holiday2025",
["Valentines"]="valentines",
["Valentines 2022"]="valentines2022",
["Valentines 2023"]="valentines2023",
["Valentines 2024"]="valentines2024",
["Valentines 2025"]="valentines2025",
["Valentines 2026"]="valentines2026",
["Easter 2022"]="easter2022",
["St. Patrick's Day 2022"]="stpatricksday2022",
["St. Patrick's Day 2025"]="stpatricksday2025",
["St. Patrick's Day 2026"]="stpatricksday2026",
["Competition Hub Unique"]="competitionHubUnique",

["Summer Hair"]="summer",
["Autumn Hair"]="autumn",
["Winter Hair"]="winter",
["Spring Hair"]="spring",
["Halloween Hair"]="halloween",
["Holiday Hair"]="holiday",
["Valentines Hair"]="valentines",
["St. Patrick's Day Hair"]="stpatricksday",
["Throwback Hair"]="throwback",
["Training Receipts Hair"]="trainingReceiptsUnique",
["Competition Hub Hair"]="competitionHubUnique",
["Unreleased Event Coat"]="unreleasedEventCoat",
}


local v={}
for w,x in pairs(u)do
v[x]=w
end


local w={}
for x,y in pairs(u)do
table.insert(w,x)
end
table.sort(w)

r:AddDropdown("FilterTypeDropdown",{
Text="Filter",
Values=w,
Default=0,
Multi=true,
Tooltip="Select which horse types to lock instead of sell",

Callback=function(x)

for y,z in pairs(u)do
am.setLockOption(z,false)
end


for y,z in pairs(x)do
if z then
local A=u[y]
if A then
am.setLockOption(A,true)
end
end
end
end,

Disabled=false,
Visible=true,
})

r:AddDivider()

r:AddToggle("WebhookEnabled",{
Text="Use Webhook",
Default=false,
Tooltip="sends a message thru webhook each time a horse is locked",
Callback=function(x)
am.setWebhookEnabled(x)
end,
})


r:AddInput("WebhookURL",{
Text="Webhook URL",
Default="",
Numeric=false,
Finished=false,
Tooltip="discord webhook input",
Placeholder="https://discord.com/api/webhooks/...",
Callback=function(x)
am.setWebhook(x)
end,
})

r:AddButton("Test Webhook",function()
am.testWebhook()
end)
r:AddButton("Pickup Horse",function()
am.pickupFollower()
end)

local x=ak.getCraftableLassos()
local y={}
local z={}

for A,B in ipairs(x)do
local C=B.name.." (str "..B.strength..")"
table.insert(y,C)
z[C]=B.id
end





s:AddToggle("AutoCraftLasso",{
Text="Auto Craft",
Default=false,
Tooltip="makes lassos when ur ur running low",
Callback=function(A)
ak.setCraftEnabled(A)
end,
})

s:AddDropdown("LassoToCraft",{
Text="Lasso Type",
Values=y,
Default=1,
Multi=false,
Tooltip="which lasso to restock",
Callback=function(A)
local B=z[A]
if B then ak.setSelectedLasso(B)end
end,
Disabled=false,
Visible=true,
})

s:AddSlider("LassoRestockThreshold",{
Text="Restock Threshold",
Default=50,
Min=1,
Max=250,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="threshold til buying more lassos",
Callback=function(A)
ak.setRestockThreshold(A)
end,
})

s:AddSlider("LassoRestockAmount",{
Text="Restock Amount",
Default=100,
Min=1,
Max=250,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="how many lassos u want",
Callback=function(A)
ak.setRestockAmount(A)
end,
})

s:AddButton("Restock Now",function()
ak.triggerRestock()
end)


local A=j.Main:AddRightTabbox()

local B=A:AddTab("Ores")
local C=A:AddTab("Pickaxe")
local D=A:AddTab("Render")

B:AddToggle('AutoMine',{
Text='Mine',
Default=false,
Tooltip='Auto mines ores for you',

Callback=function(E)
ao.setEnabled(E)
ao.setNoclip(E)
end
})

B:AddToggle("RandomTP",{
Text="Random Teleport",
Default=false,
Tooltip="Teleports to a random spot on the island when idle",

Callback=function(E)
ao.setRandomTeleport(E)
end,
})

B:AddSlider("ClickCooldown",{
Text="Click Cooldown",
Default=0.05,
Min=0,
Max=1,
Rounding=2,
Compact=true,
HideMax=true,
Tooltip="Delay between clicks in seconds",

Callback=function(E)
ao.setClickCooldown(E)
end,
})


B:AddSlider("IdleThreshold",{
Text="Idle Threshold",
Default=5,
Min=1,
Max=30,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="Seconds idle before random teleport fires",

Callback=function(E)
ao.setIdleThreshold(E)
end,
})

local E=ao.getOreValues()

B:AddDropdown("OreSelector",{
Text="Ore Types",
Values=E,
Default=0,
Multi=true,
Tooltip="Select which ores to mine",

Callback=function(F)

for G,H in ipairs(E)do
ao.setOreTarget(H,false)
end

for G,H in pairs(F)do
if H then
ao.setOreTarget(G,true)
end
end
end,

Disabled=false,
Visible=true,
})

C:AddToggle("EquipPickaxe",{
Text="Pickaxe",
Default=false,
Tooltip="Equips the pickaxe for you automatically",

Callback=function(F)
ao.setPickaxeEnabled(F)
end,
})

local F=game:GetService("ReplicatedStorage")
local G=require(F.References)
local H=G.Utilities.Network





local I={
["Stone Harvester"]={shop="Mainland Shop",idx=4},
["Tin Harvester"]={shop="Mainland Shop",idx=5},
["Copper Harvester"]={shop="Blizzard Island Shop",idx=4},
["Bronze Harvester"]={shop="Blizzard Island Shop",idx=5},
["Iron Harvester"]={shop="Forest Island Shop",idx=4},
["Silver Harvester"]={shop="Forest Island Shop",idx=5},
["Gold Harvester"]={shop="Desert Island Shop",idx=4},
["Ruby Harvester"]={shop="Desert Island Shop",idx=5},
["Topaz Harvester"]={shop="Jungle Island Shop",idx=4},
["Emerald Harvester"]={shop="Jungle Island Shop",idx=5},
["Amethyst Harvester"]={shop="Jungle Island Shop",idx=6},
["Diamond Harvester"]={shop="Mountain Island Shop",idx=4},
["Sapphire Harvester"]={shop="Mountain Island Shop",idx=5},
["Clear Quartz Harvester"]={shop="Royal Island Shop",idx=4},
["Obsidian Harvester"]={shop="Lunar Islands Shop",idx=4},
["Moonstone Harvester"]={shop="Lunar Islands Shop",idx=5},
["Prismatic Harvester"]={shop="Volcano Island Shop",idx=4},
}

local J={
"Harvester",
"Stone Harvester",
"Tin Harvester",
"Copper Harvester",
"Bronze Harvester",
"Iron Harvester",
"Silver Harvester",
"Gold Harvester",
"Ruby Harvester",
"Topaz Harvester",
"Emerald Harvester",
"Amethyst Harvester",
"Diamond Harvester",
"Sapphire Harvester",
"Clear Quartz Harvester",
"Obsidian Harvester",
"Moonstone Harvester",
"Prismatic Harvester",
}

local K=J[1]

C:AddDropdown("PickaxeSelector",{
Text="Pickaxe",
Values=J,
Default=1,
Multi=false,
Tooltip="Select which pickaxe to buy",
Callback=function(L)
K=L
end,
Disabled=false,
Visible=true,
})

C:AddButton("Buy Pickaxe",function()
local L=I[K]
if not L then return end


H:FireServer("Shopping","BuyShopItem",L.shop,L.idx,1,nil)
end)

D:AddToggle("HighlightOre",{
Text="Highlight",
Default=false,
Tooltip="Highlights ore that is being mined",

Callback=function(L)
ao.setHighlight(L)
end,
})

local L=j.Main:AddLeftTabbox()

local M=L:AddTab("Train")

M:AddToggle("AutoTrainEnabled",{
Text="Cross Country",
Default=false,
Tooltip="Automatically completes cross country",
Callback=function(N)
at.setEnabled(N)
at.setNoclip(N)
end,
})

M:AddToggle("AutoSingleStakeEnabled",{
Text="Single Stake",
Default=false,
Tooltip="Automatically completes single stake",
Callback=function(N)
d.setEnabled(N)
d.setNoclip(N)
end,
})

local N=L:AddTab("Settings")

N:AddSlider("TrainSpeed",{
Text="Speed",
Default=42,
Min=20,
Max=80,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="training speed",
Callback=function(O)
at.setMoveSpeed(O)
end,
})

local O=j.Main:AddRightGroupbox("Treasure")

O:AddToggle("TreasureEnabled",{
Text="Enable",
Default=false,
Tooltip="Auto digs treasure",
Callback=function(P)
b.setEnabled(P)
b.setNoclip(P)
end,
})












local P=j.Misc:AddLeftTabbox()

local Q=P:AddTab("Player")
local R=P:AddTab("Horse")

Q:AddToggle("WalkspeedEnabled",{
Text="Walkspeed",
Default=false,
Tooltip="Enhances characters speed",
Callback=function(S)
ap.setEnabled(S)
end,
})

Q:AddSlider("WalkspeedValue",{
Text="Walkspeed Value",
Default=16,
Min=16,
Max=100,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="walkspeed value",
Callback=function(S)
ap.setValue(S)
end,
})

Q:AddToggle("JumpPowerEnabled",{
Text="JumpPower",
Default=false,
Tooltip="Enhances JumpPower",
Callback=function(S)
ap.setJumpEnabled(S)
end,
})

Q:AddSlider("JumpPowerValue",{
Text="JumpPower Value",
Default=50,
Min=0,
Max=1000,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="jumppower value",
Callback=function(S)
ap.setJumpValue(S)
end,
})

R:AddToggle("HWalkspeedEnabled",{
Text="Walkspeed",
Default=false,
Tooltip="Enhances horses speed",
Callback=function(S)
aq.setEnabled(S)
end,
})

R:AddSlider("HWalkspeedValue",{
Text="Walkspeed Value",
Default=16,
Min=16,
Max=100,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="walkspeed value",
Callback=function(S)
aq.setValue(S)
end,
})

R:AddToggle("HJumpPowerEnabled",{
Text="JumpPower",
Default=false,
Tooltip="Enhances horses JumpPower",
Callback=function(S)
aq.setJumpEnabled(S)
end,
})

R:AddSlider("HJumpPowerValue",{
Text="JumpPower Value",
Default=50,
Min=0,
Max=1000,
Rounding=0,
Compact=true,
HideMax=true,
Tooltip="jumppower value",
Callback=function(S)
aq.setJumpValue(S)
end,
})

local S=j.Misc:AddRightGroupbox("Performance")

local T=Instance.new("ScreenGui")
T.Name="BackgroundCover"
T.DisplayOrder=-1
T.IgnoreGuiInset=true
T.Parent=game:GetService("CoreGui")

local U=Instance.new("Frame",T)
U.Size=UDim2.new(1,0,1,0)
U.BackgroundColor3=Color3.fromRGB(0,0,0)
U.ZIndex=1
U.BorderSizePixel=0
U.Visible=false

local V=Instance.new("TextLabel",U)
V.Size=UDim2.new(1,0,1,0)
V.Position=UDim2.new(0,0,0,0)
V.BackgroundTransparency=1
V.TextColor3=Color3.fromRGB(255,255,255)
V.Font=Enum.Font.SourceSansBold
V.TextSize=18
V.ZIndex=2
V.TextXAlignment=Enum.TextXAlignment.Center
V.TextYAlignment=Enum.TextYAlignment.Center
V.TextWrapped=true
V.Text=""


local W={
mismatchHairColour="Mismatch Hair",
summer2026="Summer 2026",
naturallyDyedHairColour="Nat. Dyed",
islandUniqueCoat="IS Coat",
islandUniqueHorn="IS Horn",
islandUniqueHairColour="IUH",
specialIslandUniqueCoat="Special IS Coat",
specialCoat="Special Coat",
specialHair="Special Hair",
horned="Horned",
rareCoat="Rare Coat",
}


local X={
"horned",
"mismatchHairColour",
"naturallyDyedHairColour",
"islandUniqueCoat",
"islandUniqueHorn",
"islandUniqueHairColour",
"specialIslandUniqueCoat",
"specialCoat",
"specialHair",
"rareCoat",
"summer2026",
}


local Y={}
for Z,_ in ipairs(X)do
Y[_]=true
end


local Z={}
for _,av in ipairs(X)do
table.insert(Z,W[av])
end


local av={}
for _,aw in pairs(W)do
av[aw]=_
end




task.spawn(function()
while true do
task.wait(1)

if not U.Visible then continue end
if not am then continue end

local aw,_=pcall(function()return am.getStats()end)
if not aw or not _ then continue end

local ax={}
table.insert(ax,string.format("Sold: %d   Locked: %d   Coins: %d",
_.sold,_.locked,_.coins))
table.insert(ax,"")

for ay,az in ipairs(X)do
if Y[az]then
local aA=_.lockedByReason and _.lockedByReason[az]or 0
table.insert(ax,string.format("%s: %d",W[az],aA))
end
end

V.Text=table.concat(ax,"\n")
end
end)
local aw={}

S:AddToggle('MuteAmbientMusic',{
Text='Ambient Music',
Default=false,
Tooltip='Turns on or off ambient music or sounds',
Callback=function(ax)
local ay=game:GetService("SoundService")
local az=ay:GetDescendants()

for aA,_ in ipairs(az)do
if _:IsA("Sound")then
if ax then

_.Playing=false
else

_.Playing=true
end
end
end
end
})

S:AddToggle('NoGraphics',{
Text='No Graphics',
Default=false,
Tooltip='Disables 3D rendering with a black background',
Callback=function(ax)
do
game:GetService("RunService"):Set3dRenderingEnabled(not ax)
U.Visible=ax
end
end
})

S:AddDropdown("OverlayStatsDisplay",{
Text="Overlay Stats",
Values=Z,
Default=Z,
Multi=true,
Tooltip="Choose which lock types to show on the black screen overlay",

Callback=function(ax)

for ay in pairs(Y)do
Y[ay]=false
end

for ay,az in pairs(ax)do
if az then
local aA=av[ay]
if aA then Y[aA]=true end
end
end
end,

Disabled=false,
Visible=true,
})

local ax=false
local ay=60

S:AddToggle('SetFPS',{
Text='FPS Cap',
Default=false,
Tooltip='Caps the game FPS at the slider value',
Callback=function(az)
do
ax=az
if ax then
setfpscap(ay)
else
setfpscap(0)
end
end
end
})

S:AddSlider('FPSCap',{
Text='FPS Cap Value',
Default=60,
Min=1,
Max=240,
Rounding=1,
Compact=false,
Callback=function(az)
do
ay=az
if ax then
setfpscap(az)
end
end
end
})

local az=j.Misc:AddLeftGroupbox("Redeem")




az:AddButton("Redeem Codes",function()
task.spawn(function()
local aA=game:GetService("ReplicatedStorage")
local _=require(aA:WaitForChild("References"))
local aB=_.Flags
local aC=require(_.PlayerScripts.Priority:WaitForChild("Data"))
local aD=_.Utilities.Network

local aE=aB.flags and aB.flags.codes or{}
local aF,aG=0,0

for aH,aI in pairs(aE)do
if aC.GetLocal({"codesRedeemed",aH})==true then
aG+=1
else
aD:FireServer("Codes","Submit",aH)
af:Notify("Submitted: "..aH,5)

aF+=1
task.wait(1.5)
end
end


end)
end)








local function aA(aB,aC,aD)
task.spawn(function()
local aE=game:GetService("ReplicatedStorage")
local aF=require(aE:WaitForChild("References"))
local aG=aF.Utilities.Network


for aH=1,aD do
local aI,_=pcall(function()
return aG:InvokeServer("TradeIn","Trade",aB)
end)
if aI and _ then

else

end
if aH<aD then task.wait(0.8)end
end

end)
end




az:AddButton("Golden Apples (20)",function()
aA("goldenAppleBasket","Golden Apples",1)
end)

az:AddButton("Volcanic Minerals (5)",function()
aA("volcanicMinerals","Volcanic Minerals",1)
end)

az:AddButton("Training Receipts (100)",function()
aA("trainingReceipts","Training Receipts",1)
end)













local aB=j.Misc:AddRightGroupbox("Teleport")




local aC=game:GetService("ReplicatedStorage")
local aD=require(aC:WaitForChild("References"))
local aE=require(aD.PlayerScripts:WaitForChild("Secondary"):WaitForChild("TravelHandler"))

local aF={

"Mainland",
"Blizzard Island",
"Forest Island",
"Royal Island",
"Desert Island",
"Glacier Island",
"Mountain Island",
"Jungle Island",
"Lunar Islands",
"Volcano Island",

"Training Island",
"Rescue Island",
}

local aG={
["Mainland"]=8,
["Blizzard Island"]=1,
["Forest Island"]=1,
["Royal Island"]=1,
["Desert Island"]=1,
["Glacier Island"]=1,
["Mountain Island"]=1,
["Jungle Island"]=1,
["Lunar Islands"]=1,
["Volcano Island"]=1,
["Training Island"]=1,
["Rescue Island"]=1,

["Stable Island"]=1,
["Competition Hub"]=1,
["Breeding Hub"]=1,
["Trading Hub"]=1,
["RP Island"]=1,
["Wild Island"]=1,
}

local function aH(aI)
if not aI then return end
local _=aG[aI]or 1
pcall(function()
aE.Travel(aI,_)
end)
end

aB:AddDropdown("IslandTravel",{
Text="Travel to Island",
Values=aF,
Default=1,
Multi=false,
Tooltip="Select an island to travel to",
Callback=function(aI)
selected_island=aI
end,
Disabled=false,
Visible=true,
})

aB:AddButton("Travel",function()
aH(selected_island)
end)

af:SetWatermarkVisibility(true)

local aI=tick()
local _=0;
local aJ=60;
local aK=(function()return math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())end)
local aL=pcall(function()return aK()end)

local aM=game:GetService("RunService").RenderStepped:Connect(function()
_+=1;

if(tick()-aI)>=1 then
aJ=_;
aI=tick();
_=0;
end;

if aL then
af:SetWatermark(("fantasyhaxx - [buyer build] / %d fps / %d ms"):format(
math.floor(aJ),
aK()
));
else
af:SetWatermark(("fantasyhaxx - [buyer build] / %d fps"):format(
math.floor(aJ)
));
end
end);

af:OnUnload(function()
aM:Disconnect()
af.Unloaded=true
end)


local aN=j["UI Settings"]:AddLeftGroupbox("Menu")




aN:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind",{Default="RightShift",NoUI=true,Text="Menu keybind"})
aN:AddButton("Unload",function()af:Unload()end)

af.ToggleKeybind=e.MenuKeybind






ah:SetLibrary(af)
ai:SetLibrary(af)



ai:IgnoreThemeSettings()



ai:SetIgnoreIndexes({"MenuKeybind"})




ah:SetFolder("coconut")
ai:SetFolder("coconut/whi")






ai:BuildConfigSection(j["UI Settings"])



ah:ApplyToTab(j["UI Settings"])




ai:LoadAutoloadConfig()

af:Notify("Loaded in "..tostring(string.format("%."..tostring(3).."f",os.clock()-aa)).." seconds",5)
