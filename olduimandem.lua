...
Options.SecondAccentColor:OnChanged(af)
...
for p,q in ipairs(n:GetDescendants())do
...
local n=ae.Character
if not n then continue end
local o=n:FindFirstChild("UpperTorso")
local p=n:FindFirstChild("LowerTorso")
if o and o.CanCollide then o.CanCollide=false end
...
k=p:FindFirstAncestorOfClass("Model")
l=q
...
i=ac.Heartbeat:Connect(function()
...
local u=ae.Character and ae.Character:FindFirstChild("HumanoidRootPart")
if not u or not h then return end

local v=p.Position+Vector3.new(0,ai,0)
local w=v-u.Position
local x=w.Magnitude
...
h.VectorVelocity=w.Unit*math.min(x*60,e)
end)
end

ae.CharacterAdded:Connect(function(p)
n()
task.wait(1.5)
m(p,false)
c=false
...
local r=ap[q]or 1
pcall(function()ah.Travel(q,r)end)
end

local function q()
local r=workspace:FindFirstChild("Islands")
if not r then return nil end
for s,t in ipairs(r:GetChildren())do
if t:FindFirstChild(ae.Name)then return t end
end
return nil
end
...
local v=u[math.random(1,#u)].Position
...
F.Parent=s
E=Instance.new("LinearVelocity")
E.Attachment0=F
E.MaxForce=1e6
E.RelativeTo=Enum.ActuatorRelativeTo.World
E.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
E.VectorVelocity=Vector3.zero
E.Parent=s
C,D=E,F
end
...
local H
H=ac.Heartbeat:Connect(function()
if tick()-d>y then
H:Disconnect()
G()
return
...
if not J or not C or not C.Parent then
H:Disconnect()
G()
return
end

local K=v-J.Position
local L=K.Magnitude

if L<A then
C.VectorVelocity=Vector3.zero
H:Disconnect()
G()
else
C.VectorVelocity=K.Unit*math.min(L*8,z)
end
end)
end
...
if b then
local x=u:FindFirstChild("WildHerdSpawners")
if x then
...
if C then
local D=(t.Position-C.Position).Magnitude
if D<w then
w=D
v=C
end
...
for x,y in ipairs(u:GetDescendants())do
if y:IsA("Model")then
local z=y:FindFirstChild("HumanoidRootPart")
local A=y:FindFirstChild("CaptureProgress",true)
if z and A then
local B=(t.Position-z.Position).Magnitude
if B<w then w=B;v=z end
end
end
end
...
local function t(u)
local v={}
for w,x in ipairs(ao)do
if at[x]then table.insert(v,x)end
end
if#v==0 then return nil end
if#v==1 then return v[1]end
for w,x in ipairs(v)do
if x==u then return v[(w%#v)+1]end
end
return v[1]
end

local function u()
local v=workspace:FindFirstChild("Islands")
if not v then return end
local w=v:FindFirstChild("Volcano Island")
if not w then return end
local x=w:FindFirstChild("LavaParts")
if not x then return end
for y,z in ipairs(x:GetDescendants())do
if z:IsA("TouchTransmitter")then z:Destroy()end
end
end
...
local w=q()
if w and w.Name=="Volcano Island"and v~="Volcano Island"then
v="Volcano Island"
u()
elseif not w or w.Name~="Volcano Island"then
v=w and w.Name or nil
end
end)
task.spawn(function()
while true do u();task.wait(3)end
end)
end
...
if not D.Parent then return false end
local E=D:FindFirstAncestorOfClass("Model")
...
task.wait(1)
local I=q()
if I and I.Name==F then break end
until tick()>H
task.wait(2)
y=false
end
...
return{
setEnabled=function(v)al=v end,
setWildherd=function(v)b=v end,
getIslands=function()
local v={}
for w,x in pairs(at)do v[w]=x end
return v
end,
setAutotravel=function(v)au=v end,
setIsland=function(v,w)at[v]=w end,
setIdleLimit=function(v)am=v end,
...
if not i then warn("Bind callback error:",j)end
end)
end
else
if d then
...
if y:IsA("BasePart")then y.CanCollide=true end
end
end
...
local function E(F)
if not F:IsA("Model")then return false end
local G=F:GetAttribute(f)
return G~=nil and d[G]~=nil
end
...
for H,I in ipairs(G:GetDescendants())do
if E(I)then A[I]=true end
end


C=G.DescendantAdded:Connect(function(H)
task.defer(function()
if E(H)then A[H]=true end
end)
end)

D=G.DescendantRemoving:Connect(function(H)
A[H]=nil
end)
end
...
local J,K=nil,math.huge

for L in pairs(A)do
if not L.Parent then A[L]=nil;continue end

local M=L:GetAttribute(f)
if d[M]~=true then continue end

local N=L:GetAttribute(g)
if not N or N<=0 then continue end



local O
if M=="Erupted Deposit"then
local P,Q=pcall(function()return L:GetBoundingBox()end)
O=P and Q and Q.Position or L:GetPivot().Position
else
local P=L.PrimaryPart or L:FindFirstChildWhichIsA("BasePart")
O=P and P.Position or L:GetPivot().Position
end

local P=(I.Position-O).Magnitude
if P<K then
K=P
J=L
end
...
local H=cloneref(game:GetService("VirtualInputManager"))
...
local M=workspace.CurrentCamera.CFrame
workspace.CurrentCamera.CFrame=M:Lerp(
CFrame.new(M.Position,K),
I
)
af.Heartbeat:Wait()
end
...
local N=L.PrimaryPart or L:FindFirstChildWhichIsA("BasePart")
if not N then return false end

local O=N.Position


if L:GetAttribute(f)=="Erupted Deposit"then
local P,Q=pcall(function()return L:GetBoundingBox()end)
if P and Q then O=Q.Position end
end

local P,Q=M:WorldToViewportPoint(O)
if not Q or P.Z<=0 then return false end

local R=math.floor(P.X)
local S=math.floor(P.Y)

pcall(function()
H:SendMouseMoveEvent(R,S,game,0)
H:SendMouseButtonEvent(R,S,0,true,game,0)
task.wait(0.05)
H:SendMouseButtonEvent(R,S,0,false,game,0)
end)
...
local L=ai.Character
local M=L and L:FindFirstChild("HumanoidRootPart")
if not M then continue end
if(M.Position-h).Magnitude>2 then
h=M.Position
i=tick()
elseif tick()-i>as then
local N=z()
if N then
local O=(N.Name=="Volcano Island")
and an
or(ad and ad[N.Name])
if O and#O>0 then
...
if not ao then M();continue end

local N=z()
if not N then continue end

local O=G(N)
if not O then M();continue end

local P=O:GetAttribute(f)=="Erupted Deposit"
...
until(O:GetAttribute(g)or 0)>0 or tick()-Q>6
if not O.Parent or(O:GetAttribute(g)or 0)<=0 then continue end
end


local Q
if P then
local R,S=pcall(function()return O:GetBoundingBox()end)
Q=R and S and S.Position or O:GetPivot().Position
else
local R=O.PrimaryPart or O:FindFirstChildWhichIsA("BasePart")
Q=R and R.Position or O:GetPivot().Position
end

local R=P and Vector3.new(0,6,0)or Vector3.new(0,2,2)
local S=Q+R
...
L=af.Heartbeat:Connect(function()
local T=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not T then M();return end
if not O or not O.Parent or(O:GetAttribute(g)or 0)<=0 then
M();return
end
...
local U
if P then
local V,W=pcall(function()return O:GetBoundingBox()end)
U=(V and W and W.Position or O:GetPivot().Position)+R
else
U=S
end

o(T)
local V=U-T.Position
local W=V.Magnitude
if W<=n then
l.VectorVelocity=Vector3.zero
else
l.VectorVelocity=V.Unit*math.min(W*10,m)
end
end)
...
repeat
task.wait(0.05)
local U=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not U or not O.Parent then break end
local V=P
and((function()
local V,W=pcall(function()return O:GetBoundingBox()end)
return(V and W and W.Position or O:GetPivot().Position)+R
end)())
or S
if(U.Position-V).Magnitude<=n+1 then break end
until tick()-T>8

local U=O:GetAttribute(g)

while ao
and O.Parent
and(O:GetAttribute(g)or 0)>0
and d[O:GetAttribute(f)]==true
do
...
while tick()-V<1 do
task.wait(0.05)
if not O.Parent or(O:GetAttribute(g)or 0)<=0 then break end
if O:GetAttribute(g)~=U then
U=O:GetAttribute(g)
break
end
end
...
ao=N
if not N then M();w(false)end
end,
setPickaxeEnabled=function(N)
at=N
if N then x()end
end,
...
ap=N
if not N then j.Enabled=false end
end,
setRandomTeleport=function(N)aq=N end,
setIdleThreshold=function(N)as=tonumber(N)or 5 end,
setClickCooldown=function(N)ar=tonumber(N)or 0.05 end,
setOreTarget=function(N,O)
if d[N]~=nil then d[N]=O end
end,
getOreValues=function()return e end,
setNoclip=function(N)w(N)end,
...
local k=j.HomePageLowkey:AddLeftGroupbox("User Info")

local l=k:AddLabel('fantasyhaxx tells you to put on that misery')
...
local m=j.HomePageLowkey:AddRightGroupbox("Session Information")

local n=tick()
local o=m:AddLabel('Time Played: 0s')

task.spawn(function()
while true do
task.wait(1)

local p=math.floor(tick()-n)

local q=math.floor(p/3600)
local r=math.floor((p%3600)/60)
local s=p%60

o:SetText(string.format(
...
local p=m:AddLabel('Coins Earned: 0')

task.spawn(function()
while true do
task.wait(2)
local q=am.getStats()
p:SetText(string.format("Coins Earned: %d",q.coins))
end
end)

local q=m:AddLabel('Horses Captured: 0')

task.spawn(function()
while true do
task.wait(2)
local r=am.getStats()
q:SetText(string.format("Horses Captured: %d",r.captures))
end
end)

local r=j.Main:AddLeftTabbox()

local s=r:AddTab("Horses")
local t=r:AddTab("Sell")
local u=r:AddTab("Lassos")

s:AddToggle('Autofarm_Enable',{
Text='Enable',
Default=false,
...
if not v[A.Name]then
table.insert(x,A.Name)
end
end
end
table.sort(x)
...
t:AddDropdown("FilterTypeDropdown",{
Text="Filter",
Values=A,
Default=0,
Multi=true,
...
t:AddButton("Pickup Horse",function()
am.pickupFollower()
end)

t:AddDivider()

t:AddToggle("WebhookEnabled",{
Text="Use Webhook",
Default=false,
Tooltip="sends a message thru webhook each time a horse is locked",
Callback=function(B)
am.setWebhookEnabled(B)
end,
})


t:AddInput("WebhookURL",{
Text="Webhook URL",
Default="",
Numeric=false,
Finished=false,
...
t:AddButton("Test Webhook",function()
am.testWebhook()
end)

local B=ak.getCraftableLassos()
local C={}
local D={}

for E,F in ipairs(B)do
local G=F.name.." (str "..F.strength..")"
table.insert(C,G)
D[G]=F.id
end

u:AddToggle("AutoCraftLasso",{
Text="Auto Craft",
Default=false,
Tooltip="makes lassos when ur ur running low",
Callback=function(E)
ak.setCraftEnabled(E)
end,
})

u:AddDropdown("LassoToCraft",{
Text="Lasso Type",
Values=C,
Default=1,
Multi=false,
...
Callback=function(E)
local F=D[E]
if F then ak.setSelectedLasso(F)end
end,
Disabled=false,
Visible=true,
})

u:AddSlider("LassoRestockThreshold",{
...
Callback=function(E)
ak.setRestockThreshold(E)
end,
})

u:AddSlider("LassoRestockAmount",{
Text="Restock Amount",
Default=100,
Min=1,
Max=250,
...
Callback=function(E)
ak.setRestockAmount(E)
end,
})

u:AddButton("Restock Now",function()
ak.triggerRestock()
end)


local E=j.Main:AddRightTabbox()

local F=E:AddTab("Ores")
local G=E:AddTab("Pickaxe")
local H=E:AddTab("Render")

F:AddToggle('AutoMine',{
Text='Mine',
Default=false,
Risky=true,
...
Callback=function(I)
ao.setClickCooldown(I)
end,
})


F:AddSlider("IdleThreshold",{
Text="Idle Threshold",
Default=5,
Min=1,
Max=30,
...
Callback=function(I)
ao.setIdleThreshold(I)
end,
})

local I=ao.getOreValues()

F:AddDropdown("OreSelector",{
Text="Ore Types",
Values=I,
Default=0,
Multi=true,
...
ao.setOreTarget(L,false)
end

for K,L in pairs(J)do
if L then
ao.setOreTarget(K,true)
end
end
end,
...
local K=require(J.References)
local L=K.Utilities.Network
...
G:AddButton("Buy Pickaxe",function()
local P=M[O]
if not P then return end


L:FireServer("Shopping","BuyShopItem",P.shop,P.idx,1,nil)
end)

H:AddToggle("HighlightOre",{
Text="Highlight",
Default=false,
...
local Q=P:AddTab("Train")

Q:AddToggle("AutoTrainEnabled",{
Text="Cross Country",
Default=false,
...
Q:AddToggle("AutoSingleStakeEnabled",{
Text="Single Stake",
Default=false,
...
local R=P:AddTab("Settings")

R:AddSlider("TrainSpeed",{
Text="Speed",
Default=42,
Min=20,
Max=80,
...
local S=j.Main:AddRightGroupbox("Treasure")

S:AddToggle("TreasureEnabled",{
Text="Enable",
Default=false,
...
local T=j.Misc:AddLeftTabbox()

local U=T:AddTab("Player")
local V=T:AddTab("Horse")

U:AddToggle("WalkspeedEnabled",{
Text="Walkspeed",
Default=false,
...
local W=j.Misc:AddRightGroupbox("Performance")

local X=Instance.new("ScreenGui")
X.Name="BackgroundCover"
X.DisplayOrder=-1
...
local Z=Instance.new("TextLabel",Y)
...
table.insert(ax,_[az])
end

local ay={}
for az,aA in pairs(_)do
ay[aA]=az
end
...
if not Y.Visible then continue end
if not am then continue end
...
table.insert(aB,string.format("%s: %d",_[aD],aE))
end
end

Z.Text=table.concat(aB,"\n")
end
end)
local az={}

W:AddToggle('MuteAmbientMusic',{
Text='Ambient Music',
Default=false,
...
W:AddDropdown("OverlayStatsDisplay",{
Text="Overlay Stats",
Values=ax,
Default=ax,
Multi=true,
...
local aC=j.Misc:AddLeftGroupbox("Redeem")
...
local aE=j.Misc:AddLeftGroupbox("Inherited")
...
local aF=j.Misc:AddRightGroupbox("Teleport")
...
local aS=j["UI Settings"]:AddLeftGroupbox("Menu")
...
af:Unload()
task.wait(2)
loadstring(game:HttpGet("https://raw.githubusercontent.com/bananasxyz/satanas/refs/heads/main/mainuimandem.lua"))()
end)
