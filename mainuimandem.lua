...
writefile(
"LoRes12OT-Regular.ttf",
game:HttpGet("https://github.com/sudoaptinstallopsec123/new/raw/refs/heads/main/LoRes12OT-Regular.ttf")
)

writefile(
"LoRes12OT-Regular.font",
cloneref(game:GetService("HttpService")):JSONEncode({
name="LoRes12OT-Regular",
faces={
{
name="Regular",
weight=400,
style="normal",
assetId=getcustomasset("LoRes12OT-Regular.ttf"),
},
{
name="Bold",
weight=700,
...
local o=Font.new(getcustomasset("Tahoma.font"),Enum.FontWeight.Regular)
local p=o
...
pcall(function()t.BlurEffect.Parent=j end)

function t:UpdateBlur()
if t.UseBlur then
if t.Toggled then
t.BlurEffect.Enabled=true
i:Create(t.BlurEffect,TweenInfo.new(0.2,Enum.EasingStyle.Linear),{Size=t.BlurSize}):Play()
end
else
local u=i:Create(t.BlurEffect,TweenInfo.new(0.2,Enum.EasingStyle.Linear),{Size=0})
u:Play()

task.delay(0.2,function()
if not t.UseBlur then
t.BlurEffect.Enabled=false
end
end)
end
...
local v=e:FindFirstChild("LinoriaMobileUI")
if v then
...
table.insert(t.Signals,k:Connect(function(w)
u=u+w
...
if B=='Font'and typeof(C)=='Font'then
A.FontFace=C;
else
A[B]=C;
end;
...
H=c.InputChanged:Connect(function(J)
...
I=c.InputEnded:Connect(function(J)
...
I=c.InputChanged:Connect(function(K)
...
C.Position=UDim2.fromOffset(m.X+15,m.Y+12)
C.Visible=true
...
local y=TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)

local function z(A,B)
local C=i:Create(A,y,B)
C:Play()
return C
end

function t:OnHighlight(A,B,C,D)
local E,F

A.MouseEnter:Connect(function()
if F then F:Cancel()end

local G=t.RegistryMap[B]
local H={}
...
for A,B in next,t.OpenedFrames do
local C,D=A.AbsolutePosition,A.AbsoluteSize;
if m.X>=C.X and m.X<=C.X+D.X
and m.Y>=C.Y and m.Y<=C.Y+D.Y then
...
function t:IsMouseOverFrame(A)
local B,C=A.AbsolutePosition,A.AbsoluteSize;
if m.X>=B.X and m.X<=B.X+C.X
and m.Y>=B.Y and m.Y<=B.Y+C.Y then
...
for A,B in next,t.DependencyBoxes do
B:Update();
end;
end;

function t:MapValue(A,B,C,D,E)
return(1-((A-B)/(C-B)))*D+((A-B)/(C-B))*E
end;

function t:GetTextBounds(A,B,C,D)
local E=Instance.new("GetTextBoundsParams")
E.Text=A
E.Font=typeof(B)=='Font'and B or Font.new("rbxasset://fonts/families/LegacyArial.json")
E.Size=C
E.Width=math.huge

local F,G=pcall(function()
return d:GetTextBoundsAsync(E)
end)
...
local H=d:GetTextSize(A,C,Enum.Font.Code,Vector2.new(math.huge,math.huge))
return H.X,H.Y
end

function t:GetDarkerColor(A)
local B,C,D=Color3.toHSV(A);
return Color3.fromHSV(B,C,D/1.5)
end;

t.AccentColorDark=t:GetDarkerColor(t.AccentColor);

function t:AddToRegistry(A,B,C)
...
table.insert(t.Registry,E);
t.RegistryMap[A]=E;

if C then
table.insert(t.HudRegistry,E);
end;
end;

function t:RemoveFromRegistry(A)
local B=t.RegistryMap[A];

if B then
for C=#t.Registry,1,-1 do
if t.Registry[C]==B then
table.remove(t.Registry,C);
end;
end;

for C=#t.HudRegistry,1,-1 do
if t.HudRegistry[C]==B then
table.remove(t.HudRegistry,C);
end;
end;
...
for A,B in next,t.Registry do
for C,D in next,B.Properties do
...
function t:GiveSignal(A)
table.insert(t.Signals,A)
end

function t:Unload()
for A=#t.Signals,1,-1 do
local B=table.remove(t.Signals,A)
B:Disconnect()
end
...
function t:OnUnload(A)
t.OnUnload=A
end

t:GiveSignal(q.DescendantRemoving:Connect(function(A)
if t.RegistryMap[A]then
t:RemoveFromRegistry(A);
end;
end))
...
function B:AddColorPicker(C,D)
local E=self.TextLabel;
assert(D.Default,'AddColorPicker: Missing default value.');

local F={
Value=D.Default;
Transparency=D.Transparency or 0;
Type='ColorPicker';
Title=type(D.Title)=='string'and D.Title or'Color picker',
Callback=D.Callback or function(F)end;
};

function F:SetHSVFromRGB(G)
local H,I,J=Color3.toHSV(G);
F.Hue=H;
F.Sat=I;
F.Vib=J;
end;

F:SetHSVFromRGB(F.Value);
local G=t:Create('Frame',{
BackgroundColor3=F.Value;
BorderColor3=t:GetDarkerColor(F.Value);
...
local H=t:Create('ImageLabel',{
BorderSizePixel=0;
Size=UDim2.new(0,27,0,13);
ZIndex=5;
Image='http://www.roblox.com/asset/?id=12977615774';
Visible=not not D.Transparency;
Parent=G;
});
...
Position=UDim2.fromOffset(G.AbsolutePosition.X,G.AbsolutePosition.Y+18),
Size=UDim2.fromOffset(230,D.Transparency and 271 or 253);
Visible=false;
ZIndex=15;
Parent=q,
});
G:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
I.Position=UDim2.fromOffset(G.AbsolutePosition.X,G.AbsolutePosition.Y+18);
end)
...
local P=t:Create('ImageLabel',{
Size=UDim2.new(0,O.Size.X.Offset-2,0,O.Size.Y.Offset-2);
...
local W=t:Create(T:Clone(),{
Position=UDim2.new(0.5,2,0,228),
Size=UDim2.new(0.5,-6,0,20),
Parent=J
});
local X=t:Create(W.Frame:FindFirstChild('TextBox'),{
Text='255, 255, 255',
...
t:AddToRegistry(Z,{BorderColor3='OutlineColor'});
...
local function ac()
ab.Container.Position=UDim2.fromOffset(
(G.AbsolutePosition.X+G.AbsoluteSize.X)+4,
G.AbsolutePosition.Y+1
)
end

local function ad()
local ae=60
for af,ag in next,ab.Inner:GetChildren()do
if ag:IsA('TextLabel')then
ae=math.max(ae,ag.TextBounds.X)
end
end

ab.Container.Size=UDim2.fromOffset(
ae+8,
ab.Inner.Layout.AbsoluteContentSize.Y+4
)
end

G:GetPropertyChangedSignal('AbsolutePosition'):Connect(ac)
ab.Inner.Layout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(ad)

task.spawn(ac)
task.spawn(ad)

t:AddToRegistry(ab.Inner,{
...
function ab:AddOption(ae,af)
if type(af)~='function'then
af=function()end
end

local ag=t:CreateLabel({
Active=false;
...
if ah.UserInputType~=Enum.UserInputType.MouseButton1 and ah.UserInputType~=Enum.UserInputType.Touch then
return
end
...
ab:AddOption('Copy color',function()
t.ColorClipboard=F.Value
t:Notify('Copied color!',2)
end)

ab:AddOption('Paste color',function()
if not t.ColorClipboard then
return t:Notify('You have not copied a color!',2)
end
F:SetValueRGB(t.ColorClipboard)
end)


ab:AddOption('Copy HEX',function()
pcall(setclipboard,F.Value:ToHex())
t:Notify('Copied hex code to clipboard!',2)
end)

ab:AddOption('Copy RGB',function()
pcall(setclipboard,table.concat({math.floor(F.Value.R*255),math.floor(F.Value.G*255),math.floor(F.Value.B*255)},', '))
t:Notify('Copied RGB values to clipboard!',2)
end)

end

t:AddToRegistry(J,{BackgroundColor3='BackgroundColor';BorderColor3='OutlineColor';});
t:AddToRegistry(K,{BackgroundColor3='AccentColor';});
t:AddToRegistry(M,{BackgroundColor3='BackgroundColor';BorderColor3='OutlineColor';});
t:AddToRegistry(U,{BackgroundColor3='MainColor';BorderColor3='OutlineColor';});
t:AddToRegistry(W.Frame,{BackgroundColor3='MainColor';BorderColor3='OutlineColor';});
t:AddToRegistry(X,{TextColor3='FontColor',});
t:AddToRegistry(V,{TextColor3='FontColor',});
...
local ad=t:Create('UIGradient',{
Color=ColorSequence.new(ac);
Rotation=90;
Parent=R;
});
V.FocusLost:Connect(function(ae)
if ae then
local af,ag=pcall(Color3.fromHex,V.Text)
if af and typeof(ag)=='Color3'then
F.Hue,F.Sat,F.Vib=Color3.toHSV(ag)
end
end
...
X.FocusLost:Connect(function(ae)
if ae then
local af,ag,ah=X.Text:match('(%d+),%s*(%d+),%s*(%d+)')
if af and ag and ah then
F.Hue,F.Sat,F.Vib=Color3.toHSV(Color3.fromRGB(af,ag,ah))
end
end
...
function F:Display()
F.Value=Color3.fromHSV(F.Hue,F.Sat,F.Vib);
N.BackgroundColor3=Color3.fromHSV(F.Hue,1,1);

t:Create(G,{
BackgroundColor3=F.Value;
BackgroundTransparency=F.Transparency;
BorderColor3=t:GetDarkerColor(F.Value);
});
if Z then
Z.BackgroundColor3=F.Value;
_.Position=UDim2.new(1-F.Transparency,0,0,0);
end;

O.Position=UDim2.new(F.Sat,0,1-F.Vib,0);
S.Position=UDim2.new(0,0,F.Hue,0);

V.Text='#'..F.Value:ToHex()
X.Text=table.concat({math.floor(F.Value.R*255),math.floor(F.Value.G*255),math.floor(F.Value.B*255)},', ')

t:SafeCallback(F.Callback,F.Value);
t:SafeCallback(F.Changed,F.Value);
end;

function F:OnChanged(ae)
F.Changed=ae;
ae(F.Value)
end;

function F:Show()
for ae,af in next,t.OpenedFrames do
if ae.Name=='Color'then
ae.Visible=false;
t.OpenedFrames[ae]=nil;
end;
end;

I.Visible=true;
t.OpenedFrames[I]=true;
end;
function F:Hide()
I.Visible=false;
t.OpenedFrames[I]=nil;
end;
function F:SetValue(ae,af)
local ag=Color3.fromHSV(ae[1],ae[2],ae[3]);
F.Transparency=af or 0;
F:SetHSVFromRGB(ag);
F:Display();
end;

function F:SetValueRGB(ae,af)
F.Transparency=af or 0;
F:SetHSVFromRGB(ae);
F:Display();
end;

N.InputBegan:Connect(function(ae)
...
local ai=N.AbsolutePosition.X;
local aj=ai+N.AbsoluteSize.X;
local ak=math.clamp(ag,ai,aj);

local al=N.AbsolutePosition.Y;
local am=al+N.AbsoluteSize.Y;
local an=math.clamp(ah,al,am);
...
af(ae.Position.X,ae.Position.Y)

local ag=c.InputChanged:Connect(function(ag)
if ag.UserInputType==Enum.UserInputType.MouseMovement or ag==ae then
af(ag.Position.X,ag.Position.Y)
end
end)

local ah
ah=c.InputEnded:Connect(function(ai)
if ai==ae or ai.UserInputType==Enum.UserInputType.Touch then
ag:Disconnect()
ah:Disconnect()
t:AttemptSave()
end
...
R.InputBegan:Connect(function(ae)
if ae.UserInputType==Enum.UserInputType.MouseButton1 or ae.UserInputType==Enum.UserInputType.Touch then
local function af(ag)
local ah=R.AbsolutePosition.Y;
local ai=ah+R.AbsoluteSize.Y;
local aj=math.clamp(ag,ah,ai);
...
G.InputBegan:Connect(function(ae)
if(ae.UserInputType==Enum.UserInputType.MouseButton1 or ae.UserInputType==Enum.UserInputType.Touch)and not t:MouseIsOverOpenedFrame()then
if I.Visible then
F:Hide()
else
ab:Hide()
...
elseif ae.UserInputType==Enum.UserInputType.MouseButton2 and not t:MouseIsOverOpenedFrame()then
ab:Show()
F:Hide()
end
end);
...
if Z then
Z.InputBegan:Connect(function(ae)
if ae.UserInputType==Enum.UserInputType.MouseButton1 or ae.UserInputType==Enum.UserInputType.Touch then
local function af(ag)
local ah=Z.AbsolutePosition.X;
local ai=ah+Z.AbsoluteSize.X;
local aj=math.clamp(ag,ah,ai);

F.Transparency=1-((aj-ah)/(ai-ah));
F:Display();
end
...
t:GiveSignal(c.InputBegan:Connect(function(ae)
if(ae.UserInputType==Enum.UserInputType.MouseButton1 or ae.UserInputType==Enum.UserInputType.Touch)then
local af,ag=I.AbsolutePosition,I.AbsoluteSize;
local ah=G.AbsolutePosition;
local ai=G.AbsoluteSize;

if m.X<af.X or m.X>af.X+ag.X
or m.Y<ah.Y or m.Y>af.Y+ag.Y then

if not(m.X>=ah.X and m.X<=ah.X+ai.X
and m.Y>=ah.Y and m.Y<=ah.Y+ai.Y)then
F:Hide();
end
end;

if not t:IsMouseOverFrame(ab.Container)then
ab:Hide()
end
end;

if ae.UserInputType==Enum.UserInputType.MouseButton2 and ab.Container.Visible then
if not t:IsMouseOverFrame(ab.Container)and not t:IsMouseOverFrame(G)then
ab:Hide()
end
end
end))

function F:GetTransparency()
return F.Transparency
end;

function F:OnTransparencyChanged(ae)
F.TransparencyChanged=ae;
ae(F.Transparency);
end;

local ae=F.Display;
F.Display=function(af)
ae(af);
t:SafeCallback(F.TransparencyChanged,F.Transparency);
end;
...
function B:AddColorPickerAlpha(aa,ab)
ab=ab or{};
if ab.Transparency==nil then
ab.Transparency=0;
end;
return B.AddColorPicker(self,aa,ab)
end;

function B:AddKeyPicker(aa,ab)
local ac=self;
...
local an=ab.Modes or{'Always','Toggle','Hold'};
local C={};

for D,E in next,an do
local F={};
local G=t:CreateLabel({
Active=false;
Size=UDim2.new(1,0,0,15);
TextSize=t.FontSize-1;
Text=E;
...
G.TextColor3=t.AccentColor;
t.RegistryMap[G].Properties.TextColor3='AccentColor';

aj.Visible=false;
end;
function F:Deselect()
af.Mode=nil;
G.TextColor3=t.FontColor;
t.RegistryMap[G].Properties.TextColor3='FontColor';
end;
...
am.Text=string.format('[%s] %s (%s)',E,ab.Text,af.Mode);
local F=t.KeybindMode or'All'
if F=='Active'then
al.Visible=D==true
...
am.TextColor3=D and t.AccentColor or t.FontColor;
t.RegistryMap[am].Properties.TextColor3=D and'AccentColor'or'FontColor';

local G=0
local H=0

for I,J in next,t.KeybindContainer:GetChildren()do
...
t.KeybindFrame.Size=UDim2.new(0,math.max(H+10+15,210),0,G+23)
end;
...
ai.Text=J;
af.Value=J;
t:SafeCallback(af.ChangedCallback,I.KeyCode or I.UserInputType)
t:SafeCallback(af.Changed,I.KeyCode or I.UserInputType)

t:AttemptSave();
H:Disconnect();
end);
elseif E.UserInputType==Enum.UserInputType.MouseButton2 and not t:MouseIsOverOpenedFrame()then
aj.Visible=true;
end;
end);

t:GiveSignal(c.InputBegan:Connect(function(E)
if(not D)then
...
t:GiveSignal(c.InputEnded:Connect(function(E)
if(not D)then
af:Update();
end;
end))
...
function aj:Resize()
local am=0
for an,B in next,ag:GetChildren()do
if B:IsA('Frame')then
local C=B:FindFirstChildOfClass('UIListLayout')
if C and C.AbsoluteContentSize.Y>am then
am=C.AbsoluteContentSize.Y
end
end
end
...
i:Create(ai,al,{BorderColor3=t.AccentColor}):Play()
i:Create(aj,al,{BackgroundColor3=t.AccentColor}):Play()
end)

ai.MouseLeave:Connect(function()
i:Create(ai,al,{BorderColor3=Color3.new(0,0,0)}):Play()
i:Create(aj,al,{BackgroundColor3=t.MainColor}):Play()
end)
return ai,aj,ak
end
...
local an=Instance.new('BindableEvent')
local B=ak:Once(function(...)
...
local function am()
local an=2
local B=ak.AbsoluteSize.X

if not al:IsFocused()or al.TextBounds.X<=B-2*an then
al.Position=UDim2.new(0,an,0,0)
else
local C=al.CursorPosition
if C~=-1 then
local D=string.sub(al.Text,1,C-1)
local E=d:GetTextSize(D,al.TextSize,al.Font,Vector2.new(math.huge,math.huge)).X

local F=al.Position.X.Offset+E

if F<an then
al.Position=UDim2.fromOffset(an-E,0)
elseif F>B-an-1 then
al.Position=UDim2.fromOffset(B-E-an-1,0)
end
end
end
end
...
if type(ad.Tooltip)=='string'then
t:AddToolTip(ad.Tooltip,am)
end

function ae:Display()
aj.BackgroundColor3=ae.Value and t.AccentColor or t.MainColor;
aj.BorderColor3=ae.Value and t.AccentColorDark or t.OutlineColor;

t.RegistryMap[aj].Properties.BackgroundColor3=ae.Value and'AccentColor'or'MainColor';
t.RegistryMap[aj].Properties.BorderColor3=ae.Value and'AccentColorDark'or'OutlineColor';
end;
...
if(an.UserInputType==Enum.UserInputType.MouseButton1 or an.UserInputType==Enum.UserInputType.Touch)and not t:MouseIsOverOpenedFrame()then
ae:SetValue(not ae.Value)
t:AttemptSave();
end;
end);
if ae.Risky then
t:RemoveFromRegistry(al)
al.TextColor3=t.RiskColor
t:AddToRegistry(al,{TextColor3='RiskColor'})
end
...
function ae:UpdateColors()
ak.BackgroundColor3=t.AccentColor;
ak.BorderColor3=t.AccentColorDark;
end;

local B=TweenInfo.new(0.1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out);

function ae:Display()
local C=ad.Suffix or'';
if ad.Compact then
an.Text=ad.Text..': '..ae.Value..C
elseif ad.HideMax then
an.Text=string.format('%s',ae.Value..C)
else
an.Text=string.format('%s/%s',ae.Value..C,ae.Max..C);
end

ae.MaxSize=aj.AbsoluteSize.X;
local D=math.ceil(t:MapValue(ae.Value,ae.Min,ae.Max,0,ae.MaxSize));


i:Create(ak,B,{
Size=UDim2.new(0,D,1,0);
}):Play();

am.Visible=not(D==ae.MaxSize or D==0);
end;


function ae:OnChanged(C)
ae.Changed=C;
C(ae.Value);
end;
local function C(D)
...
return tonumber(string.format('%.'..ae.Rounding..'f',D))
end;
function ae:GetMaxSize()
return aj.AbsoluteSize.X
end
function ae:GetValueFromXOffset(D)
return C(t:MapValue(D,0,ae:GetMaxSize(),ae.Min,ae.Max))
end
function ae:SetValue(D)
local E=tonumber(D);
if(not E)then
return
end;

E=math.clamp(E,ae.Min,ae.Max);
...
aj.InputBegan:Connect(function(D)
...
local function E(F)
local G=aj.AbsolutePosition.X
local H=ae:GetMaxSize()

local I=F-G
local J=math.clamp(I,0,H)
...
local F=c.InputChanged:Connect(function(F)
if F.UserInputType==Enum.UserInputType.MouseMovement or F==D then
E(F.Position.X)
end
end)
...
if H==D or H.UserInputType==Enum.UserInputType.Touch then
F:Disconnect()
G:Disconnect()
t:AttemptSave()
end
...
local function C()
B.Position=UDim2.fromOffset(aj.AbsolutePosition.X,aj.AbsolutePosition.Y+aj.Size.Y.Offset+1);
end;

local function D(E)
B.Size=UDim2.fromOffset(aj.AbsoluteSize.X,E or(an*20+2))
end;
C();
D();

aj:GetPropertyChangedSignal('AbsolutePosition'):Connect(C);
...
local F=t:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
...
for I,J in next,F:GetChildren()do
...
N.TextColor3=O and t.AccentColor or t.FontColor;
t.RegistryMap[N].Properties.TextColor3=O and'AccentColor'or'FontColor';
end;
N.InputBegan:Connect(function(P)
if(P.UserInputType==Enum.UserInputType.MouseButton1 or P.UserInputType==Enum.UserInputType.Touch)then
local Q=not O;

if ae:GetActiveValues()==1 and(not Q)and(not ad.AllowNull)then
else
if ad.Multi then
O=Q;
...
F.CanvasSize=UDim2.fromOffset(0,(I*20)+1);

local J=math.clamp(I*20,0,an*20)+1;
D(J);
end;

function ae:SetValues(G)
if G then
ae.Values=G;
end;
...
B.Visible=true;
t.OpenedFrames[B]=true;
al.Rotation=180;
end;

function ae:CloseDropdown()
B.Visible=false;
t.OpenedFrames[B]=nil;
al.Rotation=0;
end;

function ae:OnChanged(G)
ae.Changed=G;
G(ae.Value);
end;
...
local H={};
for I,J in next,G do
if table.find(ae.Values,I)then
H[I]=true
end;
end;
...
if(not G)then
ae.Value=nil;
elseif table.find(ae.Values,G)then
ae.Value=G;
end;
end;
...
if(G.UserInputType==Enum.UserInputType.MouseButton1 or G.UserInputType==Enum.UserInputType.Touch)and not t:MouseIsOverOpenedFrame()then
if B.Visible then
ae:CloseDropdown();
else
ae:OpenDropdown();
...
local H,I=B.AbsolutePosition,B.AbsoluteSize;

if m.X<H.X or m.X>H.X+I.X
or m.Y<(H.Y-20-1)or m.Y>H.Y+I.Y then
...
local H=table.find(ae.Values,ad.Default)
if H then
table.insert(G,H)
end
elseif type(ad.Default)=='table'then
for H,I in next,ad.Default do
local J=table.find(ae.Values,I)
if J then
table.insert(G,J)
end
end
elseif type(ad.Default)=='number'and ae.Values[ad.Default]~=nil then
table.insert(G,ad.Default)
end
...
local I=G[H]
if ad.Multi then
ae.Value[ae.Values[I] ]=true
else
ae.Value=ae.Values[I];
end
...
local ac=t.Watermark.Position.Y
t.WatermarkText.Text=ab
t.Watermark.AnchorPoint=Vector2.new(0.5,0)
t.Watermark.Position=UDim2.new(0.5,0,ac.Scale,ac.Offset)
...
local an=(ae=='Left'or ae=='Right')and-2 or-2
local B=(ae=='Top'or ae=='Bottom')and-(ak+1)or-2

local C=(ae=='Left')and aj+2 or 4
local D=(ae=='Left'or ae=='Right')and-(aj+4)or-4
...
Size=UDim2.new(1,an,1,B);
ZIndex=102;
Parent=H;
});
local J=t:Create('UIGradient',{
Color=ColorSequence.new({
ColorSequenceKeypoint.new(0,t:GetDarkerColor(t.MainColor)),
ColorSequenceKeypoint.new(1,t.MainColor),
});
...
L.Size=UDim2.new(0,aj,1,2)
elseif ae=='Right'then
L.Position=UDim2.new(1,-aj+1,0,-1)
L.Size=UDim2.new(0,aj,1,2)
elseif ae=='Top'then
L.Position=UDim2.new(0,-1,0,-1)
L.Size=UDim2.new(1,2,0,ak)
elseif ae=='Bottom'then
L.Position=UDim2.new(0,-1,1,-ak+1)
L.Size=UDim2.new(1,2,0,ak)
end
...
pcall(G.TweenSize,G,
UDim2.new(0,M,0,ai),'Out','Quad',0.4,true);
task.spawn(function()
wait(ac or 5);
pcall(G.TweenSize,G,
...
local an=t:Create('UIListLayout',{
...
function ad:SetWindowTitle(E)
ah.Text=E;
end;
function ad:AddTab(E)
local F={
Groupboxes={};
Tabboxes={};
};

local G=t:GetTextBounds(E,t.Font,t.FontSize+2)

local H=t:Create('Frame',{
BackgroundColor3=t.BackgroundColor;
BackgroundTransparency=1;
BorderSizePixel=0;
Size=UDim2.new(0,G+12,1,0);
ZIndex=9;
Parent=am;
})
t:AddToRegistry(H,{
...
local L=t:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
...
local M=t:Create('ScrollingFrame',{
BackgroundTransparency=1;
BorderSizePixel=0;
...
for N,O in next,{L,M}do
O:WaitForChild('UIListLayout'):GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
O.CanvasSize=UDim2.fromOffset(0,O.UIListLayout.AbsoluteContentSize.Y)
end)
end


function F:ShowTab()
for N,O in next,ad.Tabs do
O:HideTab()
end
...
J.TextColor3=t.AccentColor
t.RegistryMap[J].Properties.TextColor3='AccentColor'
...
H.BackgroundTransparency=1
J.TextColor3=t.FontColor
t.RegistryMap[J].Properties.TextColor3='FontColor'
end
function F:SetLayoutOrder(N)
H.LayoutOrder=N;
an:ApplyLayout();
end;
function F:AddGroupbox(N)
local O={};
...
function O:Resize()
local W=0;
for X,Y in next,O.Container:GetChildren()do
if(not Y:IsA('UIListLayout'))and Y.Visible then
W=W+Y.Size.Y.Offset;
end;
end;

P.Size=UDim2.new(1,0,0,20+W+2+2);
end;
...
F.Groupboxes[N.Name]=O;

return O
end;

function F:AddLeftGroupbox(N)
return F:AddGroupbox({Side=1;Name=N;})
end;

function F:AddRightGroupbox(N)
return F:AddGroupbox({Side=2;Name=N;})
end;

function F:AddTabbox(N)
local O={
Tabs={};
};
...
BackgroundColor3=t:GetDarkerColor(t.OutlineColor);
...
t:AddToRegistry(W,{
BackgroundColor3=function()return t:GetDarkerColor(t.OutlineColor)end;
});
...
W.BackgroundColor3=t.BackgroundColor;
t.RegistryMap[W].Properties.BackgroundColor3='BackgroundColor';
Y.TextColor3=t.AccentColor;
t.RegistryMap[Y].Properties.TextColor3='AccentColor';
V:Resize();
end;
...
W.BackgroundColor3=t:GetDarkerColor(t.OutlineColor);
t.RegistryMap[W].Properties.BackgroundColor3=function()return t:GetDarkerColor(t.OutlineColor)end;
Y.TextColor3=t.FontColor;
t.RegistryMap[Y].Properties.TextColor3='FontColor';
end;
function V:Resize()
local ao=0;
for ap,aq in next,O.Tabs do
ao=ao+1;
end;

for ap,aq in next,T:GetChildren()do
if not aq:IsA('UIListLayout')then
aq.Size=UDim2.new(1/ao,0,1,0);
end;
end;
...
for aq,ar in next,V.Container:GetChildren()do
if(not ar:IsA('UIListLayout'))and ar.Visible then
ap=ap+ar.Size.Y.Offset;
end;
end;

P.Size=UDim2.new(1,0,0,20+ap+2+2);
end;
W.InputBegan:Connect(function(ao)
if(ao.UserInputType==Enum.UserInputType.MouseButton1 or ao.UserInputType==Enum.UserInputType.Touch)and not t:MouseIsOverOpenedFrame()then
V:Show();
V:Resize();
end;
end);
...
F.Tabboxes[N.Name or'']=O;

return O
end;
function F:AddLeftTabbox(ao)
return F:AddTabbox({Name=ao,Side=1;})
end;

function F:AddRightTabbox(ao)
return F:AddTabbox({Name=ao,Side=2;})
end;

H.InputBegan:Connect(function(ao)
if(ao.UserInputType==Enum.UserInputType.MouseButton1 or ao.UserInputType==Enum.UserInputType.Touch)then
F:ShowTab();
end;
end);
if#D:GetChildren()==1 then
F:ShowTab();
end;
ad.Tabs[E]=F;
return F
...
local E=c:GetMouseLocation();

aq.Color=t.AccentColor;

aq.PointA=Vector2.new(E.X,E.Y);
aq.PointB=Vector2.new(E.X+16,E.Y+6);
aq.PointC=Vector2.new(E.X+6,E.Y+16);
ar.PointA=aq.PointA;
ar.PointB=aq.PointB;
ar.PointC=aq.PointC;
...
t:GiveSignal(c.InputBegan:Connect(function(ap,aq)
if type(t.ToggleKeybind)=='table'and t.ToggleKeybind.Type=='KeyPicker'then
if ap.UserInputType==Enum.UserInputType.Keyboard and ap.KeyCode.Name==t.ToggleKeybind.Value then
task.spawn(t.Toggle)
end
elseif type(t.ToggleKeybind)=='string'then
if ap.UserInputType==Enum.UserInputType.Keyboard and ap.KeyCode.Name==t.ToggleKeybind then
task.spawn(t.Toggle)
end
elseif ap.KeyCode==Enum.KeyCode.RightControl or(ap.KeyCode==Enum.KeyCode.RightShift and(not aq))then
task.spawn(t.Toggle)
end
end))
...
ao.InputBegan:Connect(function(F)
if F.UserInputType==Enum.UserInputType.MouseButton1
or F.UserInputType==Enum.UserInputType.Touch then
ar=true
E=false
C=F.Position
D=ap.Position
...
c.InputChanged:Connect(function(F)
if F==B and ar then
local G=F.Position-C
if G.Magnitude>3 then
E=true
end
if am and E then
ap.Position=UDim2.new(
D.X.Scale,D.X.Offset+G.X,
D.Y.Scale,D.Y.Offset+G.Y
)
end
end
end)
...
["Forest Island (Throwback)"]={
CFrame.new(-2645.7587890625,19.09952163696289,3957.66552734375,0.900820255279541,-8.90305607015307e-10,-0.43419215083122253,0.019455183297395706,0.9989956021308899,0.040363747626543045,0.4337560534477234,-0.04480776935815811,0.8999155163764954),
CFrame.new(-2811.169921875,32.69327163696289,3581.891357421875,0.9338151216506958,-5.614745335336124e-10,0.3577558994293213,-0.016030246391892433,0.9989956021308899,0.041842181235551834,-0.35739657282829285,-0.04480777680873871,0.932877242565155),
CFrame.new(-3028.420654296875,19.09952163696289,3233.06640625,0.8672376871109009,-1.2765652979140896e-09,-0.49789437651634216,0.02230953797698021,0.9989956021308899,0.038858987390995026,0.49739429354667664,-0.04480776935815811,0.8663666844367981),
CFrame.new(-3125.812744140625,58.78828811645508,3278.41015625,-0.9544302225112915,6.415117326241671e-10,-0.29843413829803467,0.03564987704157829,0.9928394556045532,-0.11401282995939255,0.29629719257354736,-0.11945643275976181,-0.9475959539413452),
CFrame.new(-3039.51611328125,55.556095123291016,3900.78076171875,-0.992558479309082,1.947409522529142e-08,0.12176892906427383,-0.0415569506585598,0.9399626851081848,-0.3387376666069031,-0.11445825546979904,-0.3412773013114929,-0.9329679012298584),
CFrame.new(-3122.237060546875,56.693267822265625,4369.5048828125,-0.04829072952270508,2.356704553729827e-10,-0.9988332986831665,0.044755496084690094,0.9989956021308899,-0.0021637999452650547,0.9978300929069519,-0.04480777308344841,-0.0482422299683094),
CFrame.new(-2704.343994140625,19.041061401367188,4454.23974609375,-0.8978657126426697,-1.1989987891425358e-09,0.44026947021484375,-0.01718595065176487,0.9992378354072571,-0.03504824638366699,-0.4399339258670807,-0.039035066962242126,-0.8971813917160034)
},
["Mountain Island (Throwback)"]={
CFrame.new(-2817.66162109375,175.23265075683594,1821.020263671875,-0.7795168161392212,-5.440058403749504e-10,0.6263812780380249,-1.0135109640430073e-08,1,-1.1744416106296285e-08,-0.6263812780380249,-1.550341366396424e-08,-0.7795168161392212),
CFrame.new(-3494.836181640625,18.852523803710938,2294.5029296875,0.9679929614067078,2.1962889285020992e-09,0.25097736716270447,0.004178429953753948,0.9998614192008972,-0.016115767881274223,-0.250942587852478,0.016648640856146812,0.9678587913513184),
CFrame.new(-2755.1767578125,18.86957359313965,2257.945068359375,-0.04128270223736763,2.072863235225153e-11,0.9991475343704224,0.013371532782912254,0.9999104738235474,0.00055248400894925,-0.9990580081939697,0.013382941484451294,-0.041279006749391556),
CFrame.new(-1706.5648193359375,18.86957359313965,1683.26904296875,0.9380949139595032,-5.677184833352555e-10,-0.34637826681137085,-0.004635559860616922,0.9999104738235474,-0.01255447044968605,0.34634724259376526,0.013382942415773869,0.9380109310150146),
CFrame.new(-3510.778564453125,117.78726196289062,2045.1370849609375,-0.688940167427063,-3.1397658517562377e-07,0.724818229675293,0.053280286490917206,0.997294545173645,0.050643377006053925,-0.7228572964668274,0.07350877672433853,-0.6870762705802917),
CFrame.new(-4112.453125,18.86957359313965,1498.7515869140625,-0.8605156540870667,-1.5045772272515023e-07,-0.5094240307807922,-0.0068177212961018085,0.9999104738235474,0.011516153812408447,0.5093783736228943,0.013382941484451294,-0.8604385852813721),
CFrame.new(-3558.8271484375,18.86957359313965,321.7713317871094,-0.9888119101524353,1.2208082322828773e-09,0.14916753768920898,0.001996301580220461,0.9999104738235474,0.013233211822807789,-0.1491541862487793,0.013382941484451294,-0.9887233972549438)
},
["Lunar Islands (Throwback)"]={
CFrame.new(-2692.972412109375,81.2981185913086,-3013.7421875,0.8737332820892334,1.1835785684866096e-09,-0.4864053428173065,0.07269331812858582,0.9887692928314209,0.13057951629161835,0.48094263672828674,-0.14945007860660553,0.8639206290245056),
CFrame.new(-3565.741455078125,15.51860237121582,-3990.25,-0.8327977061271667,1.6515574463582539e-09,0.5535774230957031,0.00791101437062025,0.9998978972434998,0.011901266872882843,-0.5535208582878113,0.014290707185864449,-0.832712709903717),
CFrame.new(-3758.672119140625,58.03699493408203,-3903.278564453125,0.42879927158355713,0.0000022030583295418182,-0.9033997654914856,0.22922514379024506,0.9672732353210449,0.10880421847105026,0.8738346695899963,-0.25373712182044983,0.41476553678512573),
CFrame.new(-3344.911376953125,27.45481300354004,-3538.0556640625,-0.18251392245292664,-7.660114209606661e-10,0.9832032918930054,0.2527621388435364,0.966390073299408,0.04692072421312332,-0.950157880783081,0.2570802569389343,-0.17637963593006134),
CFrame.new(-3667.556396484375,14.357109069824219,-1678.4263916015625,0.8500844836235046,0.0000012958216757397167,-0.5266463160514832,-0.18660850822925568,0.9351198077201843,-0.30121123790740967,0.49247702956199646,0.3543316721916199,0.7949311137199402),
CFrame.new(-3152.14111328125,53.04074478149414,-818.3273315429688,-0.5497733354568481,-3.9937089746366894e-10,0.8353138566017151,-0.01294289343059063,0.9998799562454224,-0.008518543094396591,-0.8352136015892029,-0.01549464650452137,-0.5497073531150818),
CFrame.new(-2589.01318359375,45.34171676635742,-732.80322265625,0.3039243221282959,8.568468388148176e-07,0.9526962041854858,-0.1402074247598648,0.9891113638877869,0.044727373868227005,-0.9423226118087769,-0.1471688151359558,0.300615131855011),
CFrame.new(-2201.864013671875,53.50882339477539,-1301.504150390625,0.9891817569732666,6.338255253979241e-09,0.14669498801231384,-0.008047064766287804,0.9984942674636841,0.054262276738882065,-0.14647410809993744,-0.054855719208717346,0.9876923561096191),
CFrame.new(-2212.5947265625,37.42900085449219,-1867.304443359375,0.9936606287956238,1.0158131003024096e-09,-0.11242134124040604,-0.03296387568116188,0.9560458660125732,-0.29135844111442566,0.10747995227575302,0.2932172417640686,0.9499850869178772)
},

["Wild Island (Throwback)"]={
CFrame.new(3432.30810546875,20.46332359313965,-4193.63134765625,0.98481285572052,-1.3883409699655402e-10,0.1736193299293518,0.002323537366464734,0.9999104738235474,-0.013179692439734936,-0.17360377311706543,0.013382941484451294,0.9847246408462524),
CFrame.new(4005.9033203125,20.812761306762695,-3152.5869140625,-0.7392423748970032,-6.944985697643347e-10,0.6734394431114197,0.004857037216424942,0.9999740123748779,0.005331627558916807,-0.6734219193458557,0.007212285418063402,-0.7392231822013855),
CFrame.new(4504.66259765625,92.46330261230469,-3211.67138671875,0.9078524708747864,5.18271122018632e-07,-0.419289767742157,-0.00561180105432868,0.9999104738235474,-0.012149520218372345,0.4192522168159485,0.013382942415773869,0.9077711701393127),
CFrame.new(3038.34619140625,34.11455154418945,-3864.0322265625,-0.6413583755493164,1.1108452824259984e-08,0.7672414779663086,0.14720547199249268,0.9814216494560242,0.12305311858654022,-0.7529873847961426,0.1918632835149765,-0.6294429898262024)
...
local ae=ac.LocalPlayer
local af=require(ab:WaitForChild("References"))
local ah=af.Utilities
local ai=require(ae.PlayerScripts.Priority:WaitForChild("Data"))
local aj=require(ae.PlayerScripts.Priority:WaitForChild("InventoryHandler"))
...
local function h(i)
local j,k=pcall(aj.GetAmountOf,i)
return(j and tonumber(k))or 0
end
...
local function o()
local p=tick()
if p-m<n then return l end
m=p
for q,r in ipairs(ak)do
if h(r.id)>0 then
l=r.id
return l
end
...
local function q(r)
local s=aj.GetGuidsOfItemId(r)
if not s or#s==0 then return false end
ah.Network:FireServer("QuickEquipment","Set","Lasso",s[1])
task.wait(0.3)
return true
end
...
local u=t and aj.GetGuidsOfItemId(s)
local v=false
if u then
...
if k()and v then return end

local w=tick()
if(w-c)<d then return end
...
local v=tick()+ao.confirmTimeout
while tick()<v do
task.wait(ao.confirmInterval)
if h(t)>u then return true end
end
return false
end
...
local x=h(u)
ah.Network:FireServer("Shopping","BuyShopItem",w.shop,w.slot,v,nil)
...
ae.CharacterAdded:Connect(function()
task.wait(1.5)
ar=false
c=0
b=false
...
setCraftEnabled=function(w)
ao.autoCraftEnabled=w
if not w then b=false end
end,


setSelectedLasso=function(w)
ao.selectedLassoId=tonumber(w)or 191
end,


setRestockThreshold=function(w)
ao.restockThreshold=tonumber(w)or 50
end,


setRestockAmount=function(w)
ao.restockAmount=math.min(tonumber(w)or 100,ap)
end,


getCraftableLassos=function()
local w={}
for x,y in ipairs(ak)do
if y.craftable then
table.insert(w,{id=y.id,name=y.name,strength=y.strength})
end
end
return w
end,
...
for x,y in ipairs(ak)do
if y.id==w then return y.name end
end
return"Unknown"
end)(),
selectedCraftId=ao.selectedLassoId,
selectedCraftCount=h(ao.selectedLassoId),
...
if b then return end
b=true
local w,x=pcall(v,ao.selectedLassoId,ao.restockAmount)
b=false
if not w then warn("Manual restock error:",x)end
end,
...
local b=(am.Position-ar.Position).Magnitude
if b<ao then
ao=b
an=aq
end
...
local ak=require(ab:WaitForChild("References"))
...
"legacyCoat",
"collectionLogReward",
"volcanicMineralsUnique",
"trainingReceiptsUnique",
"goldenAppleBasket",
...
hybridUniqueCoat="Hybrid Unique Coat",
breedingUnique="Breeding Unique",
cobreedingUnique="Co-Breeding Unique",
legacyCoat="Legacy Coat",
collectionLogReward="Collection Log Reward",
volcanicMineralsUnique="Volcanic Minerals Unique",
trainingReceiptsUnique="Training Receipts Unique",
goldenAppleBasket="Golden Apple Basket",
horned="Horned",
...
autumn2022="Autumn 2022",
autumn2023="Autumn 2023",
autumn2024="Autumn 2024",
autumn2025="Autumn 2025",
winter2022="Winter 2022",
...
halloween="Halloween",
halloween2022="Halloween 2022",
halloween2023="Halloween 2023",
halloween2024="Halloween 2024",
halloween2025="Halloween 2025",
holiday="Holiday",
holiday2023="Holiday 2023",
holiday2024="Holiday 2024",
holiday2025="Holiday 2025",
valentines="Valentines",
valentines2022="Valentines 2022",
valentines2023="Valentines 2023",
valentines2024="Valentines 2024",
valentines2025="Valentines 2025",
valentines2026="Valentines 2026",
easter2022="Easter 2022",
stpatricksday2022="St. Patrick's Day 2022",
stpatricksday2025="St. Patrick's Day 2025",
stpatricksday2026="St. Patrick's Day 2026",
competitionHubUnique="Competition Hub Unique",
summer="Summer Hair",
autumn="Autumn Hair",
winter="Winter Hair",
spring="Spring Hair",
stpatricksday="St. Patrick's Day Hair",
unreleasedEventCoat="Unreleased Event Coat",
}
...
if not ar or aq==""then return end

local h=f and f.variants or{}
local i={}

table.insert(i,{
name="Lock Reason",
value=b[g]or g or"Unknown",
inline=true,
})

if h.colour then
local j=ad.colour[h.colour]
table.insert(i,{
name="Coat",
value=(j and j.name)or h.colour,
inline=true,
})
if j then
table.insert(i,{
name="Rarity",
value=tostring(math.floor((j.rarityFloat or 0)*100)).."%",
inline=true,
})
end
end

if h.maneColour then
local j=ad.maneAndTailColour[h.maneColour]
table.insert(i,{
name="Mane Colour",
value=(j and j.name)or h.maneColour,
inline=true,
})
end

if h.tailColour then
local j=ad.maneAndTailColour[h.tailColour]
table.insert(i,{
name="Tail Colour",
value=(j and j.name)or h.tailColour,
inline=true,
})
end

if h.maneColour and h.tailColour
and h.maneColour~=h.tailColour then
table.insert(i,{name="Mismatch",value="Yes",inline=true})
end

if h.hornStyle and h.hornStyle~=""then
table.insert(i,{name="Horn Style",value=h.hornStyle,inline=true})
end

if h.hornColour and h.hornColour~=""then
table.insert(i,{name="Horn Colour",value=h.hornColour,inline=true})
end

if f.isNatDyed then
table.insert(i,{name="Naturally Dyed",value="Yes",inline=true})
end

if h.maneStyle then
table.insert(i,{name="Mane Style",value=h.maneStyle,inline=true})
end

if h.tailStyle then
table.insert(i,{name="Tail Style",value=h.tailStyle,inline=true})
end
...
j.sold,j.locked,j.coins),
inline=false,
})

local k=ac:JSONEncode({
embeds={{
title="Horse Locked — "..(b[g]or g or"Unknown"),
color=0xF5A623,
fields=i,
footer={text="fantasyhaxx • "..os.date("!%Y-%m-%d %H:%M:%S UTC")},
...
local g=f.variants
if not g then return false,nil end

if an["horned"]==true then
if(g.hornStyle and g.hornStyle~="")
or(g.hornColour and g.hornColour~="")then
return true,"horned"
end
end

if g.colour then
local h=ad.colour[g.colour]
if h then
local i=h.specialItemIndicator
if i and an[i]==true then
return true,i
end
if an["rareCoat"]==true
and(h.rarityFloat or 0)>=ao then
return true,"rareCoat"
end
end
end

for h,i in ipairs({"maneColour","tailColour"})do
local j=g[i]
if j then
local k=ad.maneAndTailColour[j]
if k then
local l=k.specialItemIndicator
if l and an[l]==true then
return true,l
end
end
if an["islandUniqueHairColour"]==true
and c[j]==true then
...
if an["mismatchHairColour"]==true then
local h=g.maneColour
local i=g.tailColour
if h and i and h~=i then return true,"mismatchHairColour"end
end

if an["naturallyDyedHairColour"]==true
and f.isNatDyed==true then
return true,"naturallyDyedHairColour"
end

if f.specialItemIndicator
and an[f.specialItemIndicator]==true then
return true,f.specialItemIndicator
end
...
local function h(i)
f=(i~=nil)and i or(not f)

if f then
if not g then
g=aj.Bind("Added",function(j,k)
local l,m=pcall(function()
local l,m=e(k)
task.delay(ap,function()
if not f then return end
if l then
ai:FireServer("Inventory","Lock",j)
d(j,k,m)
ae.recordLock(m)

task.delay(ap,function()
if not f then return end
local n=al.GetFollowerInstance()
if n then
al.Interact(n,"Pickup")
else
task.delay(0.3,function()
local o=al.GetFollowerInstance()
if o then
...
ai:FireServer("Shopping","QuickSellItem",j)
ae.recordSell()
end
end)
end)
...
return{
setEnabled=function(i)h(i)end,
isEnabled=function()return f end,

getStats=ae.getStats,
resetCounters=ae.reset,
snapshotBalance=ae.snapshotBalance,

setWebhook=function(i)aq=i or""end,
setWebhookEnabled=function(i)ar=i==true end,
testWebhook=function()
d("{test-0000-0000-0000-000000000000}",{
isNatDyed=true,
...
setLockOption=function(i,j)
local k=an[i]~=nil
if not k then
for l,m in ipairs(am)do
...
local i={}
for j,k in ipairs(am)do i[k]=an[k]==true end
return i
end,
pickupFollower=function()
local i=al.GetFollowerInstance()
if i then al.Interact(i,"Pickup")end
end,

setRareThreshold=function(i)ao=tonumber(i)or ao end,
getRareThreshold=function()return ao end,
setActionDelay=function(i)ap=tonumber(i)or ap end,
getActionDelay=function()return ap end,
checkHorse=function(i)return e(i)end,
getLockOptionNames=function()return am end,
...
local o=Instance.new("LinearVelocity")
o.Attachment0=n
o.MaxForce=1e6
...
local B=12
local C=tick()
local D=w.Parent and w.Parent:FindFirstChildOfClass("Humanoid")
if D then D.PlatformStand=true end

r(w)
if t then t:Disconnect()end
t=af.Heartbeat:Connect(function()
local E=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not E or not o.Parent then
if t then t:Disconnect();t=nil end
if D then D.PlatformStand=false end
s();u=false
return
end
if tick()-C>B then
...
o.VectorVelocity=F.Unit*math.min(G*15,z)
end
end)
end
...
local function x()
local y=ai.Character
if not y then return end
for z,A in ipairs(y:GetDescendants())do
if A:IsA("BasePart")then A.CanCollide=false end
end
end

local function y()
local z=ai.Character
if not z then return end
for A,B in ipairs(z:GetDescendants())do
if B:IsA("BasePart")then B.CanCollide=true end
end
end
...
if A then
w=af.Heartbeat:Connect(function()
local B=ai.Character
...
local B=al.GetLocal({"quickEquipment","Harvester"})
if not B then return end
if not d then
d=true
...
am.ObjectChanged:Connect(function()
local C=al.GetLocal({"quickEquipment","Harvester"})
local D=al.GetLocal({"temporary","equippedEquipment"})
if D~=C then
d=false
task.wait(0.1)
A()
...
if ai.Character then B()end
ai.CharacterAdded:Connect(B)
...
if F:FindFirstChild(ai.Name)then return F end
end
return nil
end
...
local function H(I)
if not I:IsA("Model")then return false end
local J=I:GetAttribute(i)
return J~=nil and g[J]~=nil
end
...
if H(L)then D[L]=true end
end


F=J.DescendantAdded:Connect(function(K)
task.defer(function()
if H(K)then D[K]=true end
end)
end)

G=J.DescendantRemoving:Connect(function(K)
D[K]=nil
end)
end
...
for O in pairs(D)do
if not O.Parent then D[O]=nil;continue end

local P=O:GetAttribute(i)
if g[P]~=true then continue end

local Q=O:GetAttribute(j)
if not Q or Q<=0 then continue end



local R
if P=="Erupted Deposit"then
local S,T=pcall(function()return O:GetBoundingBox()end)
R=S and T and T.Position or O:GetPivot().Position
else
local S=O.PrimaryPart or O:FindFirstChildWhichIsA("BasePart")
R=S and S.Position or O:GetPivot().Position
end

local S=(L.Position-R).Magnitude
if S<N then
N=S
M=O
end
...
local K=cloneref(game:GetService("VirtualInputManager"))
...
local P=workspace.CurrentCamera.CFrame
workspace.CurrentCamera.CFrame=P:Lerp(
CFrame.new(P.Position,N),
L
)
af.Heartbeat:Wait()
end
...
local function N(O)
if not O or not O.Parent or(O:GetAttribute(j)or 0)<=0 then
return false
end

local P=workspace.CurrentCamera
local Q=O.PrimaryPart or O:FindFirstChildWhichIsA("BasePart")
if not Q then return false end

local R=Q.Position


if O:GetAttribute(i)=="Erupted Deposit"then
local S,T=pcall(function()return O:GetBoundingBox()end)
if S and T then R=T.Position end
end

local S,T=P:WorldToViewportPoint(R)
if not T or S.Z<=0 then return false end

local U=math.floor(S.X)
local V=math.floor(S.Y)

pcall(function()
K:SendMouseMoveEvent(U,V,game,0)
K:SendMouseButtonEvent(U,V,0,true,game,0)
task.wait(0.05)
K:SendMouseButtonEvent(U,V,0,false,game,0)
end)
...
while true do
task.wait(1)
if not ao or not aq or e then
l=tick()
continue
end
local O=ai.Character
local P=O and O:FindFirstChild("HumanoidRootPart")
if not P then continue end
if(P.Position-k).Magnitude>2 then
k=P.Position
l=tick()
elseif tick()-l>b then
local Q=C()
if Q then
local R=(Q.Name=="Volcano Island")
and an
or(ad and ad[Q.Name])
if R and#R>0 then
local S=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if S then
local T=R[math.random(1,#R)]
v(S,T.Position,nil)
end
end
end
...
if not ao then P();continue end

local Q=C()
if not Q then continue end

local R=J(Q)
if not R then P();continue end

local S=R:GetAttribute(i)=="Erupted Deposit"
...
until(R:GetAttribute(j)or 0)>0 or tick()-T>6
if not R.Parent or(R:GetAttribute(j)or 0)<=0 then continue end
end


local T
if S then
local U,V=pcall(function()return R:GetBoundingBox()end)
T=U and V and V.Position or R:GetPivot().Position
else
local U=R.PrimaryPart or R:FindFirstChildWhichIsA("BasePart")
T=U and U.Position or R:GetPivot().Position
end

local U=S and Vector3.new(0,6,0)or Vector3.new(0,2,2)
local V=T+U
...
O=af.Heartbeat:Connect(function()
local W=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not W then P();return end
if not R or not R.Parent or(R:GetAttribute(j)or 0)<=0 then
P();return
end
...
local X
if S then
local Y,Z=pcall(function()return R:GetBoundingBox()end)
X=(Y and Z and Z.Position or R:GetPivot().Position)+U
else
X=V
end

r(W)
local Y=X-W.Position
local Z=Y.Magnitude
if Z<=q then
o.VectorVelocity=Vector3.zero
else
o.VectorVelocity=Y.Unit*math.min(Z*10,p)
end
end)
...
repeat
task.wait(0.05)
local X=ai.Character and ai.Character:FindFirstChild("HumanoidRootPart")
if not X or not R.Parent then break end
local Y=S
and((function()
local Y,Z=pcall(function()return R:GetBoundingBox()end)
return(Y and Z and Z.Position or R:GetPivot().Position)+U
end)())
or V
if(X.Position-Y).Magnitude<=q+1 then break end
until tick()-W>8

local X=R:GetAttribute(j)

while ao
and R.Parent
and(R:GetAttribute(j)or 0)>0
and g[R:GetAttribute(i)]==true
do
...
while tick()-Y<1 do
task.wait(0.05)
if not R.Parent or(R:GetAttribute(j)or 0)<=0 then break end
if R:GetAttribute(j)~=X then
X=R:GetAttribute(j)
break
end
end
...
ao=Q
if not Q then P();z(false)end
end,
setPickaxeEnabled=function(Q)
c=Q
if Q then A()end
end,
isEnabled=function()return ao end,
setHighlight=function(Q)
ap=Q
if not Q then m.Enabled=false end
end,
setRandomTeleport=function(Q)aq=Q end,
setIdleThreshold=function(Q)b=tonumber(Q)or 5 end,
setClickCooldown=function(Q)ar=tonumber(Q)or 0.05 end,
setOreTarget=function(Q,R)
if g[Q]~=nil then g[Q]=R end
end,
getOreValues=function()return h end,
setNoclip=function(Q)z(Q)end,
isNoclipEnabled=function()return f end,
}
end
...
for ar,b in ipairs(aq:GetDescendants())do
if b:IsA("Model")then
local c=b:FindFirstChild("HumanoidRootPart")
local d=b:FindFirstChildOfClass("Humanoid")

if c and d and(
b:FindFirstChildWhichIsA("AlignPosition")or
b:FindFirstChildWhichIsA("AlignOrientation")
)then
local e=(al.Position-c.Position).Magnitude
if e<ao then
ao=e
an=d
end
...
local g,h=pcall(require,f.References)
if not g then warn("[CC] References not found:",h);return false end

ap=h
aq=ap.Utilities.Network
ar=ap.Utilities.Time

local i=ao.PlayerScripts:FindFirstChild("RidingHandler",true)
if not i then warn("[CC] RidingHandler not found");return false end
b=require(i)

local j=ao.PlayerScripts:FindFirstChild("CheckpointActivityHandler",true)
if not j then warn("[CC] CheckpointActivityHandler not found");return false end
c=require(j)
...
local f={"Collision","BallCollision","HumanoidRootPart"}
local g={"LowerTorso","UpperTorso"}
local h={}
...
m.CanCollide=false
table.insert(h,m:GetPropertyChangedSignal("CanCollide"):Connect(function()
...
n.CanCollide=false
table.insert(h,n:GetPropertyChangedSignal("CanCollide"):Connect(function()
if ak and n.CanCollide then n.CanCollide=false end
end))
end
end
end
...
local function j(k)
for l,m in h do pcall(function()m:Disconnect()end)end
h={}
if not k then return end
for l,m in f do
local n=k.instance:FindFirstChild(m,true)
if n and n:IsA("BasePart")then n.CanCollide=true end
end
local l=ao.Character
if l then
for m,n in g do
local o=l:FindFirstChild(n,true)
if o and o:IsA("BasePart")then o.CanCollide=true end
end
end
end
...
local function l()
return b and b.GetCurrentMount and b.GetCurrentMount()
end
...
local n,o=pcall(require,game:GetService("ReplicatedStorage").References)
if not n then warn("[AT] References failed:",o);return false end
i=o
j=i.Utilities

local p=b.PlayerScripts:FindFirstChild("Data",true)
if not p then warn("[AT] Data not found");return false end
local q,r=pcall(require,p)
if not q then warn("[AT] Data require failed:",r);return false end
k=r

local s=b.PlayerScripts:FindFirstChild("CharacterHandler",true)
if not s then warn("[AT] CharacterHandler not found");return false end
local t,u=pcall(require,s)
if not t then warn("[AT] CharacterHandler require failed:",u);return false end
l=u
...
local n={"LowerTorso","UpperTorso","HumanoidRootPart"}

local function o()
for p,q in h do pcall(function()q:Disconnect()end)end
h={}
...
s.CanCollide=false
table.insert(h,s:GetPropertyChangedSignal("CanCollide"):Connect(function()
if an and s.CanCollide then
s.CanCollide=false
end
end))
end
...
local function p()
for q,r in h do pcall(function()r:Disconnect()end)end
h={}

local q=b.Character
if not q then return end
for r,s in n do
local t=q:FindFirstChild(s,true)
if t and t:IsA("BasePart")then t.CanCollide=true end
end
end
...
local t=s:FindFirstChild("HumanoidRootPart")
if not t then return end
...
local w=Instance.new("LinearVelocity")
w.Attachment0=v
w.MaxForce=1e6
w.RelativeTo=Enum.ActuatorRelativeTo.World
w.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
w.VectorVelocity=Vector3.zero
w.Parent=t

local x=false

local y=ap.Heartbeat:Connect(function()
local y=b.Character and b.Character:FindFirstChild("HumanoidRootPart")
if not y or not w.Parent then x=true;return end

local z=r-y.Position
local A=z.Magnitude
...
w.VectorVelocity=z.Unit*math.min(A*8,al)
end
end)

local z=tick()+8
while not x and tick()<z do
task.wait(0.05)
end
...
if f then return end
local t=tick()
if(t-g)<ae then return end

local u,v=pcall(function()
local u=k.GetLocal({"quickEquipment","Tool"})
if not u then return end

local v=k.GetLocal({"temporary","equippedEquipment"})
if tostring(v)==tostring(u)then return end

f=true
g=tick()

for w=1,af do
j.Network:FireServer("QuickEquipment","Use","Tool")
task.wait(ah)
local x=k.GetLocal({"temporary","equippedEquipment"})
if tostring(x)==tostring(u)then break end
end
...
if not u then
f=false
warn("[AT] equipShovel error:",v)
end
end

local function t()
pcall(function()
j.Network:FireServer(
"Inventory","Use",
k.GetLocal({"quickEquipment","Tool"}),
"Unequip"
)
end)
task.wait(0.4)
...
return j.Network:InvokeServer("BuriedTreasure","GetPoint")
end)
...
local function v(w)
pcall(function()
local x=c.ViewportSize
local y=Vector2.new(x.X/2,x.Y/2)
for z=1,w do
ar:ClickButton1(y,c.CFrame)
task.wait(ak)
end
end)
end
...
while not x and y<10 do
if not d or not ac then break end
x=u()
if not x then
y+=1
t()
...
if ao then print("[AT] Treasure at",tostring(x))end


r(x)


if not l.object:InRange(x,ai)then
...
an=y
if d then
if y then o()else p()end
end
end,

setDigClicks=function(y)aj=y end,
setDigClickDelay=function(y)ak=y end,
setDigRange=function(y)ai=y end,
setTeleSpeed=function(y)al=y end,
setPollInterval=function(y)ad=y end,
setEquipCooldown=function(y)ae=y end,
setDebug=function(y)ao=y end,
stop=x,
}
end

return ab end function a.m():typeof(aa())local ab=a.cache.m if not ab then ab={c=aa()}a.cache.m=ab end return ab.c end end do local function aa()
...
if not f then warn("[SS] References failed:",g);return false end

ap=g.Utilities.Network
aq=g.Utilities.Time

local h=ao.PlayerScripts:FindFirstChild("RidingHandler",true)
if not h then warn("[SS] RidingHandler not found");return false end
ar=require(h)

local i=ao.PlayerScripts:FindFirstChild("CheckpointActivityHandler",true)
if not i then warn("[SS] CheckpointActivityHandler not found");return false end
b=require(i)


local j,k=pcall(function()
return workspace.Islands["Training Island"]["Outdoor Arena"]
.DynamicArena._LAYOUT.CheckpointActivity
end)
if not j or not k then
warn("[SS] CheckpointActivity instance not found:",k)
return false
end
c=k
...
local function k()
return ar and ar.GetCurrentMount and ar.GetCurrentMount()
end
...
local function m()
local n=b and b.currentObject
if not n then return nil end


local o=n.actualCheckpointStep or n.checkpointStep
if not o then return nil end


local p={}
for q,r in ipairs(n.checkpoints or{})do
for s,t in ipairs(r.checkpoints or{})do
if t.position then
table.insert(p,{
position=t.position,
size=t.size or 7.5,
...
local function w()
ap:FireServer("CheckpointActivity","TriggerInteractable",c)
task.wait(2)
s=0
end
...
if u then return end
if not d()then
warn("[SS] Cannot start — refs failed")
return
end

local y=k()
if not y then warn("[SS] Mount up before enabling");return end
local z=l(y)
if not z then warn("[SS] No HumanoidRootPart on mount");return end
...
if not b.currentObject then
w()
end


r=b.ActivityChanged:Connect(function()
if not u then return end
task.wait(0.3)
...
local G=B.Position
local H=D.position
local I=(H-G).Magnitude
local J=I<=af and ae or 0
local K=(Vector3.new(H.X,H.Y+J,H.Z)-G).Unit

o.VectorVelocity=K*ad

if al then
print(string.format("[SS] cp %d/%d | dist=%.1f | target=%s",E,F,I,tostring(H)))
end
end)
end
...
local ab=game:GetService("Players")
local ac=ab.LocalPlayer
local ad=ac:WaitForChild("PlayerGui")

local ae=ad:FindFirstChild("Loading")
if ae then
local af=ae:WaitForChild("Main"):WaitForChild("Bottom"):WaitForChild("Play")
repeat task.wait(0.1)until af.Visible==true
firesignal(af.Activated)
else
...
local am=a.h()
local an=a.i()
local ao=an(m_References,IslandTeleports)
local ap=a.j()
local aq=a.k()
local ar=a.g()
local b=a.l()
...
local h=getgenv().Options
local i=getgenv().Toggles

af.ShowToggleFrameInKeybinds=true
af.ShowCustomCursor=true
af.NotifySide="Left"

local j=af:CreateWindow({
...
l:Button2Down(Vector2.new(0,0),workspace.CurrentCamera.CFrame)
task.wait(1)
l:Button2Up(Vector2.new(0,0),workspace.CurrentCamera.CFrame)
end)
...
HomePageLowkey=j:AddTab("Home"),
Main=j:AddTab("Automation"),
HorseRender=j:AddTab("Render"),
Misc=j:AddTab("Misc"),
["UI Settings"]=j:AddTab("UI Settings"),
}


local n=m.HomePageLowkey:AddLeftGroupbox("User Info")

local o=n:AddLabel('fantasyhaxx tells you to put on that misery')
...
local p=m.HomePageLowkey:AddRightGroupbox("Session Information")

local q=tick()
local r=p:AddLabel('Time Played: 0s')

task.spawn(function()
while true do
task.wait(1)

local s=math.floor(tick()-q)

local t=math.floor(s/3600)
local u=math.floor((s%3600)/60)
local v=s%60
...
local s=p:AddLabel('Coins Earned: 0')

task.spawn(function()
while true do
task.wait(2)
local t=am.getStats()
s:SetText(string.format("Coins Earned: %d",t.coins))
end
end)

local t=p:AddLabel('Horses Captured: 0')

task.spawn(function()
while true do
task.wait(2)
local u=am.getStats()
t:SetText(string.format("Horses Captured: %d",u.captures))
end
end)

local u=m.Main:AddLeftTabbox()

local v=u:AddTab("Horses")
local w=u:AddTab("Sell")
local x=u:AddTab("Lassos")

v:AddToggle('Autofarm_Enable',{
Text='Enable',
Default=false,
...
if not y[D.Name]then
table.insert(A,D.Name)
end
end
end
table.sort(A)
...
["Hybrid Unique Coat"]="hybridUniqueCoat",
["Breeding Unique"]="breedingUnique",
["Co-Breeding Unique"]="cobreedingUnique",
["Legacy Coat"]="legacyCoat",
["Collection Log Reward"]="collectionLogReward",
["Volcanic Minerals Unique"]="volcanicMineralsUnique",
["Training Receipts Unique"]="trainingReceiptsUnique",
["Golden Apple Basket"]="goldenAppleBasket",
...
w:AddDropdown("FilterTypeDropdown",{
Text="Filter",
Values=D,
Default=0,
Multi=true,
...
w:AddButton("Pickup Horse",function()
am.pickupFollower()
end)

w:AddDivider()

w:AddToggle("WebhookEnabled",{
Text="Use Webhook",
Default=false,
Tooltip="sends a message thru webhook each time a horse is locked",
Callback=function(E)
am.setWebhookEnabled(E)
end,
})


w:AddInput("WebhookURL",{
Text="Webhook URL",
Default="",
Numeric=false,
Finished=false,
...
w:AddButton("Test Webhook",function()
am.testWebhook()
end)

local E=ak.getCraftableLassos()
local F={}
local G={}

for H,I in ipairs(E)do
local J=I.name.." (str "..I.strength..")"
table.insert(F,J)
G[J]=I.id
end

x:AddToggle("AutoCraftLasso",{
Text="Auto Craft",
Default=false,
Tooltip="makes lassos when ur ur running low",
Callback=function(H)
ak.setCraftEnabled(H)
end,
})

x:AddDropdown("LassoToCraft",{
Text="Lasso Type",
Values=F,
Default=1,
Multi=false,
...
Callback=function(H)
local I=G[H]
if I then ak.setSelectedLasso(I)end
end,
Disabled=false,
Visible=true,
})

x:AddSlider("LassoRestockThreshold",{
...
Callback=function(H)
ak.setRestockThreshold(H)
end,
})

x:AddSlider("LassoRestockAmount",{
Text="Restock Amount",
Default=100,
Min=1,
Max=250,
...
Callback=function(H)
ak.setRestockAmount(H)
end,
})

x:AddButton("Restock Now",function()
ak.triggerRestock()
end)


local H=m.Main:AddRightTabbox()

local I=H:AddTab("Ores")
local J=H:AddTab("Pickaxe")
local K=H:AddTab("Render")

I:AddToggle('AutoMine',{
Text='Mine',
Default=false,
Risky=true,
...
Callback=function(L)
ao.setClickCooldown(L)
end,
})


I:AddSlider("IdleThreshold",{
Text="Idle Threshold",
Default=5,
Min=1,
Max=30,
...
Callback=function(L)
ao.setIdleThreshold(L)
end,
})

local L=ao.getOreValues()

I:AddDropdown("OreSelector",{
Text="Ore Types",
Values=L,
Default=0,
Multi=true,
...
ao.setOreTarget(O,false)
end

for N,O in pairs(M)do
if O then
ao.setOreTarget(N,true)
end
end
end,
...
local N=require(M.References)
local O=N.Utilities.Network
...
J:AddButton("Buy Pickaxe",function()
local S=P[R]
if not S then return end


O:FireServer("Shopping","BuyShopItem",S.shop,S.idx,1,nil)
end)

K:AddToggle("HighlightOre",{
Text="Highlight",
Default=false,
...
local S=m.Main:AddLeftTabbox()

local T=S:AddTab("Train")

T:AddToggle("AutoTrainEnabled",{
Text="Cross Country",
Default=false,
...
T:AddToggle("AutoSingleStakeEnabled",{
Text="Single Stake",
Default=false,
...
local U=S:AddTab("Settings")

U:AddSlider("TrainSpeed",{
Text="Speed",
Default=42,
Min=20,
Max=80,
...
local V=m.Main:AddRightGroupbox("Treasure")

V:AddToggle("TreasureEnabled",{
Text="Enable",
Default=false,
...
local W=m.Misc:AddLeftTabbox()

local X=W:AddTab("Player")
local Y=W:AddTab("Horse")

X:AddToggle("WalkspeedEnabled",{
Text="Walkspeed",
Default=false,
...
local Z=m.Misc:AddRightGroupbox("Performance")

local _=Instance.new("ScreenGui")
_.Name="BackgroundCover"
_.DisplayOrder=-1
_.IgnoreGuiInset=true
_.Parent=game:GetService("CoreGui")

local as=Instance.new("Frame",_)
...
at.Font=Enum.Font.SourceSansBold
at.TextSize=18
at.ZIndex=2
...
if not as.Visible then continue end
if not am then continue end

local az,aA=pcall(function()return am.getStats()end)
if not az or not aA then continue end

local aB={}
table.insert(aB,string.format("Sold: %d   Locked: %d   Coins: %d",
aA.sold,aA.locked,aA.coins))
table.insert(aB,"")

for aC,aD in ipairs(av)do
if aw[aD]then
local aE=aA.lockedByReason and aA.lockedByReason[aD]or 0
table.insert(aB,string.format("%s: %d",au[aD],aE))
end
end

at.Text=table.concat(aB,"\n")
end
end)
local az={}

Z:AddToggle('MuteAmbientMusic',{
Text='Ambient Music',
Default=false,
Tooltip='Turns on or off ambient music or sounds',
Callback=function(aA)
local aB=game:GetService("SoundService")
local aC=aB:GetDescendants()

for aD,aE in ipairs(aC)do
if aE:IsA("Sound")then
if aA then
...
Z:AddToggle('NoGraphics',{
Text='No Graphics',
Default=false,
Tooltip='Disables 3D rendering with a black background',
Callback=function(aA)
do
game:GetService("RunService"):Set3dRenderingEnabled(not aA)
as.Visible=aA
end
end
})

Z:AddDropdown("OverlayStatsDisplay",{
Text="Overlay Stats",
Values=ax,
Default=ax,
Multi=true,
...
local aC=m.Misc:AddLeftGroupbox("Redeem")

aC:AddButton("Redeem Codes",function()
task.spawn(function()
local aD=game:GetService("ReplicatedStorage")
local aE=require(aD:WaitForChild("References"))
local aF=aE.Flags
local aG=require(aE.PlayerScripts.Priority:WaitForChild("Data"))
local aH=aE.Utilities.Network

local aI=aF.flags and aF.flags.codes or{}
local aJ,aK=0,0

for aL,aM in pairs(aI)do
if aG.GetLocal({"codesRedeemed",aL})==true then
aK+=1
else
aH:FireServer("Codes","Submit",aL)
...
task.spawn(function()
local aH=game:GetService("ReplicatedStorage")
local aI=require(aH:WaitForChild("References"))
local aJ=aI.Utilities.Network


for aK=1,aG do
local aL,aM=pcall(function()
return aJ:InvokeServer("TradeIn","Trade",aE)
end)
if aL and aM then
...
if aK<aG then task.wait(0.8)end
end
...
aC:AddButton("Golden Apples (20)",function()
aD("goldenAppleBasket","Golden Apples",1)
end)

aC:AddButton("Volcanic Minerals (5)",function()
aD("volcanicMinerals","Volcanic Minerals",1)
end)

aC:AddButton("Training Receipts (100)",function()
aD("trainingReceipts","Training Receipts",1)
end)
...
local aF=m.Misc:AddRightGroupbox("Teleport")




local aG=game:GetService("ReplicatedStorage")
local aH=require(aG:WaitForChild("References"))
local aI=require(aH.PlayerScripts:WaitForChild("Secondary"):WaitForChild("TravelHandler"))
...
aF:AddButton("Travel",function()
aL(selected_island)
end)

af:SetWatermarkVisibility(true)

local aM=tick()
local aN=0;
local aO=60;
local aP=(function()return math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())end)
local aQ=pcall(function()return aP()end)

local aR=game:GetService("RunService").RenderStepped:Connect(function()
aN+=1;
...
if aQ then
af:SetWatermark(("fantasyhaxx - [buyer build] / %d fps / %d ms"):format(
math.floor(aO),
aP()
));
else
af:SetWatermark(("fantasyhaxx - [buyer build] / %d fps"):format(
math.floor(aO)
));
end
end);
...
local aS=m["UI Settings"]:AddLeftGroupbox("Menu")




aS:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind",{Default="RightShift",NoUI=true,Text="Menu keybind"})
aS:AddButton("Unload",function()af:Unload()end)
...
ai:SetIgnoreIndexes({"MenuKeybind"})




ah:SetFolder("coconut")
ai:SetFolder("coconut/whi")
...
ai:BuildConfigSection(m["UI Settings"])



ah:ApplyToTab(m["UI Settings"])
...
af:Notify("Loaded in "..tostring(string.format("%."..tostring(3).."f",os.clock()-aa)).." seconds",5)
